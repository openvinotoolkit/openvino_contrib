...
Run it manually (dry-run is the default, so nothing is deleted):
    ...
    GITHUB_TOKEN=<token> GITHUB_REPOSITORY=openvinotoolkit/openvino_contrib \\
...
def get_recent_open_pr_branches(repository, max_age_days: int) -> set[str]:
    ...
    for pull in repository.get_pulls(state="open", sort="updated", direction="desc"):
...
def branch_has_open_pr(repository, owner_login: str, branch_name: str) -> bool:
    ...
    """
    open_pulls = repository.get_pulls(state="open", head=f"{owner_login}:{branch_name}")
...
def list_dependabot_branches(repository) -> list[str]:
    ...
    return [branch.name for branch in repository.get_branches() if is_dependabot_branch(branch.name)]
...
def run_cleanup(repository, dry_run: bool, open_pr_max_age_days: int) -> CleanupResult:
    ...
    """
    recent_open_pr_branches = get_recent_open_pr_branches(repository, open_pr_max_age_days)
    logger.info(
    ...
    logger.info(f'Found {len(dependabot_branches)} "{DEPENDABOT_BRANCH_PREFIX}" branch(es) to inspect')
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Repository in OWNER/REPO format (defaults to the GITHUB_REPOSITORY env var)",
    )
...
def build_config() -> Config | None:
    ...
    if not args.repository_name:
        logger.error("Repository name is required (pass --repository-name or set GITHUB_REPOSITORY)")
        return None
...
def main() -> int:
    ...
    logger.info(f'Cleaning dependabot branches in "{config.repository_name}" (dry_run={config.dry_run})')

    github = Github(auth=Auth.Token(token=config.token))
    try:
        repository = github.get_repo(config.repository_name)
        result = run_cleanup(repository, config.dry_run, config.open_pr_max_age_days)
    finally:
        github.close()
