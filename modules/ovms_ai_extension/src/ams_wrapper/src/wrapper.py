...
def start_rest_service(port, num_threads, ovms_port):
    ...
    #B104:hardcoded_bind_all_interfaces since we are an RestAPI inside container, 
    #so we need to bind to all interfaces
    server = WSGIServer(('0.0.0.0', port), dispatcher,numthreads=num_threads)   # nosec
