...
/**
 * Information about user's input tensor. By default, it will be initialized to same data
 * (type/shape/etc) as model's input parameter. User application can override particular parameters
 * (like 'element_type') according to application's data and specify appropriate conversions in
...
public class InputTensorInfo extends Wrapper {
    ...
    /**
     * By default, input image shape is inherited from model input shape. Use this method to specify
     * different width and height of user's input image. In case if input image size is not known,
     * use {@link InputTensorInfo#set_spatial_dynamic_shape} method.
     *
    ...
    /**
     * By default, input image shape is inherited from model input shape. This method specifies that
     * user's input image has dynamic spatial dimensions (width and height). This can be useful for
     * adding resize preprocessing from any input image to model's expected dimensions.
     *
     * @return Reference to 'this' to allow chaining with other calls in a builder-like manner.
     */
