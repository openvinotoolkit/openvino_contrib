...
def main():
    ...
    def eval_fn(model, eval_dataloader):
        ...
        return outputs.loss, eval_metric
    
    def train_fn(model, train_dataloader, optimizer, compression_ctrl):
        ...
        for epoch in range(starting_epoch, args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                # NNCF compression controller step
            ...
            # NNCF needs the loss and metric accuracy for optimization
            return loss, 1.0
    ...
    # NNCF suitable dataloader
    train_dataloader = QADataLoader(train_dataloader)

    # NNCF suitable dataloader
    eval_dataloader = QADataLoader(eval_dataloader)
    
    nncf_config = NNCFConfig.from_json('nncf_configs/nncf_bert_config_squad.json') 

    nncf_config = register_default_init_args(nncf_config, train_dataloader, model_eval_fn=eval_fn(model, eval_dataloader))
    print(f'NNCF config: {nncf_config}')
