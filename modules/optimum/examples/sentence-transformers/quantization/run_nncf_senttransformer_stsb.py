...
from sentence_transformers_nncf_utils import NNCFSTDataLoader
...
def main():
    ...
    def preprocess_function(examples):
        ...
        result = tokenizer(texts, padding="max_length", truncation=True, return_tensors='pt')

        if args.labels_name in examples:
            norm_label = examples[args.labels_name] / 5.0  # Normalize score to range 0 ... 1
            result["labels"] = norm_label
    ...
    def train_epoch_fn(compression_ctrl, compressed_model, epoch=None, optimizer=optimizer, lr_scheduler=lr_scheduler):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            outputs = compressed_model(input_ids=batch['input_ids'][0], attention_mask=batch['attention_mask'][0], return_dict=False)[0]
            
            # Perform pooling. In this case, mean pooling.
            sentence_embeddings = mean_pooling(outputs, batch['attention_mask'][0])

            cos_sim = torch.nn.CosineSimilarity(dim=0)
            cos_sim_output=cos_sim(sentence_embeddings[0],sentence_embeddings[1])

            loss = loss_criterion(cos_sim_output.view(-1), batch["labels"].view(-1))
            compression_loss = compression_ctrl.loss().clone().detach().requires_grad_(True)
    ...
    def validate_fn(model, epoch=None):
        ...
        for step, batch in enumerate(tqdm(eval_dataloader)):
            with torch.no_grad():
                ...
                predictions, references = cos_sim_output, batch["labels"]
    ...
    nncf_config = register_default_init_args(nncf_config, train_dataloader, model_eval_fn=validate_fn, criterion=loss_criterion)
