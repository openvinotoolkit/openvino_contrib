...
FusedConvolutionBackpropDataOp::FusedConvolutionBackpropDataOp(
    ...
    , add_in_bytes_{ngraph::element::Type(params_.conv_.element_type_).size() * ngraph::shape_size(params_.add_shape_)} {
...
std::size_t
FusedConvolutionBackpropDataOp::GetBufferSize(const ngraph::Output<ngraph::Node>& output) {
    return output.get_element_type().size() * shape_size(output.get_shape());
}

OPERATION_REGISTER(FusedConvolutionBackpropDataOp, FusedConvBackpropData2D);
