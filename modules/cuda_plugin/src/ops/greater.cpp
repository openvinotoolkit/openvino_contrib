...
GreaterOp::GreaterOp(const CreationContext& context,
    ...
    kernel_ = kernel::Greater{
        convertDataType<CUDAPlugin::kernel::Type_t>(element_type), output_size, num_blocks, threads_per_block};
}

void GreaterOp::Execute(const InferenceRequestContext& context,
                        Inputs inputs,
                        Outputs outputs,
...
void GreaterOp::InitSharedImmutableWorkbuffers(const Buffers& buffers) {
...
WorkbufferRequest GreaterOp::GetWorkBufferRequest() const {
    return {std::vector<WorkbufferRequest::size_in_bytes_t>(SIZES + 1, kOffsetBufferSize), {}};
}

void GreaterOp::calculateOffsets() {
