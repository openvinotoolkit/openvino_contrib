...
MemorySolver::MemorySolver(const std::vector<Box> &boxes) : _boxes(boxes) {
    ...
    for (Box &box : _boxes)
        if (box.finish == -1) box.finish = max_ts;
...
int64_t MemorySolver::solve() {
    ...
    std::sort(_boxes.begin(), _boxes.end(), [](const Box &l, const Box &r) { return l.size > r.size; });
    ...
    for (Box &box : _boxes) {
        ...
        for (int i_slot = box.start; i_slot <= box.finish; i_slot++) time_slots[i_slot].push_back(&box);
...
int64_t MemorySolver::getOffset(int id) const {
    ...
    if (res == _offsets.end()) throwIEException("There are no box for provided ID");
    return res->second;
}
