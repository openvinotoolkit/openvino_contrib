...
MemoryManagerPool::MemoryManagerPool(const size_t num,
...
MemoryManagerPool::Proxy MemoryManagerPool::WaitAndGet(CancellationToken& cancellationToken) {
    std::unique_lock<std::mutex> lock{mtx_};
    cond_var_.wait(lock, [this, &cancellationToken] {
        cancellationToken.Check();
        return !memory_managers_.empty();
    });
    Proxy memoryManagerProxy{shared_from_this(), move(memory_managers_.back())};
    memory_managers_.pop_back();
    return memoryManagerProxy;
}

size_t MemoryManagerPool::Size() const { return memory_managers_.size(); }
...
void MemoryManagerPool::Resize(size_t count) {
    ...
    if (count > memoryManagersCount) {
        throwIEException(
            fmt::format("Cannot resize MemoryManagerPool with {} value > than it was {}", count, memoryManagersCount));
    }
