...
struct FusedConvCallbacks {
    ...
    static bool fuse_convolution_with_biasadd(ngraph::pattern::Matcher &m) {
        ...
        auto [m_conv, m_const] =
            parse_eltwise_inputs<typename TFusedConvolution::BaseConvolution, ngraph::op::Constant>(eltwise);
        if (!m_conv || !m_const) {
            return false;
        }
        ...
        const ngraph::Output<ngraph::Node> &data = m_conv->input(0).get_source_output();
        const ngraph::Output<ngraph::Node> &filters = m_conv->input(1).get_source_output();
        const ngraph::Output<ngraph::Node> &bias = m_const->output(0);
    ...
    static std::pair<std::shared_ptr<TFusedConvolution>, std::shared_ptr<ngraph::Node>> parse_fusedconv_inputs(
        std::shared_ptr<ngraph::Node> add) {
        std::shared_ptr<TFusedConvolution> fused_conv = nullptr;
        std::shared_ptr<ngraph::Node> node = nullptr;
    ...
    static bool sink_add_to_fused_convolution(ngraph::pattern::Matcher &m) {
        ...
        const ngraph::Output<ngraph::Node> &data = fused_conv->input(0).get_source_output();
        const ngraph::Output<ngraph::Node> &filters = fused_conv->input(1).get_source_output();
        const ngraph::Output<ngraph::Node> &bias = fused_conv->input(2).get_source_output();
        ...
        if (rt_info.count(ExecGraphInfoSerialization::ORIGINAL_NAMES) > 0) {
            ...
            const auto original_names =
                std::dynamic_pointer_cast<ngraph::VariantImpl<std::string>>(rt_info_layer_names);
...
bool fuse_convolution_backprop_data_with_add(ngraph::pattern::Matcher &m) {
    ...
    auto [conv_backprop_data, add_constant] =
        parse_eltwise_inputs<ngraph::opset1::ConvolutionBackpropData, ngraph::op::Constant>(add);
    ...
    const ngraph::Output<ngraph::Node> &data = conv_backprop_data->input(0).get_source_output();
    const ngraph::Output<ngraph::Node> &filters = conv_backprop_data->input(1).get_source_output();
...
ngraph::pass::FuseConvolutionWithBiasAdd::FuseConvolutionWithBiasAdd() {
    auto conv = ngraph::pattern::wrap_type<ngraph::opset1::Convolution>(pattern::consumers_count(1));
    auto add = ngraph::pattern::wrap_type<ngraph::opset1::Add>({conv, pattern::any_input()});
...
ngraph::pass::FuseGroupConvolutionWithBiasAdd::FuseGroupConvolutionWithBiasAdd() {
    auto conv = ngraph::pattern::wrap_type<ngraph::opset1::GroupConvolution>(pattern::consumers_count(1));
    auto add = ngraph::pattern::wrap_type<ngraph::opset1::Add>({conv, pattern::any_input()});
...
ngraph::pass::FuseConvolutionWithBiasAddAdd::FuseConvolutionWithBiasAddAdd() {
    ...
    auto relu = ngraph::pattern::wrap_type<ngraph::op::Relu>(pattern::consumers_count(2));
    auto add = ngraph::pattern::wrap_type<ngraph::opset1::Add>({pattern::any_input(), fused_convolution});
...
ngraph::pass::SinkReluToFusedConvolution::SinkReluToFusedConvolution() {
    auto fused_convolution = ngraph::pattern::wrap_type<FusedConvolution>(pattern::consumers_count(1));
    auto activation = ngraph::pattern::wrap_type<ngraph::op::Relu>({fused_convolution});
...
ngraph::pass::SinkSigmoidToFusedConvolution::SinkSigmoidToFusedConvolution() {
    auto fused_convolution = ngraph::pattern::wrap_type<FusedConvolution>(pattern::consumers_count(1));
    auto activation = ngraph::pattern::wrap_type<ngraph::op::Sigmoid>({fused_convolution});
...
ngraph::pass::SinkTanhToFusedConvolution::SinkTanhToFusedConvolution() {
    auto fused_convolution = ngraph::pattern::wrap_type<FusedConvolution>(pattern::consumers_count(1));
    auto activation = ngraph::pattern::wrap_type<ngraph::op::Tanh>({fused_convolution});
...
ngraph::pass::CudaFuseConvBackpropDataAdd::CudaFuseConvBackpropDataAdd() {
    auto conv_backprop_data =
        ngraph::pattern::wrap_type<ngraph::opset1::ConvolutionBackpropData>(pattern::consumers_count(1));
    auto add = ngraph::pattern::wrap_type<ngraph::opset1::Add>({conv_backprop_data, pattern::any_input()});
