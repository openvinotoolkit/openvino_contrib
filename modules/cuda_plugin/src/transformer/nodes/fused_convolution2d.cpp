...
#include "fused_convolution2d.hpp"
...
bool FusedConv2D::visit_attributes(ngraph::AttributeVisitor& visitor) {
...
std::shared_ptr<ngraph::Node> FusedConv2D::clone_with_new_inputs(
    const ngraph::OutputVector& new_args) const {
  check_new_args_count(this, new_args);
    if (new_args.size() == 3) {
        return std::make_shared<FusedConv2D>(
...
void FusedConv2D::conv_validate_and_infer_types() {
...
void FusedConv2D::validate_and_infer_types() {
...
bool FusedConv2D::has_add_node() const {
  return has_add_node_;
}

void FusedConv2D::set_activation(ActivationMode act) {
  activation_ = act;
}

ActivationMode FusedConv2D::get_activation() const {
  return activation_;
}
