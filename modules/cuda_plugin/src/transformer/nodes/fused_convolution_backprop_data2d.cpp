...
#include "fused_convolution_backprop_data2d.hpp"
...
FusedConvBackpropData2D::FusedConvBackpropData2D(
...
bool FusedConvBackpropData2D::visit_attributes(ngraph::AttributeVisitor& visitor) {
...
std::shared_ptr<ngraph::Node> FusedConvBackpropData2D::clone_with_new_inputs(
    const ngraph::OutputVector& new_args) const {
  check_new_args_count(this, new_args);
  if (new_args.size() == 3) {
    return std::make_shared<FusedConvBackpropData2D>(
        new_args.at(0), new_args.at(1), new_args.at(2),
        strides_, pads_begin_, pads_end_, dilations_, auto_pad_, output_padding_);
  } else {
    return std::make_shared<FusedConvBackpropData2D>(
...
void FusedConvBackpropData2D::conv_validate_and_infer_types() {
  ...
  if (data_pshape.rank().is_static() && filters_pshape.rank().is_static())
  {
    ...
    NODE_VALIDATION_CHECK(this,
                          strides_.size() == num_spatial_dims,
...
ngraph::PartialShape FusedConvBackpropData2D::get_output_shape() const
{
  auto data_pshape = get_input_partial_shape(0);
...
void FusedConvBackpropData2D::infer_conv_backprop_output_spatial_shape(
...
void FusedConvBackpropData2D::validate_and_infer_types() {
