...
class DevicePointer : private gsl::not_null<T> {
 public:
  ...
  auto cast() const noexcept {
      ...
      static_assert(std::is_void_v<std::remove_pointer_t<T>> ||
  ...
  auto as_mutable() const noexcept {
    return DevicePointer<std::remove_const_t<std::remove_pointer_t<T>>*>{
        const_cast<std::remove_const_t<std::remove_pointer_t<T>>*>(
            this->get())};
  }
};
...
auto operator+(DevicePointer<T*> p, std::ptrdiff_t d) noexcept {
    return DevicePointer<T*>{static_cast<T*>(
        const_cast<char*>(static_cast<const char*>(p.get()) + d))};
}
...
class DeviceBuffer : private gsl::span<T, Extent> {
 public:
  explicit DeviceBuffer(gsl::span<T, Extent> o) noexcept
      : gsl::span<T, Extent>{o} {}
  using gsl::span<T, Extent>::data;
  using gsl::span<T, Extent>::size;

  auto as_mutable() const noexcept {
      return DeviceBuffer<std::remove_const_t<T>>{
          const_cast<std::remove_const_t<T>*>(this->data()), this->size()};
  }
};
