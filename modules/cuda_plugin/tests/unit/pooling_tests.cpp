...
std::shared_ptr<NGraphPoolingNode> build_ngraph_pooling_node(
    NodePtr input, const Strides& strides, const Shape& pads_begin,
    const Shape& pads_end, const Shape& kernel) {}

template <>
std::shared_ptr<ngraph::op::v1::MaxPool> build_ngraph_pooling_node(
    NodePtr input, const Strides& strides, const Shape& pads_begin,
    const Shape& pads_end, const Shape& kernel) {
  auto node = std::make_shared<ngraph::op::v1::MaxPool>(
      input->output(0), strides, pads_begin, pads_end, kernel);
  return node;
}
...
std::shared_ptr<ngraph::op::v1::AvgPool> build_ngraph_pooling_node(
  ...
  auto node = std::make_shared<ngraph::op::v1::AvgPool>(
      input->output(0), strides, pads_begin, pads_end, kernel,
      exclude_padding_from_pooling);
  return node;
}

template <class NGraphPoolingNode>
std::shared_ptr<NGraphPoolingNode> build_ngraph_pooling_dummy() {
  auto const_input_node = std::make_shared<ngraph::op::Constant>(
      default_data_type, min_supported_pooling_shape);
  return build_ngraph_pooling_node<NGraphPoolingNode>(
      const_input_node, dummy_strides, dummy_padding, dummy_padding,
      dummy_kernel);
}
...
struct PoolingTest : testing::Test {
  ...
  void test(gsl::span<const float> input, std::vector<size_t> in_shape,
    ...
    auto const_input = std::make_shared<ngraph::op::Constant>(
        ngraph::element::f32, Shape{in_shape});
...
class MaxPoolRegistryTest
    : public PoolingRegistryTest<ngraph::op::v1::MaxPool, MaxPoolOp> {};

TEST_F(MaxPoolRegistryTest, GetOperationBuilder_Available) {
  ASSERT_TRUE(OperationRegistry::getInstance().hasOperation(
      std::make_shared<ngraph::op::v1::MaxPool>()));
}
...
class AvgPoolRegistryTest
    : public PoolingRegistryTest<ngraph::op::v1::AvgPool, AvgPoolOp> {};

TEST_F(AvgPoolRegistryTest, GetOperationBuilder_Available) {
  ASSERT_TRUE(OperationRegistry::getInstance().hasOperation(
      std::make_shared<ngraph::op::v1::AvgPool>()));
}
