...
public:
    static std::string getTestCaseName(const testing::TestParamInfo<gatherParamsTuple> &obj) {
        ...
        std::tie(indices, indicesShape, axis, inputShape,
                 netPrecision, inPrc, outPrc, inLayout, outLayout, targetName) = obj.param;
...
INSTANTIATE_TEST_CASE_P(smoke_Gather_01, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(smoke_01_params)),
...
INSTANTIATE_TEST_CASE_P(smoke_Gather_02, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(smoke_02_params)),
...
INSTANTIATE_TEST_CASE_P(smoke_Gather_03, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(smoke_03_params)),
...
INSTANTIATE_TEST_CASE_P(smoke_Gather_04, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(smoke_04_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_Tacotron2_shapes_enc, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(tacotron2_enc_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_LPCnet_shapes_enc, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(lpcnet_enc_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_LPCnet_shapes_dec, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(lpcnet_dec_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_OV_v1, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(ov_v1_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_big_01, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(big_01_params)),
...
INSTANTIATE_TEST_CASE_P(Gather_big_02, CudaGatherLayerTest,
                        ::testing::Combine(
                            ::testing::Values(generate_indices<int>(big_02_params)),
...
const ParamsVec all_params = { smoke_01_params, smoke_02_params, smoke_03_params, smoke_04_params,
                               tacotron2_enc_params, lpcnet_enc_params, lpcnet_dec_params,
                               ov_v1_params, big_01_params, big_02_params };
...
void test_one_shape(const GatherTestParams& params) {
    ...
    CUDAPlugin::OperationBase::Ptr operation = [&] {
        ...
        auto dict_param = std::make_shared<ngraph::op::v0::Parameter>(
            ngraph::element::from<ElementType>(), ngraph::PartialShape { params.params_shape_ });
        auto indices_param = std::make_shared<ngraph::op::v0::Parameter>(
            ngraph::element::from<IndicesType>(), ngraph::PartialShape { params.indices_shape_ });
        auto axis_constant = std::make_shared<ngraph::op::v0::Constant>(
            ngraph::element::i64, ngraph::Shape({ }), std::vector<int64_t> { params.axis_ });
        auto node = std::make_shared<ngraph::op::v1::Gather>(
            dict_param->output(0),
            indices_param->output(0),
            axis_constant);
        ...
        auto op = registry.createOperation(
            ...
            node, std::array { CUDAPlugin::TensorID { } }, std::array { CUDAPlugin::TensorID { } });
        return op;
    }();
    ...
    auto gen_dict = [&dist, &mersenne_engine](){
        return static_cast<IndicesType>(
            10.f * dist(mersenne_engine) / std::numeric_limits<int>::max());
    };
    ...
    std::cout << std::fixed << std::setfill('0')
              << "Gather " << CommonTestUtils::vec2str(params.params_shape_) << ", "
