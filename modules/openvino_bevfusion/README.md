...
We implement BEVFusion and support training and testing on multi-sensor 3D detection datasets.
...
Run a demo using [BEVFusion model](https://drive.google.com/file/d/1QkvbYDk4G2d6SZoeJqish13qSyXA4lp3/view?usp=share_link):

```shell
python projects/BEVFusion/demo/multi_modality_demo.py \
    /path/to/lidar.pcd.bin \
    /path/to/data_root/ \
    /path/to/sample_info.pkl \
    projects/BEVFusion/configs/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d.py \
    ${CHECKPOINT_FILE} --cam-type all --score-thr 0.2 --show
```
...
```bash
bash tools/dist_train.py projects/BEVFusion/configs/${LIDAR_ONLY_CONFIG}.py 8
```
...
```bash
bash tools/dist_train.sh projects/BEVFusion/configs/${FUSION_CONFIG}.py 8 --cfg-options load_from=${LIDAR_PRETRAINED_CHECKPOINT} model.img_backbone.init_cfg.checkpoint=${IMAGE_PRETRAINED_BACKBONE}
```
...
```bash
bash tools/dist_test.sh projects/BEVFusion/configs/${FUSION_CONFIG}.py ${CHECKPOINT_PATH} 8
```
...
| :------: | :---------------------: | :-: | :------: | :------------: | :--: | :--: | :------: |
| lidar    |      voxel (0.075)      |  ×  |    -     |       -        | 69.6 | 64.9 | [model](https://download.openmmlab.com/mmdetection3d/v1.1.0_models/bevfusion/bevfusion_lidar_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d-2628f933.pth) |
| lidar-cam |     voxel (0.075)      |  ×  |    -     |       -        | 71.4 | 68.6 | [model](https://download.openmmlab.com/mmdetection3d/v1.1.0_models/bevfusion/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d-5239b1af.pth) |
