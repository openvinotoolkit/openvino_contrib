 ...
 */
/*
 * Ball Query Single - GPU kernel with BFYX format
 * Supports FP16/FP32 via OpenVINO type macros
 * Matches CPU behavior: uses -1 for unfilled slots
 *
...
__kernel void ball_query_single_v2_kernel(
          ...
          __global OUTPUT0_TYPE* output)
{
    ...
    // Storage for K nearest neighbors within radius
    ...
    // Find K nearest neighbors within radius (strict <, not <=)
    for (int j = 0; j < N2; j++) {
        ...
        // CPU uses strict < (not <=)
        if (dist_sq < radius_sq && dist_sq < best_dists[K-1]) {
            int insert_pos = K - 1;
    ...
    // Write output - use -1 for unfilled slots (match CPU behavior)
    ...
    for (int k = 0; k < K; k++) {
        if (best_idx[k] >= 0) {
            ...
            output[out_base + k * OUTPUT0_PITCHES[2]] = (OUTPUT0_TYPE)(-1.0f);
            output[out_base + (K + k) * OUTPUT0_PITCHES[2]] = (OUTPUT0_TYPE)(-1.0f);
        }
    }
}
