...
def _solve_qp(P, q, G=None, h=None, A=None, b=None):
    """Solve a QP: minimize 0.5 x'Px + q'x subject to Gx <= h, Ax = b."""
    ...
    if G is not None and h is not None:
        ...
        h_ = np.array(h, dtype=float).ravel()
        constraints.append({'type': 'ineq', 'fun': lambda x, G=G_, h=h_: h - G @ x})
    if A is not None and b is not None:
        A_ = np.array(A, dtype=float)
        b_ = np.array(b, dtype=float).ravel()
        constraints.append({'type': 'eq', 'fun': lambda x, A=A_, b=b_: A @ x - b})
    result = _scipy_minimize(
        fun=lambda x: 0.5 * x @ P @ x + q @ x,
        x0=np.zeros(n),
        jac=lambda x: P @ x + q,
        method='SLSQP',
...
class PointGraspMetrics3D:
    ...
    def wrench_in_positive_span(wrench_basis,
        ...
        v = _solve_qp(P, q, G, h).reshape(-1, 1)
    ...
    def min_norm_vector_in_facet(facet, wrench_regularizer=1e-10):
        ...
        P_mat = 2 * grasp_matrix  # quadratic cost for Euclidean dist
        q_vec = np.zeros(dim)
        G_mat = -np.eye(dim)  # greater than zero constraint
        h_vec = np.zeros(dim)
        A_mat = np.ones((1, dim))  # sum constraint to enforce convex
        b_vec = np.ones(1)  # combinations of vertices

        v = _solve_qp(P_mat, q_vec, G_mat, h_vec, A_mat, b_vec)
        v = v.reshape(-1, 1)
        min_norm = np.sqrt(0.5 * v.ravel() @ P_mat @ v.ravel())
