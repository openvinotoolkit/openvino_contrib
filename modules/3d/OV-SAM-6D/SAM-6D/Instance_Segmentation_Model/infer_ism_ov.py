...
class OVSamPredictor:
    ...
    Loads two compiled OV models:
      - SAM image encoder  (input 1x3x1024x1024 -> output 1x256x64x64)
      - SAM mask decoder   (fused prompt_encoder + mask_decoder)

    Provides the same interface as SamPredictor: ``set_image``,
    ``predict_torch``, ``reset_image``, ``device``, ``transform``.
    """
    ...
    def __init__(
        ...
        # SAM pixel normalisation (same as Sam.pixel_mean / Sam.pixel_std)
        ...
        # at init and reused on every call, preventing USM buffer accumulation
        # across images that previously caused SIGSEGV on the 2nd image.
    ...
    # ---- public API (matches SamPredictor) ----
    ...
    def set_torch_image(
        ...
        # Use persistent request: reuses GPU buffers, no new USM allocation.
...
class OVCustomSamAutomaticMaskGenerator:
    ...
    Replicates the same mask-generation logic (point-grid prompts, NMS,
    stability filtering) but delegates all NN calls to ``OVSamPredictor``.
    """
    ...
    def _build_nms_model(self):
        ...
        # NMS is called with a different number of boxes every image.
        # Running it on the GPU causes OV to cache a per-shape set of
        # activation buffers for each unique N, accumulating GPU USM
        # memory across images until the driver limit is hit (SIGSEGV).
        # CPU has no such accumulation problem and NMS is fast enough.
        device = "CPU"
        core = getattr(self.predictor, "ov_core", None) or ov.Core()
        self._nms_core = core  # keep reference so caller can reset
        self._nms_compiled = core.compile_model(model, device)
        # Persistent request: buffers reused every NMS call.
        self._nms_request = self._nms_compiled.create_infer_request()
        logging.info(f"OV NMS model compiled on {device} (CPU avoids GPU USM accumulation)")
    ...
    def _ov_nms(self, boxes: torch.Tensor, scores: torch.Tensor,
        ...
        result = self._nms_request.infer({
            "boxes": boxes_np,
            "scores": scores_np,
    ...
    # ---- public API (matches CustomSamAutomaticMaskGenerator) ----
    ...
    def _generate_masks(self, image: np.ndarray) -> dict:
        ...
        # With single crop (crop_n_layers=0), masks are kept as tensors
        # (no RLE roundtrip). Multi-crop path still uses RLE for dedup.
    ...
    def _process_crop(
        ...
        # Accumulate batch results in lists to avoid O(n^2) tensor cat
        batch_results = []
        ...
        # Combine all batches in one pass (O(n) instead of O(n^2))
        data = MaskData()
        if batch_results:
    ...
    def _process_batch(
        ...
        if has_v2:
            ...
            # masks: [B, 3, H, W] binary float32
            # stability_scores_v2: [B, 3]
            # boxes_v2: [B, 3, 4]

            data = MaskData(
                masks=masks.flatten(0, 1),             # [B*3, H, W]
                iou_preds=iou_preds.flatten(0, 1),     # [B*3]
                points=torch.as_tensor(
                    points.repeat(masks.shape[1], axis=0)),
                stability_score=stability_scores_v2.flatten(0, 1),  # [B*3]
                boxes=boxes_v2.reshape(-1, 4),         # [B*3, 4]
            )
            del masks
            ...
            # Filter by stability score (already computed on GPU)
            if self.stability_score_thresh > 0.0:
                keep_mask = data["stability_score"] >= self.stability_score_thresh
                data.filter(keep_mask)

            # Masks are already binary from v2 decoder
            data["masks"] = data["masks"] > 0.5

            # Boxes already computed on GPU — skip batched_mask_to_box
            # (boxes_v2 was already placed in data["boxes"])
        ...
        # Compress to RLE (only needed for multi-crop dedup)
...
class OVCustomDINOv2:
    ...
    Pre/post-processing (normalise, crop, resize, mask) stays in
    PyTorch/NumPy on the host.
    """
    ...
    def __init__(
        ...
        # Dummy ``model`` attribute so detector.move_to_device() doesn't crash
        self.model = type(
    ...
    def _ov_forward(
        ...
        # Always pad the batch to chunk_size so OV sees exactly ONE shape.
        # Without this, each unique last-chunk size causes OV GPU plugin to
        # allocate and cache a separate set of activation buffers on the GPU,
        # which accumulates across images and causes SIGSEGV from OOM.
        N = images_np.shape[0]
    ...
    # ---- pre-processing (identical to CustomDINOv2) ----
...
# ===================================================================
#  Extended OV classes: custom ops baked into IR (no CPU pre/post)
# ===================================================================
...
class OVSamPredictorExt:
    ...
    No Python pre/post-processing — everything runs on the OV device.
    """
    ...
    def set_image(
        ...
        result = self.encoder_compiled({'image': input_4d})
        self.features = result[0]         # [1, 256, 64, 64]
        input_size = result[1]            # [1, 1, 1, 2] -> (new_h, new_w)
    ...
    def predict_torch(
        ...
        result = self.decoder_compiled(
            [self.features, coords_np, labels_np]
        )

        n_outputs = len(self.decoder_compiled.outputs)
        if n_outputs >= 4:
            # v2 decoder: masks, iou, stability_scores, boxes (no logits)
            masks = torch.from_numpy(np.array(result[0]))       # [B,3,H,W]
            iou_preds = torch.from_numpy(np.array(result[1]))   # [B, 3]
            stability_scores = torch.from_numpy(np.array(result[2]))  # [B, 3]
            boxes = torch.from_numpy(np.array(result[3]))       # [B, 3, 4]

            # v2 has no logits — return masks with stability_scores + boxes
            return masks, iou_preds, (stability_scores, boxes)
        else:
            # v1 decoder: masks, iou, logits
            masks = torch.from_numpy(np.array(result[0]))       # [B,3,H,W]
            iou_preds = torch.from_numpy(np.array(result[1]))   # [B, 3]
            logits = torch.from_numpy(np.array(result[2]))       # [B,3,H,W]
...
class OVCustomDINOv2Ext:
    """Extended OV DINOv2 using custom-op IR (patch postprocess baked in).
    ...
    Pre-processing (CropResizePad) still done in Python for now since
    it operates per-proposal with dynamic crop sizes. The DINOv2Preprocess
    custom op can be used as a standalone model for full GPU offload.
    """
    ...
    def _ov_forward_ext(
        ...
        images_np = images_tensor.cpu().float().numpy()
        # masks must be [N, 1, 224, 224]
        masks_np = masks_tensor.cpu().float().numpy()
        if masks_np.ndim == 3:
            masks_np = masks_np[:, np.newaxis, :, :]

        result = self.dinov2_compiled(
            [images_np, masks_np]
        )
        cls_token = torch.from_numpy(np.array(result[0]))
        patch_features = torch.from_numpy(np.array(result[1]))
    ...
    def _ov_forward(
        ...
        images_np = images_tensor.cpu().float().numpy()
        N = images_np.shape[0]
        # For cls_token only, pass all-ones masks (all patches valid)
        dummy_masks = np.ones(
            (N, 1, self.proposal_size, self.proposal_size), dtype=np.float32)
        result = self.dinov2_compiled([images_np, dummy_masks])
        cls_token = torch.from_numpy(np.array(result[0]))
...
# ===================================================================
#  OVFastSAM — OpenVINO ``model.fast_sam.FastSAM``
# ===================================================================

class OVFastSAM:
    """OpenVINO-based FastSAM (YOLOv8x-seg) mask generator.

    Implements the same ``generate_masks(image) -> {"masks", "boxes"}``
    interface as ``model.fast_sam.FastSAM``.

    Pre-processing (letterbox, normalize, HWC->CHW) and post-processing
    (NMS, proto->mask, resize) use Ultralytics ops for exact CUDA parity.
    Only the NN forward pass runs through OpenVINO.

    Uses ``auto=True`` letterbox (minimal stride-aligned padding) to match
    the CUDA SegmentationPredictor preprocessing exactly.  Requires the
    OV model to have dynamic spatial dimensions (fastsam_x_dynamic.xml).
    """
    ...
    def __init__(self, compiled_model, conf=0.25, iou=0.9, max_det=200,
        ...
        # Import ultralytics post-processing ops (same ones used by CUDA path)
    ...
    def generate_masks(self, image):
        ...
        # Preprocess (auto letterbox — matches CUDA)
        img_tensor = self._preprocess(image)
        model_h, model_w = img_tensor.shape[2], img_tensor.shape[3]

        # OV inference
        result = self.compiled(img_tensor)
        output0 = result[self.compiled.output(0)]  # [1, nc+4+nm, anchors]
        output1 = result[self.compiled.output(1)]  # [1, 32, mh, mw]

        # NMS — use exact ultralytics implementation
        preds = self._nms(
            ...
            nc=1,
            max_time_img=10.0,  # avoid JIT-warmup timeout
        )
        ...
        # Proto masks — use exact ultralytics process_mask
        ...
        # Resize masks to original size (matches CUDA FastSAM.postprocess_resize)
...
# ===================================================================
#  init_model_ov — for run_ism_batch.init_model
# ===================================================================
...
def init_model_ov(
    ...
    Returns the same 5-tuple as ``run_ism_batch.init_model()``:
        (model, cfg, device, selected_poses, proposal_processor)

    ``device`` is always ``torch.device("cpu")`` — OV manages the
    actual hardware placement via ``ov_device``.
    """
    ...
    def _compile(core, xml_path, device):
        ...
        if device.upper().startswith("GPU"):
            # LATENCY: 1 stream (prevents multi-stream GPU memory overcommit).
            # NUM_STREAMS=1: explicit single-stream to avoid pool growth.
    ...
    if segmentor_model_name == "sam":
        ...
        ov_predictor.ov_core = core  # share core so NMS + recompile use same instance
    ...
    # Torch device for pre/post-processing tensors (always CPU for OV)
...
def init_model_ov_ext(
    ...
    Uses *_ext.xml models that have pre/post-processing baked in using
    standard OV ops only — no custom C++ extension or GPU kernels needed.

    Returns the same 5-tuple as init_model_ov().
    """
    from hydra import initialize, compose
    from hydra.core.global_hydra import GlobalHydra
    from hydra.utils import instantiate
    ...
    # All ext models use standard OV ops only — no custom extension needed.
    ...
    if segmentor_model_name == "sam":
        ...
        # Prefer v2 decoder (stability_score + mask_to_box on GPU, no logits transfer)
        ...
        if os.path.isfile(sam_decoder_v2_xml):
            ...
            logging.info("Using v2 decoder (stability_score + mask_to_box on GPU)")
        else:
            sam_decoder_xml = sam_decoder_v1_xml
            logging.info("Using v1 decoder (stability_score + mask_to_box on CPU)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="OV ISM inference — smoke-test or single-image pipeline"
    )
    parser.add_argument(
        "--ov_model_dir",
    ...
    parser.add_argument(
        ...
        help="SAM grid size: NxN point prompts (default: 32, total=1024)",
    )
    parser.add_argument(
    ...
    if args.image:
        ...
        if args.cad and (os.path.isfile(args.cad) or os.path.isdir(args.cad)):
            # ---------------------------------------------------------------------------
            # Inlined from run_ism_batch (load_object_pointcloud, batch_input_data)
            # ---------------------------------------------------------------------------
            ...
            if template_dir is None:
                ...
                # -- Load template images (I/O, not timed) ----------
