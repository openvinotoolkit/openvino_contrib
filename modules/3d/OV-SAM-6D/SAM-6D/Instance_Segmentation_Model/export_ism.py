...
def _block_xformers():
    ...
    The DINOv2 layers check ``XFORMERS_AVAILABLE`` at module import time.
    If xformers is installed, those flags become True and the model uses
    fused kernels that cannot be traced to ONNX.

    Solution: insert a fake failing loader for 'xformers' into sys.modules
    so that ``from xformers.ops import ...`` raises ImportError.
    """
    if "xformers" in sys.modules:
        # Already imported — too late to block, must patch flags directly
...
# Block xformers BEFORE any model imports — only if user requests it
# (default: try with xFormers first to see if it works)
...
# ===================================================================
#  SAM Mask Decoder wrapper (fuses prompt encoder + mask decoder)
# ===================================================================

class SAMMaskDecoderWrapper(nn.Module):
    """Wraps prompt_encoder + mask_decoder into a single traceable module.

    The automatic mask generator uses POINT prompts only (no box, no mask).
    This wrapper inlines the prompt encoding to avoid boolean-index
    ``index_put_`` ops that OV's CPU plugin cannot handle with dynamic
    shapes.  Instead it uses ``torch.where`` / masked-multiply which
    trace to element-wise OV ops.
    ...
    def forward(self, image_embed, point_coords, point_labels):
        ...
        # --- inline _embed_points (avoids index_put_ from boolean indexing) ---
...
# ===================================================================
#  DINOv2 wrapper for cls + patch token export
# ===================================================================
...
def export_sam_image_encoder(sam, output_dir, compress_fp16=False,
    ...
    print(f"[export] Conversion took {elapsed:.1f}s")
...
def export_sam_encoder_ext(sam, output_dir, compress_fp16=False,
    ...
    Also outputs input_size[1,1,1,2] for use by mask postprocess.
    """
    ...
    perm = opset.constant(_np.array([0, 3, 1, 2], dtype=_np.int64))
    ...
    target_hw = opset.constant(_np.array([_new_h, _new_w], dtype=_np.int64))
    axes_hw = opset.constant(_np.array([2, 3], dtype=_np.int64))
    ...
    mean = opset.constant(
        _np.array([123.675, 116.28, 103.53],
                   dtype=_np.float32).reshape(1, 3, 1, 1))
    inv_std = opset.constant(
        _np.array([1.0 / 58.395, 1.0 / 57.12, 1.0 / 57.375],
                   dtype=_np.float32).reshape(1, 3, 1, 1))
    ...
    tensor_out = opset.pad(
        normalized,
        pads_begin=opset.constant(_np.array([0, 0, 0, 0], dtype=_np.int64)),
        pads_end=opset.constant(
            _np.array([0, 0, pad_h, pad_w], dtype=_np.int64)),
        arg_pad_value=opset.constant(_np.float32(0)),
        pad_mode='constant',
    )  # [1, 3, 1024, 1024]

    # Compute input_size as a Constant (deterministic from im_h, im_w)
    input_size_const = opset.constant(
        _np.array([[[[float(_new_h), float(_new_w)]]]], dtype=_np.float32),
...
def export_sam_decoder_ext(sam, output_dir, compress_fp16=False,
    ...
    target_1024 = opset.constant(_np.array([image_size, image_size], dtype=_np.int64))
    axes_hw = opset.constant(_np.array([2, 3], dtype=_np.int64))
    ...
    begin_crop = opset.constant(_np.array([0, 0, 0, 0], dtype=_np.int64))
    end_crop = opset.constant(
        _np.array([0, 0, input_h, input_w], dtype=_np.int64))
    cropped = opset.strided_slice(
        upsampled, begin_crop, end_crop,
        opset.constant(_np.array([1, 1, 1, 1], dtype=_np.int64)),
        begin_mask=[1, 1, 0, 0],   # pass-through for B and C
        end_mask=[1, 1, 0, 0],
    )

    # Step C: Interpolate to original size
    target_orig = opset.constant(_np.array([orig_h, orig_w], dtype=_np.int64))
    ...
    zero_const = opset.constant(_np.float32(0.0))
...
def export_sam_decoder_ext_v2(sam, output_dir, compress_fp16=False,
    ...
    entirely on GPU using standard OV ops. This eliminates ~10s of CPU
    ...
    (Logits NOT returned — saves ~50% GPU→CPU transfer)

    All ops are standard OV opset13 — no custom .cl or C++ extension needed.
    """
    ...
    begin_crop = opset.constant(_np.array([0, 0, 0, 0], dtype=_np.int64))
    end_crop = opset.constant(_np.array([0, 0, input_h, input_w], dtype=_np.int64))
    cropped = opset.strided_slice(
        upsampled, begin_crop, end_crop,
        opset.constant(_np.array([1, 1, 1, 1], dtype=_np.int64)),
        begin_mask=[1, 1, 0, 0],
    ...
    zero_const = opset.constant(_np.float32(0.0))
    masks_bool = opset.greater(logits_node, zero_const)
    masks_node = opset.convert(masks_bool, OVType.f32)  # [B, 3, orig_h, orig_w]
    ...
    offset_pos = opset.constant(_np.float32(1.0))   # mask_threshold + offset
    offset_neg = opset.constant(_np.float32(-1.0))   # mask_threshold - offset
    reduce_hw = opset.constant(_np.array([2, 3], dtype=_np.int64))
    ...
    eps_const = opset.constant(_np.float32(1e-6))
    ...
    # masks_node is [B, 3, orig_h, orig_w] float32 {0, 1}

    # in_height = max(mask, dim=-1(W))  → [B, 3, H]
    w_axis = opset.constant(_np.array([3], dtype=_np.int64))
    in_height = opset.reduce_max(masks_node, w_axis, keep_dims=False)  # [B, 3, H]

    # height coords: in_height * arange(H)
    h_range = opset.constant(
        _np.arange(orig_h, dtype=_np.float32).reshape(1, 1, -1))  # [1, 1, H]
    in_height_coords = opset.multiply(in_height, h_range)  # [B, 3, H]

    # bottom_edges = max(in_height_coords, dim=-1) → [B, 3]
    h_axis = opset.constant(_np.array([2], dtype=_np.int64))
    ...
    h_const = opset.constant(_np.float32(float(orig_h)))
    ones_const = opset.constant(_np.float32(1.0))
    ...
    unsq_axis = opset.constant(_np.array([2], dtype=_np.int64))
    ...
    zero_box = opset.constant(
        _np.array([0.0, 0.0, 0.0, 0.0], dtype=_np.float32).reshape(1, 1, 4))
    ...
    print(f'  stability_score + mask_to_box computed on GPU (standard OV ops)')
    print(f'  logits NOT in output (saves ~50% GPU→CPU transfer)')
    ...
    if verify:
        _verify_decoder_v2(xml_path, nn_params, im_h, im_w)

    return xml_path


def _verify_decoder_v2(xml_path, nn_params, im_h, im_w):
    """Verify decoder v2 outputs against CPU reference."""

    from segment_anything.utils.amg import calculate_stability_score, batched_mask_to_box
...
def export_dinov2_ext(dinov2_model, output_dir, compress_fp16=False,
    ...
    shape_256 = opset.constant(_np.array([0, num_patches], dtype=_np.int64))
    pooled_flat = opset.reshape(pooled, shape_256, special_zero=True)  # [N, 256]
    thresh = opset.constant(_np.float32(validpatch_thresh))
    ...
    valid_expanded = opset.unsqueeze(
        valid_f32, opset.constant(_np.array([2], dtype=_np.int64)))

    # Step C: L2 normalize patch_tokens along last dim
    eps = opset.constant(_np.float32(1e-6))
    reduce_axes = opset.constant(_np.array([2], dtype=_np.int64))
...
def _verify_model(pt_model, ov_model, example_input, label):
    ...
    for i, pt_tensor in enumerate(pt_out):
        ...
        ov_np = ov_result[i]
        max_err = np.max(np.abs(pt_np - ov_np))
        mean_err = np.mean(np.abs(pt_np - ov_np))
        cos_sim = (np.sum(pt_np * ov_np) /
                   (np.linalg.norm(pt_np) * np.linalg.norm(ov_np) + 1e-12))
...
def export_fastsam(checkpoint_path, output_dir, compress_fp16=False,
    ...
    """Export FastSAM (YOLOv8x-seg) to OpenVINO IR via dynamic-shape ONNX.
    ...
    Dynamic shapes allow auto letterbox (no square padding), matching
    the CUDA SegmentationPredictor behaviour for better accuracy.

    The exported model does NOT include NMS — that remains in Python.
    ...
    if verify:
        _verify_fastsam(xml_path, imgsz)

    return xml_path


def _verify_fastsam(xml_path, imgsz):
    """Quick sanity check: compile and run with random input."""
...
def main():
    ...
    print(f"{'='*60}")
    print(f"  All models use standard OV ops only.")
    print(f"  No custom C++ extension or GPU kernels needed.")
    print(f"{'='*60}")
