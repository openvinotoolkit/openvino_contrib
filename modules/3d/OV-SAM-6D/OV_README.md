...
This directory contains **only** the files added for the OpenVINO port plus
patches for the original files that were modified. To obtain a complete,
runnable tree, run the setup script once:

```bash
cd openvino_contrib/modules/3d/OV-SAM-6D
./setup_from_original.sh
```

The script clones the original SAM-6D (commit `1c2543b`), overlays the
OpenVINO port files (without overwriting them), and applies the ISM and PEM
patches from `SAM-6D/patches/`. Afterwards, follow the rest of this guide for
