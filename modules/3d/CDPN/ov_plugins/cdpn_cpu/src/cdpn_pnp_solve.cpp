...
static void symmetric_3x3_eigenvalues(const double A[3][3],
  ...
  int idx[3] = {0, 1, 2};
  if (M[idx[0]][idx[0]] < M[idx[1]][idx[1]])
    std::swap(idx[0], idx[1]);
  if (M[idx[0]][idx[0]] < M[idx[2]][idx[2]])
    std::swap(idx[0], idx[2]);
  if (M[idx[1]][idx[1]] < M[idx[2]][idx[2]])
...
static void svd_3x3(const double A[3][3], double U[3][3], double S[3],
                    double Vt[3][3]) {
  // Compute AtA
  double AtA[3][3] = {};
  for (int i = 0; i < 3; ++i)
...
static void symmetric_NxN_eigen(const double *A, int N, double *eigenvalues,
                                double *eigenvectors) {
...
struct EPnPSolver {
  ...
  EPnPSolver(const double *pts3d, const double *pts2d, int npts, double fx,
             double fy, double cx, double cy)
      : n_pts(npts), fu(fx), fv(fy), uc(cx), vc(cy), pws(3 * npts),
        us(2 * npts), alphas(4 * npts), pcs(3 * npts) {
  ...
  void compute_barycentric_coordinates() {
    ...
    double det = cc[0][0] * (cc[1][1] * cc[2][2] - cc[1][2] * cc[2][1]) -
    ...
    for (int i = 0; i < n_pts; ++i) {
      ...
      double dz = pws[3 * i + 2] - cws[0][2];
      alphas[4 * i + 1] =
          cc_inv[0][0] * dx + cc_inv[0][1] * dy + cc_inv[0][2] * dz;
      alphas[4 * i + 2] =
          cc_inv[1][0] * dx + cc_inv[1][1] * dy + cc_inv[1][2] * dz;
      alphas[4 * i + 3] =
          cc_inv[2][0] * dx + cc_inv[2][1] * dy + cc_inv[2][2] * dz;
      alphas[4 * i] =
          1.0 - alphas[4 * i + 1] - alphas[4 * i + 2] - alphas[4 * i + 3];
    }
  }
  ...
  void compute_pcs() {
    for (int i = 0; i < n_pts; ++i) {
      for (int j = 0; j < 3; ++j)
        pcs[3 * i + j] =
            alphas[4 * i] * ccs[0][j] + alphas[4 * i + 1] * ccs[1][j] +
  ...
  double reprojection_error(const double R[3][3], const double t[3]) {
    double sum = 0;
    for (int i = 0; i < n_pts; ++i) {
      double Xc = R[0][0] * pws[3 * i] + R[0][1] * pws[3 * i + 1] +
                  R[0][2] * pws[3 * i + 2] + t[0];
      double Yc = R[1][0] * pws[3 * i] + R[1][1] * pws[3 * i + 1] +
                  R[1][2] * pws[3 * i + 2] + t[1];
      double Zc = R[2][0] * pws[3 * i] + R[2][1] * pws[3 * i + 1] +
                  R[2][2] * pws[3 * i + 2] + t[2];
      if (std::fabs(Zc) < 1e-10) {
        sum += 1e6;
        continue;
      }
  ...
  void gauss_newton(const double l_6x10[6][10], const double rho[6],
    ...
    for (int iter = 0; iter < 5; ++iter) {
      ...
      for (int i = 0; i < 6; ++i) {
        ...
        A[i][0] = 2 * L[0] * betas[0] + L[1] * betas[1] + L[3] * betas[2] +
                  L[6] * betas[3];
        A[i][1] = L[1] * betas[0] + 2 * L[2] * betas[1] + L[4] * betas[2] +
                  L[7] * betas[3];
        A[i][2] = L[3] * betas[0] + L[4] * betas[1] + 2 * L[5] * betas[2] +
                  L[8] * betas[3];
        A[i][3] = L[6] * betas[0] + L[7] * betas[1] + L[8] * betas[2] +
                  2 * L[9] * betas[3];

        b[i] =
            rho[i] - (L[0] * betas[0] * betas[0] + L[1] * betas[0] * betas[1] +
                      L[2] * betas[1] * betas[1] + L[3] * betas[0] * betas[2] +
                      L[4] * betas[1] * betas[2] + L[5] * betas[2] * betas[2] +
                      L[6] * betas[0] * betas[3] + L[7] * betas[1] * betas[3] +
  ...
  bool compute_pose(double R_out[3][3], double t_out[3]) {
    ...
    double eigenvectors[144]; // column major: col[i] = eigenvectors[i*12 ..
                              // i*12+11]
    ...
    int best = 1;
    if (rep_errors[2] < rep_errors[best])
      best = 2;
    if (rep_errors[3] < rep_errors[best])
      best = 3;
...
CdpnPnpSolve::CdpnPnpSolve(const ov::Output<ov::Node> &denorm_coords,
                           ...
                           float mask_threshold, int out_res,
                           int max_iterations, float reproj_threshold)
    : Op({denorm_coords, confidence, obj_extents, crop_meta, cam_K}),
      m_mask_threshold(mask_threshold), m_out_res(out_res),
      m_max_iterations(max_iterations), m_reproj_threshold(reproj_threshold) {
  constructor_validate_and_infer_types();
}

void CdpnPnpSolve::validate_and_infer_types() {
  auto batch = get_input_partial_shape(0)[0];
  set_output_type(0, ov::element::f32, ov::PartialShape({batch, 1, 3, 3})); // R
  set_output_type(1, ov::element::f32, ov::PartialShape({batch, 1, 1, 3})); // T
  set_output_type(2, ov::element::f32,
                  ov::PartialShape({batch, 1, 1, 1})); // num_corres
  set_output_type(3, ov::element::f32,
                  ov::PartialShape({batch, 1, 1, 1})); // pnp_success
}

std::shared_ptr<ov::Node>
CdpnPnpSolve::clone_with_new_inputs(const ov::OutputVector &new_args) const {
...
bool CdpnPnpSolve::evaluate(ov::TensorVector &outputs,
  ...
  // --- Read batch size from denorm_coords shape ---
  const auto shape0 = inputs[0].get_shape();
  const int N = static_cast<int>(shape0[0]);
  const int RES = m_out_res;
  const int spatial = RES * RES;
  const int denorm_stride = 3 * spatial;
  const int conf_stride = spatial; // [N, 1, 64, 64]

  // --- Set output shapes ---
  outputs[0].set_shape({static_cast<size_t>(N), 1, 3, 3});
  outputs[1].set_shape({static_cast<size_t>(N), 1, 1, 3});
  outputs[2].set_shape({static_cast<size_t>(N), 1, 1, 1});
  outputs[3].set_shape({static_cast<size_t>(N), 1, 1, 1});

  const float *denorm_base = inputs[0].data<float>(); // [N, 3, 64, 64]
  const float *conf_base = inputs[1].data<float>();   // [N, 1, 64, 64]
  const float *ext_base = inputs[2].data<float>();    // [N, 1, 1, 3]
  const float *meta_base = inputs[3].data<float>();   // [N, 1, 1, 5]
  const float *camK_base = inputs[4].data<float>();   // [N, 1, 1, 4]

  float *R_out_base = outputs[0].data<float>();
  float *T_out_base = outputs[1].data<float>();
  float *nc_out_base = outputs[2].data<float>();
  float *ok_out_base = outputs[3].data<float>();

  for (int batch = 0; batch < N; ++batch) {
    const float *denorm = denorm_base + batch * denorm_stride;
    const float *conf = conf_base + batch * conf_stride;
    const float *ext = ext_base + batch * 3;
    const float *meta = meta_base + batch * 5;
    const float *camK = camK_base + batch * 4;

    float *R_out = R_out_base + batch * 9;
    float *T_out = T_out_base + batch * 3;
    float *nc_out = nc_out_base + batch;
    float *ok_out = ok_out_base + batch;

    // Default output: identity R, zero T, failure
    for (int i = 0; i < 9; ++i)
      R_out[i] = (i % 4 == 0) ? 1.0f : 0.0f;
    ...
    // denorm layout: [1, 3, 64, 64] -> channel c at offset c*spatial + row*RES
    // + col
    ...
    for (int iter = 0; iter < niters; ++iter) {
      ...
      for (int i = 0; i < MODEL_POINTS; ++i) {
        bool unique;
        do {
          sample[i] = static_cast<int>(rng() % static_cast<uint32_t>(n_pts));
          unique = true;
          for (int j = 0; j < i; ++j)
      ...
      if (!solver.compute_pose(cand_R, cand_t))
        continue;
      ...
      for (int i = 0; i < n_pts; ++i) {
        double Xc = cand_R[0][0] * pts3d[3 * i] +
                    cand_R[0][1] * pts3d[3 * i + 1] +
                    cand_R[0][2] * pts3d[3 * i + 2] + cand_t[0];
        double Yc = cand_R[1][0] * pts3d[3 * i] +
                    cand_R[1][1] * pts3d[3 * i + 1] +
                    cand_R[1][2] * pts3d[3 * i + 2] + cand_t[1];
        double Zc = cand_R[2][0] * pts3d[3 * i] +
                    cand_R[2][1] * pts3d[3 * i + 1] +
      ...
      if (good_count > std::max(max_good_count, MODEL_POINTS - 1)) {
        ...
        niters = ransac_update_num_iters(CONFIDENCE, outlier_ratio,
                                         MODEL_POINTS, niters);
      }
    }
    ...
    if (n_inliers >= 5) {
      EPnPSolver refit(inlier_3d.data(), inlier_2d.data(), n_inliers, fx, fy,
                       cx, cy);
...
bool CdpnPnpSolve::has_evaluate() const { return true; }
