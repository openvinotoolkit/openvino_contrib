...
function parseReturn(parameters: string[], body: string[]): Returns | undefined {
  ...
  if (returnType == null || (returnType.type && isIterator(returnType.type))) {
...
function parseYields(parameters: string[], body: string[]): Yields | undefined {
  const returnType = parseReturnFromDefinition(parameters);
...
function parseFromBody(body: string[], pattern: RegExp): Returns | Yields | undefined {
  for (const line of body) {
    const match = line.match(pattern);
