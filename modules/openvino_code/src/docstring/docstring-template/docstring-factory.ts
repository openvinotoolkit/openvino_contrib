...
import { DocstringParts } from "./docstring-parts";
...
export class DocstringFactory {
  ...
  public generateDocstring(docstringParts: DocstringParts, indentation = ""): string {
    ...
    let docstring: string = render(this.template, (templateData as unknown) as Context);
  ...
  private addSnippetPlaceholders(snippetString: string): string {
    ...
    snippetString = snippetString.replace(/@@@/g, () => (++placeholderNumber).toString());
