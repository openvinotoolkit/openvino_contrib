...
template<> Converter::Conversion::Ptr Converter::Convert(const opset::ReverseSequence& node) {
    ...
    switch (node.get_input_element_type(0)) {
        ...
        case ngraph::element::Type_t::f16 :
            if (node.get_input_element_type(1) == ngraph::element::i32) {
                return make(ngraph::runtime::reference::reverse_sequence<ngraph::float16, std::int32_t>);
            }
            return make(ngraph::runtime::reference::reverse_sequence<ngraph::float16, std::int64_t>);
