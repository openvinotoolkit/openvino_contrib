...
ArmPlugin::pass::ConvertMaxPoolV8::ConvertMaxPoolV8() {
    auto m = std::make_shared<ngraph::pattern::Matcher>(
            ngraph::pattern::wrap_type<opset::ArmMaxPoolV8>({ngraph::pattern::any_input(ngraph::pattern::has_static_shape())},
                                                                 ngraph::pattern::has_static_shape()), "ConvertMaxPoolV8");

    register_matcher(m, [&](ngraph::pattern::Matcher& m) {
        auto maxpool = std::dynamic_pointer_cast<opset::ArmMaxPoolV8>(m.get_match_root());
        if (!maxpool) {
            return false;
        }
        ...
        auto new_maxpool = std::make_shared<opset::ArmMaxPoolV8>(maxpool->input_value(0),
                                                                     maxpool->get_strides(),
