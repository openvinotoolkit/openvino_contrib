...
- `tests/unit/gfx_stage_policy_test.cpp` also locks the accepted `f32` image policy: default Metal Conv/GroupConv/Pool placement stays on MSL, while the diagnostic-only `GFX_DIAGNOSTIC_F32_MPS_IMAGE` path can select MPSImage through the same stage policy
...
- `tests/unit/gfx_parallelism_test.cpp` now also covers Broadcom V3D-specific matmul and convolution tuning behavior, including huge-spatial pointwise/light Conv2D selection of the hardware-cap 256-thread tile
...
- `ov_gfx_compare_runner` is useful for numeric diffs and per-op narrowing when a failure is hard to isolate from the full suite; it also supports `--per-op-all`, `--reference-device`, `--reference-plugin`, `--per-op-input-mode`, `--per-op-generated-inputs`, `--per-op-recursive-limit`, `--per-op-recursive-trace`, and `--gfx-only`
- Full-graph `--per-op-all` scans are thresholded gates: a clean scan ends with `PER_OP_MATCH max_abs=... max_rel=...`, while drift emits `PER_OP_FIRST_MISMATCH` plus `PER_OP_MISMATCH` and exits non-zero. `bench/gfx_eval.py` preserves this as `mode=per_op_mismatch` on host, Android, and SSH transports.
