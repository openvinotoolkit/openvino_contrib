...
Dynamic Tile and runtime-indexed Slice/StridedSlice paths should use shared
runtime-value planners and manifest scalar ABI, not op-specific OpenCL branches.
Static Concat/Split/VariadicSplit artifacts that exceed four direct inputs or
outputs should be chunked through `make_gfx_opencl_*_chunk_source_artifact()`
helpers so the generated kernels still inherit the published artifact contract.
