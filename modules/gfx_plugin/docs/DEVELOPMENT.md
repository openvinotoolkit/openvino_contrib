...
- Broadcom V3D pointwise/light-reduction Conv2D tuning belongs in `gfx_parallelism.*` and must flow through the same MLIR dispatch attrs as other convolution plans. For huge-spatial 1x1-style workloads, the V3D policy can choose the full hardware-cap `16x16`/256-thread tile to reduce workgroup count; do not implement this as a Vulkan executor shortcut or a separate Conv2D route.
...
- `GFX_DIAGNOSTIC_F32_MPS_IMAGE` is diagnostic-only Metal placement plumbing for `f32` MPSImage experiments and must not become default production routing without the documented full-model quality gate
