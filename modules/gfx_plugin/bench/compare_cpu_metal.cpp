...
//
// Small utility to compare CPU vs GFX inference outputs for a given model.
//
// Build (from build directory):
//   c++ -std=c++17 -I${OPENVINO_SRC}/src/core/include -I${OPENVINO_SRC}/src/inference/include \\
//       modules/gfx_plugin/bench/compare_cpu_metal.cpp \\
...
int main(int argc, char** argv) {
    ...
    auto compiled_metal = compile("GFX");
    ...
    for (const auto& node : ordered_ops) {
        ...
        try {
            ...
            sub_metal = core.compile_model(sub, "GFX", {{ov::hint::inference_precision.name(), ov::element::f32}});
