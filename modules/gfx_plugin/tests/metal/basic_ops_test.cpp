...
#include "../src/backends/metal/runtime/memory.hpp"
...
TEST(GfxTransforms, ConvReluFusionMarksAttribute) {
    auto param = std::make_shared<ov::op::v0::Parameter>(ov::element::f32, ov::Shape{1, 3, 4, 4});
    ...
    auto it = rt.find(ov::gfx_plugin::transforms::kGfxFusePrevConvAttr);
    ASSERT_NE(it, rt.end());
...
TEST(GfxBasicOps, DevicePropertiesAndQuery) {
    ...
    EXPECT_EQ(std::find(devices.begin(), devices.end(), "METAL"), devices.end());

    auto full_name = core.get_property("GFX", ov::device::full_name);
    EXPECT_FALSE(full_name.empty());
    EXPECT_NE(full_name.find("GFX"), std::string::npos);

    auto dtype = core.get_property<ov::device::Type>("GFX", ov::device::type);
    EXPECT_EQ(dtype, ov::device::Type::INTEGRATED);

    // Capabilities are optional; if provided, they should not be empty.
    try {
        auto caps = core.get_property("GFX", ov::device::capabilities);
...
// Ensure graph with Relu + Sigmoid + Add matches TEMPLATE output when executed on GFX.
...
ov::Core core;
    ...
    auto sigmoid = std::make_shared<ov::op::v0::Sigmoid>(relu);  // supported by GFX
    auto c = std::make_shared<ov::op::v0::Constant>(ov::element::f32, ov::Shape{1, 4},
                                                   std::vector<float>{0.1f, -0.2f, 0.3f, -0.4f});
    auto add = std::make_shared<ov::op::v1::Add>(sigmoid, c);  // supported by GFX
    auto res = std::make_shared<ov::op::v0::Result>(add);
    auto model = std::make_shared<ov::Model>(ov::ResultVector{res}, ov::ParameterVector{param}, "auto_gfx_ref");
