...
// Try compile_model on GFX; if it throws ov::Exception, skip the test gracefully.
...
protected:
    ...
    std::string device_metal = ov::test::utils::DEVICE_GFX;
    ...
    void compare_metal_vs_template(const std::shared_ptr<ov::Model>& model,
        ...
        if (device_ref == ov::test::utils::DEVICE_CPU) {
            ...
            // and produce larger numerical deltas than GFX's FP32 path.
        ...
        if (ref_failed && ov::test::utils::gfx_tests_debug_enabled()) {
            std::cerr << "[GFX TEST] reference execution failed; comparing GFX vs GFX\n";
        }

        auto out_metal = run_on_device(model, device_metal, clone_inputs(inputs), metal_config);
        if (ref_failed) {
            out_ref = out_metal;  // fall back to GFX self-compare to avoid spurious failures
        }
