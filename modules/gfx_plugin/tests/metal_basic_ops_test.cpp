...
inline void metal_try_catch_skip(const std::function<void()>& fn) {
    try {
        ...
        if (msg.find("device-only") != std::string::npos ||
            ...
            GTEST_SKIP() << "GFX outputs are device-only; skipping CPU readback test";
        }
        throw;
    }
}
...
void register_gfx_plugin(ov::Core& core) {
    ...
    try {
        ...
        core.register_plugin(GFX_PLUGIN_PATH, "GFX");
#else
...
// Ensure graph with Relu + Sigmoid + Add matches CPU output when executed on GFX.
