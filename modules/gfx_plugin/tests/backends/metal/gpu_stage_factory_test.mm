...
TEST(GpuStageFactory, CreatesStubForRelu) {
  ...
  auto stage = GpuStageFactory::create(RuntimeStageMaterializationContext{
                                           descriptor},
...
TEST(GpuStageFactory,
     MetalFactoryUsesSharedViewOnlyStageForMetadataDescriptor) {
  ensure_metal_stage_factory_registered();
...
TEST(GpuStageFactory,
     MpsrtMslKernelLoaderBuildsStageFromKernelSourceDescriptor) {
  const auto &source = mpsrt_image_bridge_kernel_source(
      MpsrtImageBridgeKernelKind::BufferToImageF32);

  const auto stage = metal::mpsrt::MpsrtMslKernelLoader::make_stage(source, 64);
