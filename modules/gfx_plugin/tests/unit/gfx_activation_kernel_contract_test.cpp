...
public:
    ...
    void verify() const {
        ...
        EXPECT_EQ(artifact->scalar_args,
                  (std::vector<GfxOpenClSourceScalarArg>{
                      ...
                      GfxOpenClSourceScalarArg::StaticF32}));
        EXPECT_EQ(artifact->static_u32_scalars,
                  std::vector<uint32_t>{});
        EXPECT_EQ(artifact->static_f32_scalars,
                  m_case.expected_static_f32_scalars);
        EXPECT_EQ(artifact->op, m_case.expected_op);
        EXPECT_EQ(artifact->scalar_constant_f32, 0.0f);
        EXPECT_EQ(artifact->input_mode,
                  GfxOpenClBaselineInputMode::Direct);
        EXPECT_EQ(artifact->source,
                  opencl_generated_activation_kernel_source().source);
        EXPECT_NE(artifact->source.find("__kernel void " +
                                        m_case.expected_entry_point),
                  std::string::npos);
        EXPECT_NE(artifact->source.find("gfx_activation_f32"),
                  std::string::npos);
...
std::vector<ActivationOpenClArtifactCase> activation_opencl_artifact_cases() {
    return {
        ...
        {"F16Tanh",
         ...
         "opencl/generated/activation_f16",
         "gfx_opencl_generated_activation_f16",
...
INSTANTIATE_TEST_SUITE_P(Activation,
                         ActivationOpenClArtifactContractTest,
                         ::testing::ValuesIn(
                             activation_opencl_artifact_cases()),
...
public:
    ...
    ActivationCompiledContract compile(
...
private:
    const compiler::StageRecord& find_activation_stage(
        const ManifestBundle& manifest) const {
        const auto it = std::find_if(
            manifest.stages.begin(),
            manifest.stages.end(),
            [this](const compiler::StageRecord& stage) {
                return stage.kernel_unit_id == m_route.expected_kernel_id;
            });
    ...
    const KernelArtifactDescriptor& find_activation_artifact(
    ...
    void verify_payload(const compiler::KernelArtifactPayload* payload) const {
        ...
        EXPECT_NE(source.find("kernel void activation_kernel"),
                  std::string::npos);
...
TEST_P(ActivationRouteContractTest, CompilesThroughExpectedKernelUnit) {
    ...
    const auto model = GetParam().expected_payload ==
                               KernelArtifactPayloadKind::MslSource
                           ? models.f32_elu()
                           : models.f32_relu();
...
INSTANTIATE_TEST_SUITE_P(ActivationBackends,
                         ActivationRouteContractTest,
