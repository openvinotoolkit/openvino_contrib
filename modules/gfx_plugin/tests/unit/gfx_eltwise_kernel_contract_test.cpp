...
std::shared_ptr<ov::op::v0::Constant> f16_const(ov::Shape shape,
                                                std::vector<float> values) {
    return ov::op::v0::Constant::create(ov::element::f16,
                                        std::move(shape),
                                        std::move(values));
}
...
public:
    ...
    void verify() const {
        ...
        EXPECT_EQ(artifact->direct_input_count,
                  m_case.expected_direct_input_count);
        EXPECT_EQ(artifact->direct_input_indices,
                  m_case.expected_direct_inputs);
...
INSTANTIATE_TEST_SUITE_P(Eltwise,
                         EltwiseOpenClArtifactContractTest,
...
public:
    ...
    EltwiseCompiledContract compile(
...
private:
    const compiler::StageRecord& find_eltwise_stage(
    ...
    const KernelArtifactDescriptor& find_eltwise_artifact(
...
class EltwiseRouteContractTest : public ::testing::TestWithParam<EltwiseRouteCase> {
protected:
    EltwiseModelFactory models;
};

TEST_P(EltwiseRouteContractTest, CompilesThroughExpectedKernelUnit) {
    EltwiseRouteContract(GetParam()).verify(
        EltwiseRouteContract(GetParam()).compile(models.f32_same_shape_add()));
}

INSTANTIATE_TEST_SUITE_P(EltwiseBackends,
                         EltwiseRouteContractTest,
