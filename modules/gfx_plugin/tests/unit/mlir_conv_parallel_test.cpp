...
TEST(GfxMlirTransforms, Conv2DParallelLowering) {
  ...
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{8, 3, 3, 3}, weights_data);
  ...
  ASSERT_NO_THROW(
      ov::gfx_plugin::run_mlir_pipeline(module,
                                        /*use_alloca=*/false,
...
TEST(GfxMlirTransforms,
     Conv2DParallelLoweringKeepsChannelBlockDispatchMetadata) {
  auto input = std::make_shared<ov::op::v0::Parameter>(ov::element::f32,
                                                       ov::Shape{1, 32, 8, 8});
  std::vector<float> weights_data(16 * 32 * 3 * 3, 0.1f);
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{16, 32, 3, 3}, weights_data);
  auto conv = std::make_shared<ov::op::v1::Convolution>(
      input, weights, ov::Strides{1, 1}, ov::CoordinateDiff{1, 1},
      ov::CoordinateDiff{1, 1}, ov::Strides{1, 1});
  auto res = std::make_shared<ov::op::v0::Result>(conv);
  ...
  auto accumulation = module->getAttrOfType<mlir::StringAttr>(
      "gfx.dispatch_channel_block_accumulation");
  ASSERT_TRUE(accumulation);
  EXPECT_EQ(accumulation.getValue(), "serial");
}

TEST(GfxMlirTransforms, Conv2DParallelLoweringMarksWidthInputReuseForSpatialTile) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 32, 16, 16});
  std::vector<float> weights_data(16 * 32 * 3 * 3, 0.1f);
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{16, 32, 3, 3}, weights_data);
  auto conv = std::make_shared<ov::op::v1::Convolution>(
  ...
  auto reuse =
      module->getAttrOfType<mlir::StringAttr>("gfx.conv_spatial_input_reuse");
  ASSERT_TRUE(reuse);
  EXPECT_EQ(reuse.getValue(), "width");
  auto unique_loads = module->getAttrOfType<mlir::IntegerAttr>(
      "gfx.conv_spatial_input_reuse_unique_width_loads");
  ASSERT_TRUE(unique_loads);
  EXPECT_LT(unique_loads.getInt(), 2 * 3);
}

TEST(GfxMlirTransforms, Conv2DParallelLoweringAcceptsPackedOc4Weights) {
  auto input = std::make_shared<ov::op::v0::Parameter>(ov::element::f32,
                                                       ov::Shape{1, 32, 8, 8});
  std::vector<float> weights_data(16 * 32 * 3 * 3, 0.1f);
  auto weights = ov::op::v0::Constant::create(
  ...
  auto model = std::make_shared<ov::Model>(ov::ResultVector{res},
                                           ov::ParameterVector{input},
                                           "conv2d_packed_oc4_test");

  mlir::MLIRContext ctx;
  auto module = ov::gfx_plugin::build_mlir_conv2d_from_model(model, ctx);
  ASSERT_TRUE(module) << "Failed to build MLIR module for Conv2D";
  module->setAttr("gfx.dispatch_channel_block",
                  mlir::IntegerAttr::get(mlir::IndexType::get(&ctx), 4));
  ...
  module.walk([&](mlir::Operation *op) {
    ...
    has_parallel = has_parallel || llvm::isa<mlir::scf::ParallelOp>(op);
  });
  EXPECT_FALSE(has_conv);
  EXPECT_TRUE(has_parallel);
  auto packed =
      module->getAttrOfType<mlir::BoolAttr>("gfx.conv2d_weights_packed_oc4");
  ASSERT_TRUE(packed);
  EXPECT_TRUE(packed.getValue());
}

TEST(GfxMlirTransforms,
     Conv2DInteriorTileInputWindowCheckIsStrictlyBoundsSafe) {
...
TEST(GfxMlirTransforms, Conv2DBuilderUsesCanonicalConvOp) {
  ...
  EXPECT_TRUE(has_conv)
      << "Conv2D builder should emit canonical linalg Conv op";
  EXPECT_TRUE(has_pad)
      << "Conv2D builder should materialize explicit padding in MLIR";
}
...
TEST(GfxMlirTransforms, GroupConv2DBuilderSetsPadAttrsForGroupsOne) {
  ...
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{1, 8, 3, 3, 3}, weights_data);
  ...
  EXPECT_TRUE(found_conv)
      << "Expected canonical linalg conv op for groups==1 GroupConv2D";
}
...
TEST(GfxMlirTransforms, AddBuilderAbsorbsInputTransposeIntoAffineMap) {
  ...
  ASSERT_TRUE(func);
  auto arg0_ty =
      llvm::dyn_cast<mlir::RankedTensorType>(func.getArgument(0).getType());
  auto arg1_ty =
      llvm::dyn_cast<mlir::RankedTensorType>(func.getArgument(1).getType());
  ASSERT_TRUE(arg0_ty);
  ASSERT_TRUE(arg1_ty);
  ...
  EXPECT_TRUE(found_generic)
      << "Expected linalg.generic for absorbed transpose Add";
  ASSERT_TRUE(module->hasAttr("gfx.absorbed_input0_perm"));
}

TEST(GfxMlirTransforms, SplitBuilderAbsorbsInputTransposeIntoGeneric) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{4, 2, 96, 400});
  auto axis = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{}, {2});
  auto lengths = ov::op::v0::Constant::create(ov::element::i64, ov::Shape{3},
                                              {32, 32, 32});
  auto split =
      std::make_shared<ov::op::v1::VariadicSplit>(input, axis, lengths);
  ...
  auto module = ov::gfx_plugin::build_mlir_split_from_node(
      split, ctx, transform.source_shape, &transform);
  ASSERT_TRUE(module);
  ...
  bool found_generic = false;
  module.walk([&](mlir::linalg::GenericOp generic) { found_generic = true; });
  EXPECT_TRUE(found_generic)
      << "Expected linalg.generic for absorbed transpose Split";
  ASSERT_TRUE(module->hasAttr("gfx.absorbed_input0_perm"));
  ASSERT_NO_THROW(
      ov::gfx_plugin::run_mlir_pipeline(module,
                                        /*use_alloca=*/false,
...
TEST(GfxMlirTransforms,
     GroupConvBuilderAbsorbsInputTransposeIntoDepthwiseGeneric) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 64, 40, 40});
  std::vector<float> weights_data(64 * 1 * 1 * 7 * 7, 0.1f);
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{64, 1, 1, 7, 7}, weights_data);
  auto gconv = std::make_shared<ov::op::v1::GroupConvolution>(
      input, weights, ov::Strides{1, 1}, ov::CoordinateDiff{3, 3},
      ov::CoordinateDiff{3, 3}, ov::Strides{1, 1});
  ...
  auto module =
      ov::gfx_plugin::build_mlir_group_conv2d_from_node(gconv, ctx, &transform);
  ASSERT_TRUE(module);
  ...
  module.walk([&](mlir::linalg::GenericOp generic) { found_generic = true; });
  EXPECT_TRUE(found_generic)
      << "Expected depthwise linalg.generic for absorbed transpose GroupConv";
  ASSERT_NO_THROW(
      ov::gfx_plugin::run_mlir_pipeline(module,
                                        /*use_alloca=*/false,
                                        /*use_parallel_loops=*/true));
...
TEST(GfxMlirTransforms, DepthwiseGroupConvUsesParallelLoweringForDirectNchw) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f16, ov::Shape{1, 32, 20, 20});
  std::vector<ov::float16> weights_data(32 * 1 * 1 * 3 * 3, ov::float16(0.1f));
  auto weights = ov::op::v0::Constant::create(
      ov::element::f16, ov::Shape{32, 1, 1, 3, 3}, weights_data);
  auto gconv = std::make_shared<ov::op::v1::GroupConvolution>(
      input, weights, ov::Strides{1, 1}, ov::CoordinateDiff{1, 1},
      ov::CoordinateDiff{1, 1}, ov::Strides{1, 1});
  auto res = std::make_shared<ov::op::v0::Result>(gconv);
  ...
  EXPECT_FALSE(has_depthwise_generic) << "Depthwise GroupConvolution should "
                                         "leave the serial linalg.generic path";
  EXPECT_EQ(parallel_count, 1u)
      << "Depthwise GroupConvolution should lower to one compute scf.parallel";
  EXPECT_FALSE(has_padded_alloc) << "Depthwise GroupConvolution should absorb "
                                    "padding into the compute loop";
  auto loop_dims =
...
TEST(GfxMlirTransforms, DepthwiseGroupConvUsesParallelLoweringForAppleMslFp32) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 32, 20, 20});
  std::vector<float> weights_data(32 * 1 * 1 * 3 * 3, 0.1f);
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{32, 1, 1, 3, 3}, weights_data);
  auto gconv = std::make_shared<ov::op::v1::GroupConvolution>(
      input, weights, ov::Strides{1, 1}, ov::CoordinateDiff{1, 1},
      ov::CoordinateDiff{1, 1}, ov::Strides{1, 1});
  auto res = std::make_shared<ov::op::v0::Result>(gconv);
  auto model = std::make_shared<ov::Model>(
      ov::ResultVector{res}, ov::ParameterVector{input},
...
TEST(GfxMlirTransforms, MaxPool2DBuilderUsesParallelGpuDispatch) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 384, 20, 20});
  auto pool = std::make_shared<ov::op::v1::MaxPool>(
      input, ov::Strides{1, 1}, ov::Shape{2, 2}, ov::Shape{2, 2},
      ov::Shape{5, 5}, ov::op::RoundingType::FLOOR);
  ...
  bool has_parallel = false;
  module.walk([&](mlir::scf::ParallelOp) { has_parallel = true; });

  EXPECT_TRUE(has_parallel)
      << "MaxPool2D must not use the legacy serial single-dispatch loop";
  auto loop_dims =
      module->getAttrOfType<mlir::IntegerAttr>("gfx.parallel_loop_dims");
  ASSERT_TRUE(loop_dims);
  EXPECT_EQ(loop_dims.getInt(), 5);
...
TEST(GfxMlirTransforms, AvgPool2DBuilderUsesParallelGpuDispatch) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 64, 16, 16});
  auto pool = std::make_shared<ov::op::v1::AvgPool>(
      input, ov::Strides{1, 1}, ov::Shape{1, 1}, ov::Shape{1, 1},
      ov::Shape{3, 3}, true, ov::op::RoundingType::FLOOR);
  auto res = std::make_shared<ov::op::v0::Result>(pool);
  auto model = std::make_shared<ov::Model>(ov::ResultVector{res},
                                           ov::ParameterVector{input},
  ...
  EXPECT_TRUE(has_parallel)
      << "AvgPool2D must not use the legacy serial single-dispatch loop";
  auto loop_dims =
      module->getAttrOfType<mlir::IntegerAttr>("gfx.parallel_loop_dims");
  ASSERT_TRUE(loop_dims);
  EXPECT_EQ(loop_dims.getInt(), 5);
...
TEST(GfxMlirTransforms, ConvBuilderAbsorbsInputTransposeIntoPointwiseGeneric) {
  auto input = std::make_shared<ov::op::v0::Parameter>(
      ov::element::f32, ov::Shape{1, 16, 4, 8400});
  std::vector<float> weights_data(1 * 16 * 1 * 1, 0.1f);
  auto weights = ov::op::v0::Constant::create(
      ov::element::f32, ov::Shape{1, 16, 1, 1}, weights_data);
  auto conv = std::make_shared<ov::op::v1::Convolution>(
  ...
  auto module =
      ov::gfx_plugin::build_mlir_conv2d_from_node(conv, ctx, &transform);
  ASSERT_TRUE(module);
  ...
  module.walk([&](mlir::linalg::GenericOp generic) { found_generic = true; });
  EXPECT_TRUE(found_generic)
      << "Expected linalg.generic for absorbed transpose Conv";
  ASSERT_NO_THROW(
      ov::gfx_plugin::run_mlir_pipeline(module,
                                        /*use_alloca=*/false,
                                        /*use_parallel_loops=*/true));
