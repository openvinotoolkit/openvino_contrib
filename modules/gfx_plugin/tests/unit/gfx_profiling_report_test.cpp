...
TEST(GfxProfilingReportTest, TraceAggregatesCountersSegmentsAndTransfers) {
    ...
    profile.min_storage_buffer_offset_alignment = 16;
    profile.non_coherent_atom_size = 64;
    profile.supports_storage_buffer_16bit = true;
    trace.set_target_profile(profile);
    ...
    EXPECT_NE(json.find("\"conv_output_channel_blocking\":false"), std::string::npos);
