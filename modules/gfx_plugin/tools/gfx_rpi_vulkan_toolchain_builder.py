...
def create_python_wrappers(output_dir: Path, profile: dict[str, object], *, dry_run: bool) -> None:
    ...
    compile_flags = [str(flag) for flag in profile["compile_flags"]]
    compile_flags_literal = ", ".join(json.dumps(flag) for flag in compile_flags)
...
TRIPLE = "{triple}"
TARGET_FLAGS = [{compile_flags_literal}]
GCC_VER = "{gcc_version}"
...
def generate_toolchain_file(output_dir: Path, profile: dict[str, object], *, dry_run: bool) -> Path:
    ...
    triple = str(profile["triple"])
    compile_flags = " ".join(str(flag) for flag in profile["compile_flags"])
...
set(GFX_RPI_VULKAN_COMPILE_FLAGS "{compile_flags}" CACHE STRING
    "Portable AArch64 compile flags for Raspberry Pi Vulkan cross-builds")
set(CMAKE_C_FLAGS_INIT "${{GFX_RPI_VULKAN_COMPILE_FLAGS}}")
set(CMAKE_CXX_FLAGS_INIT "${{GFX_RPI_VULKAN_COMPILE_FLAGS}}")
