...
public:
  VulkanConstBufferReuseContext()
      : m_device_alloc(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT),
        m_staging_alloc(VK_BUFFER_USAGE_TRANSFER_SRC_BIT),
        m_const_cache(
            std::make_shared<ImmutableGpuBufferCache>([this](GpuBuffer &&buf) {
  ...
  void flush_upload_batch(GpuCommandBufferHandle command_buffer,
  ...
  GpuBuffer wrap_const(const std::string &key, const void *data, size_t bytes,
    ...
    return m_const_cache->get_or_create(key, data, bytes, type, [&]() {
      ...
      if (bytes <= kDirectMappedConstBytes) {
        // Small immutable metadata buffers are cheaper and more robust as
        // direct host-visible storage buffers than as one-off staging uploads.
      ...
      if (bytes && staging.valid() && device_buf.valid()) {
        ...
        if (state.depth > 0) {
          ...
          state.pending_uploads.push_back(
              PendingUpload{std::move(staging), device_buf, bytes});
  ...
  void release_temp(GpuBuffer &&buf) { m_device_alloc.release(std::move(buf)); }
...
private:
  ...
  struct BatchSubmitContext {
    ...
    void ensure() {
      auto &vk = VulkanContext::instance();
      if (pool != VK_NULL_HANDLE && command_buffer != VK_NULL_HANDLE &&
      ...
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkCreateCommandPool failed for const upload batch");
      }
      ...
      if (res != VK_SUCCESS) {
        OPENVINO_THROW("GFX Vulkan: vkAllocateCommandBuffers failed for const "
                       "upload batch");
      }
      ...
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkCreateFence failed for const upload batch");
      }
    }
  ...
  static std::unordered_map<const VulkanConstBufferReuseContext *,
                            UploadBatchState> &
  upload_batch_states() {
    thread_local std::unordered_map<const VulkanConstBufferReuseContext *,
                                    UploadBatchState>
        states;
    return states;
  }
  ...
  UploadBatchState &batch_state() { return upload_batch_states()[this]; }

  static void release_pending_uploads(std::vector<PendingUpload> &uploads,
                                      VulkanGpuAllocator &staging_alloc) {
  ...
  void flush_upload_batch_state(UploadBatchState &state,
    ...
    if (command_buffer) {
      for (const auto &upload : state.pending_uploads) {
        gpu_copy_buffer(command_buffer, upload.staging, upload.device,
                        upload.bytes);
      }
    } else {
      ...
      VkResult res = vkWaitForFences(vk.device(), 1, &submit_ctx.fence, VK_TRUE,
                                     UINT64_MAX);
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkWaitForFences failed for const upload batch");
      }
      res = vkResetFences(vk.device(), 1, &submit_ctx.fence);
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkResetFences failed for const upload batch");
      }
      ...
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkBeginCommandBuffer failed for const upload batch");
      }
      for (const auto &upload : state.pending_uploads) {
        gpu_copy_buffer(
            reinterpret_cast<GpuCommandBufferHandle>(submit_ctx.command_buffer),
            upload.staging, upload.device, upload.bytes);
      }
      res = vkEndCommandBuffer(submit_ctx.command_buffer);
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkEndCommandBuffer failed for const upload batch");
      }
      ...
      if (res != VK_SUCCESS) {
        OPENVINO_THROW(
            "GFX Vulkan: vkQueueSubmit failed for const upload batch");
      }
      res = vkWaitForFences(vk.device(), 1, &submit_ctx.fence, VK_TRUE,
                            UINT64_MAX);
      if (res != VK_SUCCESS) {
        OPENVINO_THROW("GFX Vulkan: vkWaitForFences failed for const upload "
                       "batch completion");
      }
    }
    ...
    if (profiler) {
      profiler->record_segment("upload", "flush_const_upload_batch", cpu_us, 0,
                               0, static_cast<uint64_t>(total_bytes), 0, 0, 0,
                               -1, 0,
                               reinterpret_cast<uint64_t>(command_buffer));
      profiler->record_transfer("const_h2d", static_cast<uint64_t>(total_bytes),
                                true, cpu_us);
      profiler->increment_counter("const_upload_batch_count");
      profiler->increment_counter(
          "const_upload_buffer_count",
...
private:
  std::mutex m_mutex;
  std::unordered_map<uintptr_t, std::weak_ptr<VulkanConstBufferReuseContext>>
      m_entries;
};
...
VulkanBufferManager::VulkanBufferManager()
    : m_reuse_context(VulkanConstBufferReuseRegistry::instance().acquire(
          VulkanContext::instance().device())) {}

VulkanBufferManager::~VulkanBufferManager() = default;

std::optional<GpuExecutionDeviceInfo>
VulkanBufferManager::query_execution_device_info() const {
...
GpuBuffer VulkanBufferManager::wrap_const(const std::string &key,
                                          ...
                                          ov::element::Type type) {
  auto context =
      std::static_pointer_cast<VulkanConstBufferReuseContext>(m_reuse_context);
...
void VulkanBufferManager::flush_const_upload_batch(
    GpuCommandBufferHandle command_buffer, GfxProfiler *profiler) {
  auto context =
      std::static_pointer_cast<VulkanConstBufferReuseContext>(m_reuse_context);
  OPENVINO_ASSERT(context, "GFX Vulkan: missing const buffer reuse context");
