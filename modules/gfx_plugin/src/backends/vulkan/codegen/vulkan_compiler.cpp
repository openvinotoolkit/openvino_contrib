...
std::shared_ptr<ICompiledKernel> VulkanCodegenBackend::compile(const KernelSource& source, std::string* log) {
    ...
    if (arg_count > 0) {
        cache_key.backend = GpuBackend::Vulkan;
        cache_key.device = reinterpret_cast<uintptr_t>(m_device);
        cache_key.code_hash =
            gfx_hash_bytes(spirv_binary.data(), spirv_binary.size() * sizeof(uint32_t));
        cache_key.entry_hash = gfx_hash_string(entry);
    ...
    auto kernel = std::make_shared<VulkanCompiledKernel>(std::move(spirv_binary),
                                                         std::move(entry),
                                                         arg_count);
    if (arg_count > 0) {
