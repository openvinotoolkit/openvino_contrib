...
#import "backends/metal/runtime/backend.hpp"

#include <vector>

#include "backends/metal/codegen/kernel_compiler.hpp"
...
std::shared_ptr<ICompiledKernel> MetalCodegenBackend::compile(const KernelSource& source,
    ...
    return std::make_shared<MetalCompiledKernel>(m_device, (void*)pipeline, source.signature.arg_count);
}
