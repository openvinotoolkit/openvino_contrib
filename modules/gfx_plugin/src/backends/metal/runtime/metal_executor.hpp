...
private:
    ...
    std::vector<KernelArg> materialize_source_args(const std::vector<GpuTensor*>& outputs);
    std::vector<KernelArg> materialize_role_ordered_source_args(
    ...
    KernelDispatch make_source_dispatch(const std::vector<GpuTensor*>& outputs) const;
