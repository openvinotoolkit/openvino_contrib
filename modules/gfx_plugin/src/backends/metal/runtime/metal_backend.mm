...
#import "backends/metal/runtime/metal_backend.hpp"
...
#include "runtime/gfx_kernel_cache.hpp"
...
std::shared_ptr<ICompiledKernel> MetalCodegenBackend::compile(const KernelSource& source,
    ...
    KernelCacheKey cache_key{};
    if (arg_count > 0) {
        cache_key.backend = GpuBackend::Metal;
        cache_key.device = reinterpret_cast<uintptr_t>(m_device);
        cache_key.code_hash = gfx_hash_bytes(msl.data(), msl.size());
        cache_key.entry_hash = gfx_hash_string(source.entry_point);
        cache_key.arg_count = arg_count;
        if (auto cached = GfxKernelCache::instance().lookup(cache_key)) {
            return cached;
        }
    }
    ...
    id<MTLComputePipelineState> pipeline =
        compiler.compile_msl_from_source(msl, source.entry_point.c_str(), compile_log);
    if (!pipeline) {
        return nullptr;
    }
    auto kernel = std::make_shared<MetalCompiledKernel>(m_device, (void*)pipeline, arg_count);
    if (arg_count > 0) {
        GfxKernelCache::instance().store(cache_key, kernel);
    }
    return kernel;
}
