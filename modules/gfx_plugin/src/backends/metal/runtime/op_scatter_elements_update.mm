...
void MetalScatterElementsUpdateOp::compile(MetalBufferManager* buffer_manager) {
    ...
    m_kernel_init = compile_msl_kernel(backend, spec, module, "scatter_elements_init", source, &log);
    OPENVINO_ASSERT(m_kernel_init, "MetalScatterElementsUpdateOp: init kernel compile failed: ", log);
    m_kernel_update = compile_msl_kernel(backend, spec, module, "scatter_elements_update", source, &log);
