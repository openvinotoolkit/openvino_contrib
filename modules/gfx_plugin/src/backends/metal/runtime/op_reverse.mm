...
void MetalReverseOp::compile(MetalBufferManager* buffer_manager) {
    ...
    auto spec = make_kernel_spec_from_custom_kernel_abi(m_node, "reverse_kernel");
    m_kernel = compile_msl_kernel(backend, spec, module, "reverse_kernel", msl_generator, &log);
...
void MetalReverseOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    args_builder.add_bytes(&m_desc, sizeof(m_desc));
