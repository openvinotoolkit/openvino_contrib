...
void MetalBroadcastOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    args_builder.add_bytes(&out_rank_u, sizeof(out_rank_u));
    args_builder.add_bytes(&in_rank_u, sizeof(in_rank_u));
    ...
    args_builder.add_bytes(m_axes.data(), m_axes.size() * sizeof(int));
