...
bool supports_static_split(const std::shared_ptr<const ov::Node>& node) {
    if (!node ||
        ...
        !all_outputs_static(node) ||
        !constant_input(node, 1)) {
        return false;
    }
...
bool supports_static_slice(const std::shared_ptr<const ov::Node>& node) {
    ...
    if (auto slice = ov::as_type_ptr<const ov::op::v8::Slice>(node)) {
        return constant_input(slice, 1) &&
               constant_input(slice, 2) &&
    ...
    return strided_slice &&
           ...
           std::all_of(strided_slice->get_ellipsis_mask().begin(),
                       ...
                       [](int64_t value) { return value == 0; }) &&
           constant_input(strided_slice, 1) &&
           constant_input(strided_slice, 2) &&
...
bool supports_mps_softmax_vendor(
    const std::shared_ptr<const ov::Node>& node) {
...
bool supports_mps_resize2d_vendor(
    const std::shared_ptr<const ov::Node>& node) {
...
OperationSupportResult query_metal_operation(const std::shared_ptr<const ov::Node>& node) {
    try {
        ...
        if (node &&
            ov::as_type_ptr<const ov::op::v4::Range>(node) &&
            node->get_output_partial_shape(0).is_static()) {
            return make_supported_operation("generated_msl_source",
                                            LoweringRouteKind::GeneratedKernel,
                                            0.55,
        ...
        if (node &&
            ov::as_type_ptr<const ov::op::v0::Tile>(node) &&
