...
MetalBuffer MetalAllocator::allocate(const BufferDesc& desc, bool persistent) {
    ...
    if (!persistent) {
        ...
        if (cached.valid()) {
            ...
            if (m_profile_allocations && m_profiler) {
                ...
                m_profiler->record_alloc(label, cached.size, true, std::chrono::microseconds{0});
            }
            return cached;
        }
    }
    ...
    if (m_profile_allocations && m_profiler) {
        ...
        m_profiler->record_alloc(label, buf.size, false, std::chrono::microseconds{0});
    }
    return buf;
}
...
MetalBuffer MetalAllocator::ensure_handle(BufferHandle& handle, const BufferDesc& desc, bool persistent) {
    ...
    if (handle.capacity >= target && handle.buf.valid() && storage_matches) {
        ...
        if (m_profile_allocations && m_profiler) {
            ...
            m_profiler->record_alloc(label, handle.buf.size, true, std::chrono::microseconds{0});
        }
        return handle.buf;
    }
    ...
    if (m_profile_allocations && m_profiler) {
        ...
        m_profiler->record_alloc(label, handle.buf.size, false, std::chrono::microseconds{0});
    }
    return handle.buf;
}
