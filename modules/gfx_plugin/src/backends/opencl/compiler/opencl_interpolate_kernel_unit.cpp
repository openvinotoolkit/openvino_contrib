...
bool is_opencl_interpolate_node(const std::shared_ptr<const ov::Node> &node) {
...
std::string support_reason_for_interpolate_unit(const KernelUnit &unit) {
  if (unit.kind() == KernelUnitKind::HandwrittenException) {
    return "registered_opencl_interpolate_handwritten_exception";
  }
  return "registered_opencl_interpolate_kernel_unit";
}

std::shared_ptr<const ::ov::gfx_plugin::KernelArtifactPayload>
materialize_interpolate_payload(const KernelArtifactDescriptor &descriptor,
...
KernelUnit resolve_opencl_interpolate_kernel_unit(
    const std::shared_ptr<const ov::Node> &node,
    const KernelRegistry &registry) {
  const auto artifact = make_opencl_interpolate_source_artifact(node);
...
query_opencl_interpolate_operation(const std::shared_ptr<const ov::Node> &node,
                                   const KernelRegistry &registry) {
  if (!is_opencl_interpolate_node(node)) {
    return make_unsupported_operation("not_opencl_interpolate");
  }
  const auto unit = resolve_opencl_interpolate_kernel_unit(node, registry);
  if (!unit.valid()) {
    return make_unsupported_operation("missing_opencl_interpolate_kernel_unit");
  }
  return make_supported_operation(support_reason_for_interpolate_unit(unit),
...
build_opencl_interpolate_kernel_artifact_payload(
  ...
  if (descriptor.kernel.backend_domain != "opencl" ||
      ...
      !op.source_node || !is_opencl_interpolate_node(op.source_node)) {
    return {};
  }

  auto artifact = make_opencl_interpolate_source_artifact(
      op.source_node, descriptor.kernel.kernel_id);
  if (!artifact || !artifact->valid) {
    return {};
  }
  return materialize_interpolate_payload(descriptor, std::move(*artifact));
}
