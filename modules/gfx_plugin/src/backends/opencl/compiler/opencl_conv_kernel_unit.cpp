...
bool is_opencl_conv2d_node(const std::shared_ptr<const ov::Node> &node) {
  return ov::as_type_ptr<const ov::op::v1::Convolution>(node) ||
         ov::as_type_ptr<const ov::op::v1::GroupConvolution>(node);
}

namespace {

std::string support_reason_for_conv2d_unit(const KernelUnit &unit) {
  if (unit.kind() == KernelUnitKind::HandwrittenException) {
    return "registered_opencl_convolution_handwritten_exception";
  }
  return "registered_opencl_convolution_kernel_unit";
}

std::string missing_conv2d_reason(const std::shared_ptr<const ov::Node> &node) {
  if (ov::as_type_ptr<const ov::op::v1::GroupConvolution>(node)) {
    return "missing_opencl_group_convolution_kernel_unit";
  }
  return "missing_opencl_convolution_kernel_unit";
}

std::shared_ptr<const ::ov::gfx_plugin::KernelArtifactPayload>
materialize_conv2d_payload(const KernelArtifactDescriptor &descriptor,
...
resolve_opencl_conv2d_kernel_unit(const std::shared_ptr<const ov::Node> &node,
  ...
  const auto artifact = make_opencl_conv2d_source_artifact(node);
...
query_opencl_conv2d_operation(const std::shared_ptr<const ov::Node> &node,
                              const KernelRegistry &registry) {
  if (!is_opencl_conv2d_node(node)) {
    return make_unsupported_operation("not_opencl_convolution");
  }
  const auto unit = resolve_opencl_conv2d_kernel_unit(node, registry);
  if (!unit.valid()) {
    return make_unsupported_operation(missing_conv2d_reason(node));
  }
  return make_supported_operation(support_reason_for_conv2d_unit(unit),
...
build_opencl_conv2d_kernel_artifact_payload(
  ...
  if (descriptor.kernel.backend_domain != "opencl" ||
      ...
      !op.source_node || !is_opencl_conv2d_node(op.source_node)) {
    return {};
  }

  auto artifact = make_opencl_conv2d_source_artifact(
      op.source_node, descriptor.kernel.kernel_id);
  if (!artifact || !artifact->valid) {
    return {};
  }
  return materialize_conv2d_payload(descriptor, std::move(*artifact));
}
