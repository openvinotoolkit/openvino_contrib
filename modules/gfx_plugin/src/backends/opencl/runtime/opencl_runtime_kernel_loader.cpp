...
void verify_opencl_source_descriptor(const RuntimeStageExecutableDescriptor& descriptor,
    ...
    OPENVINO_ASSERT(artifact.artifact_ref.kind == GfxKernelArtifactKind::OpenClSource,
...
OpenClRuntimeKernelLoader::OpenClRuntimeKernelLoader(std::shared_ptr<OpenClRuntimeContext> context)
    : m_context(std::move(context)) {
    OPENVINO_ASSERT(m_context, "GFX OpenCL: runtime kernel loader requires context");
}
...
std::unique_ptr<GpuStage> OpenClRuntimeKernelLoader::load_source_stage(
    ...
    return create_opencl_source_stage(node, m_context, descriptor, std::move(artifact));
}
