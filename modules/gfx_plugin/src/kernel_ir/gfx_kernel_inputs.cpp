...
KernelInputMapping build_kernel_inputs(const std::shared_ptr<const ov::Node>& node,
    ...
    if (mapping.func_inputs == 0) {
        mapping.func_inputs = node_inputs;
    }
    ...
    OPENVINO_ASSERT(mapping.func_inputs >= nonconst_count,
                    "GFX: MLIR expects fewer inputs than non-constant inputs for ",
                    stage_name);
    const size_t need_consts = mapping.func_inputs - nonconst_count;
    size_t const_added = 0;
    mapping.kernel_inputs.reserve(mapping.func_inputs);
    for (size_t i = 0; i < node_inputs; ++i) {
        auto src = node->get_input_node_shared_ptr(i);
    ...
    OPENVINO_ASSERT(mapping.kernel_inputs.size() == mapping.func_inputs,
