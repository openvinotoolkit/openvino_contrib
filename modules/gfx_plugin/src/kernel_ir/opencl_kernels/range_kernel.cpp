...
GfxKernelStageManifest make_opencl_range_manifest(
    std::string specialization_key, std::string entry_point,
    uint32_t direct_inputs, uint32_t direct_outputs,
...
GfxOpenClSourceArtifact make_range_artifact(
    const GfxKernelSource &source, std::string specialization_key,
    uint32_t direct_inputs, std::vector<size_t> direct_input_indices) {
...
std::optional<GfxOpenClSourceArtifact> make_opencl_range_source_artifact(
  ...
  if (range_dynamic_i64_unit_supported(node)) {
    ...
    return make_range_artifact(
        source, "opencl:generated:Range:i64:dynamic_unit",
        /*direct_inputs=*/1, {1});
  }
