...
GfxKernelStageFamily gfx_kernel_stage_family_from_kernel_family(GfxKernelFamily family) {
    switch (family) {
        case GfxKernelFamily::EltwiseFusedBuffer:
...
GfxKernelExternalBufferAbiSpec gfx_kernel_external_buffer_abi_spec_for_family(
...
GfxKernelExternalBufferAbiSpec gfx_kernel_external_buffer_abi_spec_for_stage(
    std::string_view stage_type,
    std::string_view entry_point,
    GfxKernelFamily family) {
    if (family == GfxKernelFamily::Conv2DDirectOrIm2col &&
        (stage_type == "Convolution" ||
    ...
    if (family == GfxKernelFamily::GatherScatterIndexed &&
        (stage_type == "ScatterUpdate" || entry_point == "scatter_update_kernel")) {
        return make_gfx_kernel_roles_abi({GfxKernelBufferRole::TensorInput,
                                          GfxKernelBufferRole::TensorInput,
                                          GfxKernelBufferRole::TensorInput,
                                          GfxKernelBufferRole::TensorOutput,
    ...
    if ((family == GfxKernelFamily::GatherScatterIndexed &&
        ...
        if (stage_type == "Slice" || stage_type == "StridedSlice" || entry_point == "slice_kernel") {
            return make_gfx_kernel_roles_abi({GfxKernelBufferRole::TensorInput,
                                              GfxKernelBufferRole::TensorOutput,
                                              GfxKernelBufferRole::RuntimeParams,
                                              GfxKernelBufferRole::RuntimeParams,
...
GfxKernelDispatchPolicy gfx_kernel_dispatch_policy_for_family(GfxKernelFamily family) {
    ...
    return make_gfx_kernel_dispatch_policy(grid,
                                           threads_per_threadgroup,
                                           /*precompiled_binary_required=*/true);
}

GfxKernelDispatchPolicy gfx_kernel_dispatch_policy_for_stage(std::string_view /*stage_type*/,
                                                             std::string_view /*entry_point*/,
                                                             GfxKernelFamily family) {
    return gfx_kernel_dispatch_policy_for_family(family);
}

GfxKernelFamily classify_gfx_custom_kernel_family(std::string_view stage_type,
                                                  std::string_view entry_point) {
    if (stage_type == "Add" ||
        stage_type == "Subtract" ||
        stage_type == "Multiply" ||
...
GfxCustomKernelStagePlan make_gfx_custom_kernel_stage_plan(
    ...
    const std::string required_entry_point = gfx_kernel_required_entry_point(plan.family);
    const uint32_t abi_kernel_family = gfx_kernel_family_abi_id(plan.family);
    const auto external_buffer_abi =
        gfx_kernel_external_buffer_abi_spec_for_stage(stage_type, entry_point, plan.family);
    const auto dispatch_policy =
        gfx_kernel_dispatch_policy_for_stage(stage_type, entry_point, plan.family);

    auto kernel_manifest = make_gfx_custom_kernel_manifest(family_name,
                                                          abi_kernel_family,
    ...
    plan.stage_manifest = make_gfx_custom_kernel_stage_manifest(
        gfx_kernel_stage_family_from_kernel_family(plan.family),
        backend_domain,
        storage,
