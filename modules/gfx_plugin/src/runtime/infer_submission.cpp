...
SubmissionDeviceProfile make_submission_device_profile(const InferSubmissionTuningCaps& caps,
    ...
    case SubmissionDeviceClass::Wide:
        profile.max_slots = 4u;
        profile.stages_per_slot = workload.simd_width >= 64u ? 96u : 64u;
        break;
    }

    if (caps.max_slots_hint != 0u) {
        profile.max_slots = caps.max_slots_hint;
    }
    if (caps.stages_per_slot_hint != 0u) {
        profile.stages_per_slot = caps.stages_per_slot_hint;
    }
    profile.mac_budget_scale_num = std::max<uint64_t>(caps.mac_budget_scale_num, 1u);
    profile.mac_budget_scale_den = std::max<uint64_t>(caps.mac_budget_scale_den, 1u);
    profile.extremely_deep_source_stage_floor =
        caps.extremely_deep_source_stage_floor;
    profile.extremely_deep_source_output_floor =
        caps.extremely_deep_source_output_floor;
    profile.extremely_deep_source_mac_floor =
...
void apply_submission_profile_budget(const SubmissionWorkloadProfile& workload,
...
StageProfileEstimate estimate_stage_profile(const InferStage& stage,
    ...
    if (const auto* descriptor = runtime_stage_descriptor_or_null(stage)) {
        estimate.macs = descriptor->submission_macs_estimate;
    }
    estimate.flops = mul_saturating(estimate.macs, 2);
    return estimate;
}

size_t submission_stage_weight(const InferStage& stage,
                               const GpuStageSubmitPolicy& policy) {
    if (const auto* descriptor = runtime_stage_descriptor_or_null(stage)) {
        return std::max<size_t>(descriptor->submission_stage_weight, 1u);
    }
...
bool is_dependency_extension_boundary(const InferStage& stage) {
    if (const auto* descriptor = runtime_stage_descriptor_or_null(stage)) {
        return descriptor->submission_dependency_boundary;
    }
    return false;
}
...
bool dependency_extension_crosses_boundary(size_t stage_index,
    ...
    const auto& stage = pipeline[stage_index];
    if (is_dependency_extension_boundary(stage)) {
        return true;
    }
    if (stage_index >= graph.input_producers.size()) {
        return false;
    ...
    for (const auto producer_index : graph.input_producers[stage_index]) {
        ...
        if (is_dependency_extension_boundary(producer)) {
            return true;
        }
    }
    return false;
...
InferSubmissionTuning select_infer_submission_tuning(const InferSubmissionTuningCaps& caps, size_t stage_count) {
    ...
    apply_submission_profile_budget(workload, device_profile, budget);
    ...
    if (caps.extremely_deep_dependency_extension_budget_num != 0u &&
        ...
        tuning.config.dependency_extension_budget_num =
            caps.extremely_deep_dependency_extension_budget_num;
        tuning.config.dependency_extension_budget_den =
            std::max<uint32_t>(
                caps.extremely_deep_dependency_extension_budget_den, 1u);
    }
...
void execute_pipeline_with_submission(
    ...
    execute_pipeline(
        ...
        [&](InferStage& stage, const std::vector<GpuTensor*>& resolved) {
            ...
            const size_t stage_weight = submission_stage_weight(stage, policy);
                ...
                if (next_index != std::numeric_limits<size_t>::max()) {
                    if (stage_depends_on_recording_window(next_index, dependency_graph, recorded_stage_mask)) {
                        ...
                        if (!next_policy.isolate &&
                            ...
                            const size_t next_stage_weight =
                                submission_stage_weight(next_stage, next_policy);
                            const auto totals = extended_window_totals(recorded_stage_count,
                                                                       recorded_output_bytes,
                                                                       recorded_macs,
                                                                       next_stage_weight,
