...
#import "runtime/metal_op_depth_to_space.hpp"
...
void MetalDepthToSpaceOp::compile(MetalBufferManager* buffer_manager) {
    ...
    m_pipeline = compiler.compile_msl_from_source(source, "depth_to_space_kernel", log);
    OPENVINO_ASSERT(m_pipeline, "MetalDepthToSpaceOp: failed to compile kernel: ", log);
...
void MetalDepthToSpaceOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    OPENVINO_ASSERT(cb, "MetalDepthToSpaceOp: command buffer is null");
