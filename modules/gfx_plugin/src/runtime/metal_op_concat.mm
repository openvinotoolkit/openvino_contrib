...
void MetalConcatOp::compile(MetalBufferManager* buffer_manager) {
    ...
    m_pipeline = compiler.compile_msl_from_source(source, "concat_kernel", log);
    OPENVINO_ASSERT(m_pipeline, "MetalConcatOp: failed to compile kernel: ", log);
...
void MetalConcatOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    OPENVINO_ASSERT(cb, "MetalConcatOp: command buffer is null");
    OPENVINO_ASSERT(m_pipeline, "MetalConcatOp: pipeline is null");
