...
RuntimeStageExecutableDescriptor make_stage_descriptor(
    size_t stage_index,
    const compiler::ExecutableStageRecord& executable_stage,
    ...
    descriptor.artifact_descriptor_index = executable_stage.artifact_descriptor_index;
    ...
    descriptor.optional_cache_payload_allowed = artifact.optional_cache_payload_allowed;
...
RuntimeExecutableDescriptorVerificationResult RuntimeExecutableDescriptor::verify(
    const compiler::ExecutableBundle& executable) const {
    RuntimeExecutableDescriptorVerificationResult result;
    if (schema_version != kRuntimeExecutableDescriptorSchemaVersion || target_fingerprint.empty()) {
        result.diagnostics.emplace_back("runtime executable descriptor header is incomplete");
    }
    if (target_fingerprint != executable.target_fingerprint) {
        result.diagnostics.emplace_back("runtime executable descriptor target drift");
    }
    if (stages.size() != executable.stages.size()) {
        result.diagnostics.emplace_back("runtime executable descriptor stage count drift");
    }
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (stage.stage_index != i) {
            result.diagnostics.push_back("runtime executable descriptor stage index drift at " +
                                         std::to_string(i));
        }
        if (stage.stage_record_key == 0 || !stage_keys.insert(stage.stage_record_key).second) {
            result.diagnostics.push_back("runtime executable descriptor duplicate or empty stage key at " +
                                         std::to_string(i));
        }
        if (stage.stage_record_key != executable_stage.stage_record_key ||
            stage.artifact_descriptor_index != executable_stage.artifact_descriptor_index) {
            result.diagnostics.push_back("runtime executable descriptor stage drift at " +
                                         std::to_string(i));
        }
        if (stage.artifact_descriptor_index >= executable.artifact_descriptors.size()) {
            result.diagnostics.push_back("runtime executable descriptor artifact index out of range at " +
                                         std::to_string(i));
            continue;
        }

        const auto& artifact = executable.artifact_descriptors[stage.artifact_descriptor_index];
        if (stage.manifest_ref != artifact.manifest_ref ||
            ...
            stage.optional_cache_payload_allowed != artifact.optional_cache_payload_allowed) {
            result.diagnostics.push_back("runtime executable descriptor artifact drift at " +
                                         std::to_string(i));
        }
        if (stage.manifest_ref.empty() || stage.abi_fingerprint.empty() ||
            ...
            result.diagnostics.push_back("runtime executable descriptor has incomplete identity at " +
                                         std::to_string(i));
        }
        if (stage.payload_kind == compiler::KernelArtifactPayloadKind::VendorDescriptor &&
            stage.origin != compiler::KernelArtifactOrigin::VendorPrimitive) {
            result.diagnostics.push_back("runtime executable descriptor vendor payload is not vendor-owned at " +
                                         std::to_string(i));
        }
        if (source_payload_kind(stage.payload_kind) && !source_origin(stage.origin)) {
            result.diagnostics.push_back("runtime executable descriptor source payload has non-source origin at " +
                                         std::to_string(i));
        }
        if (source_payload_kind(stage.payload_kind) &&
            (stage.abi_arg_count == 0 || stage.abi_output_arg_count == 0)) {
            result.diagnostics.push_back("runtime executable descriptor source ABI is incomplete at " +
                                         std::to_string(i));
        }
        const auto executable_payload = executable.find_artifact_payload(stage.artifact_key);
        if (stage.payload != executable_payload) {
            result.diagnostics.push_back("runtime executable descriptor payload drift at " +
                                         std::to_string(i));
        }
        if (stage.payload) {
            if (!stage.payload->valid() ||
                ...
                result.diagnostics.push_back("runtime executable descriptor payload identity drift at " +
                                             std::to_string(i));
            }
        }
    }
...
bool RuntimeExecutableDescriptor::valid(const compiler::ExecutableBundle& executable) const {
...
RuntimeExecutableDescriptor RuntimeExecutableDescriptorBuilder::build(
    ...
    for (size_t i = 0; i < executable.stages.size(); ++i) {
        const auto& executable_stage = executable.stages[i];
        if (executable_stage.artifact_descriptor_index >= executable.artifact_descriptors.size()) {
        ...
        const auto& artifact =
            executable.artifact_descriptors[executable_stage.artifact_descriptor_index];
