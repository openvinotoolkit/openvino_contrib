...
#import "runtime/metal_op_conv3d.hpp"
...
void MetalConv3DOp::compile(MetalBufferManager* buffer_manager) {
    ...
    m_pipeline = compiler.compile_msl_from_source(source, "conv3d_kernel", log);
    OPENVINO_ASSERT(m_pipeline, "MetalConv3DOp: failed to compile conv3d kernel: ", log);
...
void MetalConv3DOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    OPENVINO_ASSERT(cb, "MetalConv3DOp: command buffer is null");
    id<MTLComputeCommandEncoder> enc = [cb computeCommandEncoder];
    [enc setComputePipelineState:m_pipeline];

    [enc setBuffer:to_mtl(src->buf) offset:0 atIndex:0];
    [enc setBuffer:to_mtl(m_weights) offset:0 atIndex:1];
    [enc setBuffer:to_mtl(dst_tensor.buf) offset:0 atIndex:2];
    struct Conv3DParams {
    ...
    MTLSize grid = MTLSizeMake(m_desc.C_out, m_desc.N, 1);
    const NSUInteger tg_size = static_cast<NSUInteger>(metal_clamp_tg_size((void*)m_pipeline, 8));
