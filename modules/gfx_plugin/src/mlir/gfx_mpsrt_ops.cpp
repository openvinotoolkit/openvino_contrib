...
const GfxMpsrtTensorDesc*
find_materialized_tensor_desc(const std::vector<std::pair<GfxMpsrtValue, GfxMpsrtTensorDesc>>& tensors,
                              GfxMpsrtValue value) {
    for (const auto& tensor : tensors) {
        if (tensor.first == value) {
...
std::string mpsrt_stage_op_name(const GfxMpsrtStageDesc& stage) {
    ...
    if (name.empty() || !gfx_mpsrt_stage_has_builder_symbol(stage.kind)) {
        return {};
    }
    return name;
}
...
mlir::func::FuncOp lookup_generated_ops_func(mlir::ModuleOp module) {
    ...
    if (!ops_func || !ops_func->getAttrOfType<mlir::BoolAttr>("gfx.mpsrt.ops.generated")) {
        return {};
    }
    return ops_func;
}

void set_op_tensor_outputs(mlir::Operation* op, mlir::Builder& builder,
                           const std::vector<GfxMpsrtTensorDesc>& outputs) {
    op->setAttr("gfx.mpsrt.op.output_count", builder.getI32IntegerAttr(static_cast<int32_t>(outputs.size())));
    for (size_t i = 0; i < outputs.size(); ++i) {
        detail::gfx_mpsrt_set_tensor_desc_attrs(op, "gfx.mpsrt.op.output" + std::to_string(i), outputs[i]);
    }
}

bool read_op_tensor_outputs(mlir::Operation* op, std::vector<GfxMpsrtTensorDesc>& outputs) {
    outputs.clear();
    ...
    for (uint32_t i = 0; i < output_count; ++i) {
        ...
        if (!detail::gfx_mpsrt_read_tensor_desc_attrs(op, "gfx.mpsrt.op.output" + std::to_string(i), desc)) {
            outputs.clear();
            return false;
        }
...
void set_program_common_attrs(mlir::Operation* op, mlir::Builder& builder, const GfxMpsrtProgram& program) {
    op->setAttr("gfx.mpsrt.ops.version", builder.getI32IntegerAttr(1));
    op->setAttr("gfx.mpsrt.ops.kind", builder.getStringAttr(program.multi_stage ? "multi_stage" : "single_stage"));
    op->setAttr("gfx.mpsrt.ops.record_key", builder.getStringAttr(program.record_key));
    op->setAttr("gfx.mpsrt.ops.input_count", builder.getI32IntegerAttr(static_cast<int32_t>(program.inputs.size())));
    op->setAttr("gfx.mpsrt.ops.stage_count", builder.getI32IntegerAttr(static_cast<int32_t>(program.stages.size())));
    op->setAttr("gfx.mpsrt.ops.output_values", detail::gfx_mpsrt_u32_vector_attr(builder, program.output_values));
    for (size_t i = 0; i < program.inputs.size(); ++i) {
        detail::gfx_mpsrt_set_tensor_desc_attrs(op, "gfx.mpsrt.ops.input" + std::to_string(i), program.inputs[i]);
    }
    if (program.external_buffer_abi.valid) {
        if (program.external_buffer_abi.has_buffer_count) {
            op->setAttr("gfx.mpsrt.ops.external_buffer_count",
                        builder.getI32IntegerAttr(static_cast<int32_t>(program.external_buffer_abi.buffer_count)));
        }
        if (program.external_buffer_abi.has_output_buffer_count) {
            op->setAttr(
                "gfx.mpsrt.ops.external_output_buffer_count",
                builder.getI32IntegerAttr(static_cast<int32_t>(program.external_buffer_abi.output_buffer_count)));
        }
        if (program.external_buffer_abi.has_buffer_roles) {
            detail::gfx_mpsrt_set_typed_program_external_buffer_roles_attrs(op,
                                                                            program.external_buffer_abi.buffer_roles);
        }
    }
    if (program.has_storage_bridges) {
        detail::gfx_mpsrt_set_storage_bridges_attrs(op, "gfx.mpsrt.ops", program.storage_bridges);
    }
}

bool read_program_common_attrs(mlir::Operation* op, GfxMpsrtProgram& out, uint32_t& stage_count) {
    std::string kind;
    ...
    for (uint32_t i = 0; i < input_count; ++i) {
        GfxMpsrtTensorDesc desc{};
        if (!detail::gfx_mpsrt_read_tensor_desc_attrs(op, "gfx.mpsrt.ops.input" + std::to_string(i), desc)) {
            return false;
        }
    ...
    external_buffer_abi.has_buffer_count =
        detail::gfx_mpsrt_read_i32_attr(op, "gfx.mpsrt.ops.external_buffer_count", external_buffer_abi.buffer_count);
    external_buffer_abi.has_output_buffer_count = detail::gfx_mpsrt_read_i32_attr(
        op, "gfx.mpsrt.ops.external_output_buffer_count", external_buffer_abi.output_buffer_count);
    const auto role_values = detail::gfx_mpsrt_read_u32_vector_attr(op, "gfx.mpsrt.ops.external_buffer_roles");
    ...
    if (external_buffer_abi.has_buffer_count || external_buffer_abi.has_output_buffer_count ||
    ...
    if (detail::gfx_mpsrt_read_storage_bridges_attrs(op, "gfx.mpsrt.ops", out.storage_bridges)) {
        out.has_storage_bridges = true;
    }
    return true;
}

bool read_u32_op_attr(mlir::Operation* op, const std::string& name, uint32_t& value) {
    return detail::gfx_mpsrt_read_i32_attr(op, name, value);
}

mlir::Operation* create_mpsrt_op(mlir::OpBuilder& builder, mlir::Location loc, llvm::StringRef name) {
    mlir::OperationState state(loc, name);
    return builder.create(state);
}

bool create_mpsrt_storage_bridge_op(mlir::OpBuilder& builder, mlir::Location loc,
...
bool create_mpsrt_storage_bridge_ops(mlir::OpBuilder& builder, mlir::Location loc,
                                     const std::vector<GfxMpsrtStorageBridgeDesc>& bridges, bool external_inputs) {
    for (const auto& bridge : bridges) {
        const bool is_input_bridge = gfx_mpsrt_storage_bridge_reads_external_buffer(bridge.direction);
...
void erase_module_mpsrt_legacy_attrs(mlir::ModuleOp module) {
    ...
    // adapter surface. Once generated gfx.mpsrt ops exist, the canonical
    // contracts live on typed ops and their stage manifests.
    for (const char* name : {
             "gfx.backend",
             "gfx.storage",
             "gfx.stage_type",
    ...
    for (const auto& named_attr : module->getAttrs()) {
        ...
        if (name.starts_with("gfx.apple.pipeline.")) {
...
bool module_has_mpsrt_ops_program(mlir::ModuleOp module) {
    return static_cast<bool>(lookup_generated_ops_func(module));
}

bool materialize_module_mpsrt_ops(mlir::ModuleOp module, const GfxMpsrtProgram& program) {
    ...
    auto ops_func = mlir::func::FuncOp::create(builder, loc, kGfxMpsrtOpsSymbol, empty_func_type);
    ...
    if (program.has_storage_bridges && !create_mpsrt_storage_bridge_ops(body_builder, loc, program.storage_bridges,
    ...
    for (size_t i = 0; i < program.stages.size(); ++i) {
        ...
        op->setAttr("gfx.mpsrt.op.stage_index", body_builder.getI32IntegerAttr(static_cast<int32_t>(i)));
        detail::gfx_mpsrt_set_stage_desc_attrs(op, "gfx.mpsrt.op.stage", spec.stage);
        detail::gfx_mpsrt_set_stage_manifest_attrs(op, spec.stage.stage_manifest);
        op->setAttr("gfx.mpsrt.op.input_values", detail::gfx_mpsrt_u32_vector_attr(body_builder, spec.inputs));
        op->setAttr("gfx.mpsrt.op.output_values", detail::gfx_mpsrt_u32_vector_attr(body_builder, spec.outputs));
...
bool read_module_mpsrt_ops_program(mlir::ModuleOp module, GfxMpsrtProgram& out) {
    out = {};
    ...
    bool invalid = false;
    std::vector<std::pair<GfxMpsrtValue, GfxMpsrtTensorDesc>> materialized_tensors;
    materialized_tensors.reserve(out.inputs.size());
    for (uint32_t input_index = 0; input_index < out.inputs.size(); ++input_index) {
        materialized_tensors.emplace_back(input_index, out.inputs[input_index]);
    }
    ops_func.walk([&](mlir::Operation* op) {
        ...
        if (op_name == kGfxMpsrtReturnOpName) {
            const auto return_values = detail::gfx_mpsrt_read_u32_vector_attr(op, "gfx.mpsrt.op.output_values");
        ...
        if (!detail::gfx_mpsrt_read_stage_desc_attrs(op, "gfx.mpsrt.op.stage", spec.stage)) {
            invalid = true;
            return;
        }
        ...
        if (expected_name.empty() || mpsrt_stage_op_name(spec.stage).empty() || op_name != expected_name ||
            !read_u32_op_attr(op, "gfx.mpsrt.op.stage_index", stage_index) || stage_index != out.stages.size()) {
            invalid = true;
            return;
        }
        ...
        if (spec.stage.input_storage == GfxMpsrtStorage::Unknown && !spec.inputs.empty()) {
            if (const auto* input_desc = find_materialized_tensor_desc(materialized_tensors, spec.inputs.front())) {
                spec.stage.input_storage = input_desc->storage;
            }
        }
        ...
        if (spec.stage.layout == GfxMpsrtLayout::Unknown && spec.stage.output_storage != GfxMpsrtStorage::Unknown) {
            spec.stage.layout = gfx_mpsrt_stage_layout_for_storage(spec.stage.output_storage);
        }
        if (gfx_mpsrt_stage_record_key(spec.stage).empty()) {
            invalid = true;
            return;
        }
        for (size_t output_index = 0; output_index < spec.outputs.size() && output_index < spec.output_descs.size();
             ++output_index) {
            materialized_tensors.emplace_back(spec.outputs[output_index], spec.output_descs[output_index]);
        }
