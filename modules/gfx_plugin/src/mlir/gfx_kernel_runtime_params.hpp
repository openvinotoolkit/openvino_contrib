...
inline GfxKernelRuntimeParamPayload make_scatter_elements_update_runtime_param_payload(
    GpuBufferManager& buffer_manager,
    std::string_view stage_name,
    const ov::Shape& data_shape,
    const ov::Shape& indices_shape,
    ...
    OPENVINO_ASSERT(data_shape.size() <= 8 && indices_shape.size() <= 8,
                    "GFX MLIR: ScatterElementsUpdate rank exceeds kernel metadata capacity for stage ",
                    stage_name);
    ...
    std::ostringstream suffix;
    suffix << "scatter_elements_update_params/"
           << params.total_data << "x" << params.total_updates << "x" << params.axis;
    GfxKernelRuntimeParamPayload payload;
    payload.extra_inputs.push_back(make_kernel_bytes_param_tensor(buffer_manager,
                                                                  stage_name,
                                                                  suffix.str(),
...
inline GfxKernelRuntimeParamPayload make_scatter_nd_update_runtime_param_payload(
    GpuBufferManager& buffer_manager,
    std::string_view stage_name,
    const ov::Shape& data_shape,
    const ov::Shape& indices_shape,
    ...
    OPENVINO_ASSERT(data_shape.size() <= 8,
                    "GFX MLIR: ScatterNDUpdate rank exceeds kernel metadata capacity for stage ",
                    stage_name);
    OPENVINO_ASSERT(!indices_shape.empty(),
    ...
    OPENVINO_ASSERT(k > 0,
                    ...
                    stage_name);
    OPENVINO_ASSERT(k <= data_shape.size(),
    ...
    std::ostringstream suffix;
    suffix << "scatter_nd_update_params/"
           << params.total_data << "x" << params.num_indices << "x"
           << params.k << "x" << params.inner;
    GfxKernelRuntimeParamPayload payload;
    payload.extra_inputs.push_back(make_kernel_bytes_param_tensor(buffer_manager,
                                                                  stage_name,
                                                                  suffix.str(),
