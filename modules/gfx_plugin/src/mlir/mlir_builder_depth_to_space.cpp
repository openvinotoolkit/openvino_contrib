...
mlir::ModuleOp build_mlir_depth_to_space_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto in_ty = to_mlir_type(d2s->get_input_element_type(0), ctx, /*fallback_f32=*/true);
    auto out_ty = to_mlir_type(d2s->get_output_element_type(0), ctx, /*fallback_f32=*/true);
