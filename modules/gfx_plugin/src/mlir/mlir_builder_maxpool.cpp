                        ...
                        auto for_kw = bkh.create<mlir::scf::ForOp>(
                            ...
                            [&](mlir::OpBuilder& bkw, mlir::Location kw_loc, mlir::Value iv_kw, mlir::ValueRange kw_args) {
                                ...
                                auto if_in = bkw.create<mlir::scf::IfOp>(
                                    kw_loc, kw_args[0].getType(), in_bounds, /*withElse=*/true);
                                {
                                    ...
                                    auto cmp = bkw.create<mlir::arith::CmpFOp>(
                                        kw_loc, mlir::arith::CmpFPredicate::OGT, inp, kw_args[0]);
                                    auto sel = bkw.create<mlir::arith::SelectOp>(kw_loc, cmp, inp, kw_args[0]);
                                    bkw.create<mlir::scf::YieldOp>(kw_loc, sel.getResult());
                                }
                                {
                                    mlir::OpBuilder::InsertionGuard else_guard(bkw);
                                    bkw.setInsertionPointToStart(&if_in.getElseRegion().front());
                                    bkw.create<mlir::scf::YieldOp>(kw_loc, kw_args[0]);
                                }
                                bkw.create<mlir::scf::YieldOp>(kw_loc, if_in.getResult(0));
                            });
                        bkh.create<mlir::scf::YieldOp>(kh_loc, for_kw.getResult(0));
                    });
                bn.create<mlir::memref::StoreOp>(
                    body_loc, for_kh.getResult(0), out_alloc, mlir::ValueRange{iv_n, iv_c, oh, ow});
