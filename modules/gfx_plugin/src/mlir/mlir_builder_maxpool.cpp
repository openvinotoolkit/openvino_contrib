...
mlir::ModuleOp build_mlir_maxpool_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto acc = bow.create<mlir::arith::ConstantOp>(
        loc,
        mlir::FloatAttr::get(elem_ty, -std::numeric_limits<float>::infinity()));
    auto for_kh = bow.create<mlir::scf::ForOp>(loc, c0, kH, c1, mlir::ValueRange{acc.getResult()});
    auto bkh = mlir::OpBuilder::atBlockBegin(for_kh.getBody());
    auto for_kw = bkh.create<mlir::scf::ForOp>(loc,
                                               c0,
                                               kW,
                                               c1,
    ...
    auto inp = bkw.create<mlir::memref::LoadOp>(loc, func.getArgument(0),
                                                mlir::ValueRange{for_n.getInductionVar(),
                                                                 ...
                                                                 ih,
                                                                 iw});
    auto cur = for_kw.getRegionIterArgs()[0];
    auto cmp = bkw.create<mlir::arith::CmpFOp>(loc, mlir::arith::CmpFPredicate::OGT, inp, cur);
    auto sel = bkw.create<mlir::arith::SelectOp>(loc, cmp, inp, cur);
    if (!for_kw.getBody()->empty() && for_kw.getBody()->back().hasTrait<mlir::OpTrait::IsTerminator>()) {
        for_kw.getBody()->back().erase();
    }
    auto bkw_end = mlir::OpBuilder::atBlockEnd(for_kw.getBody());
    bkw_end.create<mlir::scf::YieldOp>(loc, sel.getResult());

    auto kw_res = for_kw.getResult(0);
    if (!for_kh.getBody()->empty() && for_kh.getBody()->back().hasTrait<mlir::OpTrait::IsTerminator>()) {
        for_kh.getBody()->back().erase();
    }
    auto bkh_end = mlir::OpBuilder::atBlockEnd(for_kh.getBody());
    bkh_end.create<mlir::scf::YieldOp>(loc, kw_res);

    bow.setInsertionPoint(for_ow.getBody()->getTerminator());
    bow.create<mlir::memref::StoreOp>(loc, for_kh.getResult(0), out_alloc,
                                      mlir::ValueRange{for_n.getInductionVar(),
                                                       for_c.getInductionVar(),
