...
mlir::ModuleOp build_mlir_batchnorm_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto elem_ty = to_elem_ty(bn->get_input_element_type(0));
    mlir::SmallVector<int64_t> dims(in_shape.begin(), in_shape.end());
    auto inputTy = mlir::RankedTensorType::get(dims, elem_ty);
    auto outputTy = inputTy;
    auto paramTy = mlir::RankedTensorType::get({static_cast<int64_t>(C)}, elem_ty);
    ...
    auto empty = b.create<mlir::tensor::EmptyOp>(loc, dims, elem_ty);
        ...
        mlir::ArrayRef<mlir::utils::IteratorType>(iterators));
    {
        ...
        block->addArguments({elem_ty, elem_ty, elem_ty, elem_ty, elem_ty, elem_ty},
        ...
        auto eps_c = body.create<mlir::arith::ConstantOp>(loc, mlir::FloatAttr::get(elem_ty, eps));
