...
std::string generate_msl_for_reduce(const ReduceCodegenDesc &d,
  ...
  ss << "                           constant uint& OP_CODE [[buffer(4)]],\n";
  ss << "                           constant int* OUT_DIMS [[buffer(5)]],\n";
  ss << "                           constant int* IN_DIMS [[buffer(6)]],\n";
  ss << "                           constant int* IN_STRIDES [[buffer(7)]],\n";
  ss << "                           constant int* AXIS_MASK [[buffer(8)]],\n";
  ss << "                           constant int* REDUCE_DIMS [[buffer(9)]],\n";
  ...
  ss << "    for (uint d = 0; d < RANK; ++d) reduce_size *= (AXIS_MASK[d] ? "
        "(uint)REDUCE_DIMS[d] : 1u);\n";
  ...
  if (d.kind == ReduceKind::LogicalAnd) {
    ss << "        acc = static_cast<scalar_t>(((acc != 0) && (v != 0)) ? 1 : "
          "0);\n";
  } else if (d.kind == ReduceKind::LogicalOr) {
    ss << "        acc = static_cast<scalar_t>(((acc != 0) || (v != 0)) ? 1 : "
          "0);\n";
  } else if (d.kind == ReduceKind::Max) {
    ss << "        if (first) { acc = v; first = false; } else { acc = (v > "
          "acc ? v : acc); }\n";
