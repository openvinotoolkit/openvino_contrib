...
mlir::ModuleOp build_reverse_stub(const std::shared_ptr<const ov::Model>& model, mlir::MLIRContext& ctx) {
    auto& params = model->get_parameters();
    auto& results = model->get_results();
    OPENVINO_ASSERT(params.size() == 1, "Reverse expects single input");
    OPENVINO_ASSERT(results.size() == 1, "Reverse expects single output");
...
mlir::ModuleOp build_mlir_reverse_from_model(const std::shared_ptr<const ov::Model>& model, mlir::MLIRContext& ctx) {
    return build_reverse_stub(model, ctx);
}
