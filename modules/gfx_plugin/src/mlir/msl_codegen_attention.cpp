...
int32_t bitcast_f32_to_i32(float value) {
    int32_t bits = 0;
    static_assert(sizeof(bits) == sizeof(value), "GFX MSL float bitcast size mismatch");
...
GfxMslRuntimeBindingPlan make_causal_sdpa_msl_runtime_binding_plan() {
    auto custom_kernel_plan = make_gfx_custom_kernel_stage_plan("GfxSDPAWithCausalMask",
...
GfxMslRuntimeBindingPlan make_sdpa_msl_runtime_binding_plan(bool has_mask) {
    auto custom_kernel_plan = make_gfx_custom_kernel_stage_plan("ScaledDotProductAttention", "sdpa_kernel");
    if (!custom_kernel_plan.valid) {
        return {};
    }
    return make_msl_runtime_binding_plan_from_stage_manifest(
        custom_kernel_plan.stage_manifest,
        has_mask ? std::vector<size_t>{0, 1, 2, 3}
...
GfxSdpaMslKernelSourcePlan make_sdpa_msl_kernel_source_plan(ov::element::Type type,
    ...
    auto source = make_sdpa_msl_kernel_source(type);
    plan.custom_kernel_plan = make_gfx_custom_kernel_stage_plan("ScaledDotProductAttention",
                                                               source.entry_point);
    ...
    source.entry_point = plan.custom_kernel_plan.stage_manifest.custom_kernel.entry_point;
...
GfxSdpaMslKernelSourcePlan make_causal_sdpa_msl_kernel_source_plan(ov::element::Type type) {
    ...
    plan.custom_kernel_plan = make_gfx_custom_kernel_stage_plan("GfxSDPAWithCausalMask",
                                                               source.entry_point);
    if (!plan.custom_kernel_plan.valid) {
        return {};
    }
...
KernelSource make_sdpa_msl_kernel_source(ov::element::Type type) {
    KernelSource source;
    source.entry_point = "sdpa_kernel";
    source.msl_source = generate_msl_for_sdpa(type);
