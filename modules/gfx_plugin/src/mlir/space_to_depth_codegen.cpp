...
std::string generate_msl_for_space_to_depth(const SpaceToDepthCodegenDesc& d, mlir::ModuleOp module) {
    ...
    ss << "  uint block_area = p.block * p.block;\n";
    ss << "  uint c_in = 0;\n";
    ss << "  uint block_idx = 0;\n";
    ss << "  if (p.mode == 0) {\n";
    ss << "    block_idx = c_out / p.C;\n";
    ss << "    c_in = c_out - block_idx * p.C;\n";
    ss << "  } else {\n";
    ss << "    c_in = c_out / block_area;\n";
    ss << "    block_idx = c_out - c_in * block_area;\n";
    ss << "  }\n";
    ss << "  uint off_h = block_idx / p.block;\n";
    ss << "  uint off_w = block_idx - off_h * p.block;\n";
    ss << "  uint h_in = h_out * p.block + off_h;\n";
    ss << "  uint w_in = w_out * p.block + off_w;\n";
