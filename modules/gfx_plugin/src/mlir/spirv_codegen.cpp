...
struct SpirvCachedMetadata {
  ...
  std::optional<std::string> conv_spatial_input_reuse;
  std::optional<int32_t> conv_spatial_input_reuse_lanes;
  std::optional<int32_t> conv_spatial_input_reuse_unique_width_loads;
  std::optional<int32_t> conv_spatial_input_reuse_saved_width_loads;
  std::optional<bool> conv_multi_kernel_requires_coarse_output_tile_preservation;
  std::optional<bool> conv_multi_kernel_has_workgroup_local_reduction_plan;
  std::optional<int64_t> conv_multi_kernel_coarse_spatial_tile_elements;
  std::optional<int64_t> conv_multi_kernel_coarse_output_channel_block;
  std::optional<int64_t> conv_multi_kernel_coarse_output_tile_elements;
  std::optional<int64_t> conv_multi_kernel_workgroup_output_tile_deficit;
  std::optional<int64_t> conv_multi_kernel_partial_sum_elements;
  std::optional<int64_t> conv_multi_kernel_reduced_accumulator_elements;
  std::optional<int64_t> conv_multi_kernel_owned_intermediate_elements;
  std::optional<int64_t> conv_multi_kernel_owned_intermediate_bytes;
  std::optional<int64_t> conv_multi_kernel_owned_intermediate_buffer_count;
  std::optional<int64_t> conv_multi_kernel_workgroup_local_accumulator_elements;
  std::optional<int64_t> conv_multi_kernel_workgroup_local_accumulator_bytes;
  std::optional<int64_t> conv_multi_kernel_launch_dispatch_count;
};
...
SpirvCachedMetadata capture_spirv_cached_metadata(mlir::ModuleOp module) {
  ...
  if (auto attr = module->getAttrOfType<mlir::StringAttr>(
          "gfx.dispatch_channel_block_accumulation")) {
    meta.dispatch_channel_block_accumulation = attr.getValue().str();
  }
  if (auto attr = module->getAttrOfType<mlir::StringAttr>(
          "gfx.conv_spatial_input_reuse")) {
    meta.conv_spatial_input_reuse = attr.getValue().str();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_spatial_input_reuse_lanes")) {
    meta.conv_spatial_input_reuse_lanes = static_cast<int32_t>(attr.getInt());
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_spatial_input_reuse_unique_width_loads")) {
    meta.conv_spatial_input_reuse_unique_width_loads =
        static_cast<int32_t>(attr.getInt());
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_spatial_input_reuse_saved_width_loads")) {
    meta.conv_spatial_input_reuse_saved_width_loads =
        static_cast<int32_t>(attr.getInt());
  }
  if (auto attr = module->getAttrOfType<mlir::BoolAttr>(
          "gfx.conv_multi_kernel_requires_coarse_output_tile_preservation")) {
    meta.conv_multi_kernel_requires_coarse_output_tile_preservation =
        attr.getValue();
  }
  if (auto attr = module->getAttrOfType<mlir::BoolAttr>(
          "gfx.conv_multi_kernel_has_workgroup_local_reduction_plan")) {
    meta.conv_multi_kernel_has_workgroup_local_reduction_plan =
        attr.getValue();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_coarse_spatial_tile_elements")) {
    meta.conv_multi_kernel_coarse_spatial_tile_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_coarse_output_channel_block")) {
    meta.conv_multi_kernel_coarse_output_channel_block = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_coarse_output_tile_elements")) {
    meta.conv_multi_kernel_coarse_output_tile_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_workgroup_output_tile_deficit")) {
    meta.conv_multi_kernel_workgroup_output_tile_deficit = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_partial_sum_elements")) {
    meta.conv_multi_kernel_partial_sum_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_reduced_accumulator_elements")) {
    meta.conv_multi_kernel_reduced_accumulator_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_owned_intermediate_elements")) {
    meta.conv_multi_kernel_owned_intermediate_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_owned_intermediate_bytes")) {
    meta.conv_multi_kernel_owned_intermediate_bytes = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_owned_intermediate_buffer_count")) {
    meta.conv_multi_kernel_owned_intermediate_buffer_count = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_workgroup_local_accumulator_elements")) {
    meta.conv_multi_kernel_workgroup_local_accumulator_elements = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_workgroup_local_accumulator_bytes")) {
    meta.conv_multi_kernel_workgroup_local_accumulator_bytes = attr.getInt();
  }
  if (auto attr = module->getAttrOfType<mlir::IntegerAttr>(
          "gfx.conv_multi_kernel_launch_dispatch_count")) {
    meta.conv_multi_kernel_launch_dispatch_count = attr.getInt();
  }
  return meta;
}
...
void restore_spirv_cached_metadata(mlir::ModuleOp module,
                                   const SpirvCachedMetadata &meta) {
  if (!module) {
    return;
  }
  auto *ctx = module.getContext();
  // A cache hit returns the SPIR-V produced after lowering. The launch ABI
  // metadata must therefore match the cached binary, even if the current
  // module already carries pre-lowering custom-kernel binding attrs.
  if (!meta.operand_kinds.empty() || !meta.operand_arg_indices.empty() ||
      !meta.scalar_values.empty() || !meta.scalar_args.empty()) {
    replace_spirv_kernel_binding_attrs(module, meta.operand_kinds,
                                       meta.operand_arg_indices,
                                       meta.scalar_values, meta.scalar_args,
                                       /*update_scalar_args=*/true);
  } else {
    restore_spirv_kernel_binding_vectors_if_missing(
        module, meta.operand_kinds, meta.operand_arg_indices,
        meta.scalar_values, meta.scalar_args);
  }
  if (meta.force_single_dispatch) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.force_single_dispatch",
                    b.getBoolAttr(*meta.force_single_dispatch));
  } else {
    module->removeAttr("gfx.force_single_dispatch");
  }
  if (meta.parallel_dispatch) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.parallel_dispatch",
                    b.getBoolAttr(*meta.parallel_dispatch));
  } else {
    module->removeAttr("gfx.parallel_dispatch");
  }
  if (meta.parallel_loop_dims) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.parallel_loop_dims",
                    b.getI32IntegerAttr(*meta.parallel_loop_dims));
  } else {
    module->removeAttr("gfx.parallel_loop_dims");
  }
  if (meta.dispatch_tile_h) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.dispatch_tile_h",
                    b.getI32IntegerAttr(*meta.dispatch_tile_h));
  } else {
    module->removeAttr("gfx.dispatch_tile_h");
  }
  if (meta.dispatch_tile_w) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.dispatch_tile_w",
                    b.getI32IntegerAttr(*meta.dispatch_tile_w));
  } else {
    module->removeAttr("gfx.dispatch_tile_w");
  }
  if (meta.dispatch_threads_h) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.dispatch_threads_h",
                    b.getI32IntegerAttr(*meta.dispatch_threads_h));
  } else {
    module->removeAttr("gfx.dispatch_threads_h");
  }
  if (meta.dispatch_threads_w) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.dispatch_threads_w",
                    b.getI32IntegerAttr(*meta.dispatch_threads_w));
  } else {
  ...
  if (meta.dispatch_channel_block) {
    ...
    module->setAttr("gfx.dispatch_channel_block",
                    b.getI32IntegerAttr(*meta.dispatch_channel_block));
  } else {
  ...
  if (meta.dispatch_channel_block_accumulation) {
    ...
    module->setAttr("gfx.dispatch_channel_block_accumulation",
                    b.getStringAttr(*meta.dispatch_channel_block_accumulation));
  } else {
    module->removeAttr("gfx.dispatch_channel_block_accumulation");
  }
  if (meta.conv_spatial_input_reuse) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.conv_spatial_input_reuse",
                    b.getStringAttr(*meta.conv_spatial_input_reuse));
  } else {
    module->removeAttr("gfx.conv_spatial_input_reuse");
  }
  if (meta.conv_spatial_input_reuse_lanes) {
    mlir::Builder b(ctx);
    module->setAttr("gfx.conv_spatial_input_reuse_lanes",
                    b.getI32IntegerAttr(*meta.conv_spatial_input_reuse_lanes));
  } else {
    module->removeAttr("gfx.conv_spatial_input_reuse_lanes");
  }
  if (meta.conv_spatial_input_reuse_unique_width_loads) {
    mlir::Builder b(ctx);
    module->setAttr(
        "gfx.conv_spatial_input_reuse_unique_width_loads",
        b.getI32IntegerAttr(*meta.conv_spatial_input_reuse_unique_width_loads));
  } else {
    module->removeAttr("gfx.conv_spatial_input_reuse_unique_width_loads");
  }
  if (meta.conv_spatial_input_reuse_saved_width_loads) {
    mlir::Builder b(ctx);
    module->setAttr(
        "gfx.conv_spatial_input_reuse_saved_width_loads",
        b.getI32IntegerAttr(*meta.conv_spatial_input_reuse_saved_width_loads));
  } else {
    module->removeAttr("gfx.conv_spatial_input_reuse_saved_width_loads");
  }
  if (meta.conv_multi_kernel_requires_coarse_output_tile_preservation) {
    mlir::Builder b(ctx);
    module->setAttr(
        "gfx.conv_multi_kernel_requires_coarse_output_tile_preservation",
        b.getBoolAttr(
  ...
  if (meta.conv_multi_kernel_has_workgroup_local_reduction_plan) {
    mlir::Builder b(ctx);
    module->setAttr(
        "gfx.conv_multi_kernel_has_workgroup_local_reduction_plan",
        b.getBoolAttr(
  ...
  auto restore_i64_attr = [&](llvm::StringRef name,
                              const std::optional<int64_t> &value) {
    if (value) {
      mlir::Builder b(ctx);
      module->setAttr(name, b.getI64IntegerAttr(*value));
    } else {
      module->removeAttr(name);
    }
  };
  restore_i64_attr("gfx.conv_multi_kernel_coarse_spatial_tile_elements",
                   meta.conv_multi_kernel_coarse_spatial_tile_elements);
  restore_i64_attr("gfx.conv_multi_kernel_coarse_output_channel_block",
                   meta.conv_multi_kernel_coarse_output_channel_block);
  restore_i64_attr("gfx.conv_multi_kernel_coarse_output_tile_elements",
                   meta.conv_multi_kernel_coarse_output_tile_elements);
  restore_i64_attr("gfx.conv_multi_kernel_workgroup_output_tile_deficit",
                   meta.conv_multi_kernel_workgroup_output_tile_deficit);
  restore_i64_attr("gfx.conv_multi_kernel_partial_sum_elements",
                   meta.conv_multi_kernel_partial_sum_elements);
  restore_i64_attr("gfx.conv_multi_kernel_reduced_accumulator_elements",
                   meta.conv_multi_kernel_reduced_accumulator_elements);
  restore_i64_attr("gfx.conv_multi_kernel_owned_intermediate_elements",
                   meta.conv_multi_kernel_owned_intermediate_elements);
  restore_i64_attr("gfx.conv_multi_kernel_owned_intermediate_bytes",
                   meta.conv_multi_kernel_owned_intermediate_bytes);
  restore_i64_attr("gfx.conv_multi_kernel_owned_intermediate_buffer_count",
                   meta.conv_multi_kernel_owned_intermediate_buffer_count);
  restore_i64_attr(
      "gfx.conv_multi_kernel_workgroup_local_accumulator_elements",
      meta.conv_multi_kernel_workgroup_local_accumulator_elements);
  restore_i64_attr("gfx.conv_multi_kernel_workgroup_local_accumulator_bytes",
                   meta.conv_multi_kernel_workgroup_local_accumulator_bytes);
  restore_i64_attr("gfx.conv_multi_kernel_launch_dispatch_count",
                   meta.conv_multi_kernel_launch_dispatch_count);
...
private:
  std::mutex m_mutex;
  std::unordered_map<SpirvModuleCacheKey, SpirvModuleCacheEntry,
                     SpirvModuleCacheKeyHash>
      m_cache;
};
...
std::array<int32_t, 3> resolve_spirv_local_size(mlir::ModuleOp module) {
  ...
  if (module) {
    if (auto single_dispatch = module->getAttrOfType<mlir::BoolAttr>(
            "gfx.force_single_dispatch")) {
      if (single_dispatch.getValue()) {
        return {1, 1, 1};
      }
    }
    if (auto th = module->getAttrOfType<mlir::IntegerAttr>(
            "gfx.dispatch_threads_h")) {
      local_y = static_cast<int32_t>(th.getInt());
      has_explicit_local_size = true;
    }
    if (auto tw = module->getAttrOfType<mlir::IntegerAttr>(
    ...
    const bool parallel_dispatch =
        ...
        module->getAttrOfType<mlir::BoolAttr>("gfx.parallel_dispatch")
            .getValue();
    if (!parallel_dispatch && !has_explicit_local_size && local_x <= 1 &&
        local_y <= 1) {
  ...
  module.walk([&](mlir::scf::ParallelOp op) {
    ...
    for (int64_t i = 0; i < num_loops; ++i) {
      ...
      if (proc == mlir::gpu::Processor::Sequential && i >= block_start) {
        ...
        case 0:
          proc = mlir::gpu::Processor::BlockX;
          break;
        case 1:
          proc = mlir::gpu::Processor::BlockY;
          break;
        case 2:
          proc = mlir::gpu::Processor::BlockZ;
          break;
        default:
          break;
        }
...
bool convert_gpu_modules_to_spirv_with_math(mlir::ModuleOp module) {
  ...
  auto target_env_supports_kernel_cap = [](mlir::gpu::GPUModuleOp module_op) {
    auto target_attr =
        mlir::spirv::lookupTargetEnvOrDefault(module_op.getOperation());
  ...
  for (auto *gpu_module : gpu_modules) {
    ...
    {
      auto mem_space_map =
          target_env_supports_kernel_cap(
              mlir::dyn_cast<mlir::gpu::GPUModuleOp>(gpu_module))
    ...
    mlir::populateGpuWMMAToSPIRVCoopMatrixKHRConversionPatterns(type_converter,
                                                                patterns);
    ...
    if (mlir::failed(mlir::applyFullConversion(gpu_module, *target,
                                               std::move(patterns)))) {
      return false;
    }
  }
...
void ensure_spirv_entry_point_interface(mlir::spirv::ModuleOp module) {
  ...
  module.walk([&](mlir::spirv::EntryPointOp entry) {
    ...
    entry->setAttr(
        mlir::spirv::EntryPointOp::getInterfaceAttrName(entry->getName()),
...
void apply_spirv_local_size(mlir::ModuleOp module,
  ...
  const bool has_explicit_local_size =
      ...
      module->getAttr("gfx.parallel_dispatch") || local_x > 1 || local_y > 1 ||
      local_size[2] > 1;
  if (!has_explicit_local_size && local_x <= 1 && local_y <= 1 &&
      local_size[2] <= 1) {
    return;
  }
  ...
  auto values = mlir::ArrayAttr::get(
      ctx, {mlir::IntegerAttr::get(mlir::IntegerType::get(ctx, 32),
                                   static_cast<int32_t>(local_x)),
            mlir::IntegerAttr::get(mlir::IntegerType::get(ctx, 32),
                                   static_cast<int32_t>(local_y)),
            mlir::IntegerAttr::get(mlir::IntegerType::get(ctx, 32),
                                   static_cast<int32_t>(local_size[2]))});
  ...
  b.create<mlir::spirv::ExecutionModeOp>(
      spirv_module.getLoc(), mlir::SymbolRefAttr::get(ctx, entry_point),
      exec_mode, values);
}
...
bool validate_spirv_module(mlir::spirv::ModuleOp module, std::string *log) {
  ...
  const auto binding_attr_name = mlir::spirv::SPIRVDialect::getAttributeName(
      mlir::spirv::Decoration::Binding);
  const auto set_attr_name = mlir::spirv::SPIRVDialect::getAttributeName(
      mlir::spirv::Decoration::DescriptorSet);
  const auto builtin_attr_name = mlir::spirv::SPIRVDialect::getAttributeName(
      mlir::spirv::Decoration::BuiltIn);
  ...
  module.walk([&](mlir::spirv::GlobalVariableOp var) {
    ...
    auto binding_attr =
        var->getAttrOfType<mlir::IntegerAttr>(binding_attr_name);
...
bool get_static_strides_and_offset(mlir::MemRefType memref,
  ...
  if (auto layout =
          mlir::dyn_cast<mlir::StridedLayoutAttr>(memref.getLayout())) {
...
std::optional<int64_t> eval_scalar_value(mlir::Value value) {
  ...
  if (auto dim = value.getDefiningOp<mlir::memref::DimOp>()) {
    ...
    if (memref) {
      ...
      if (maybe_index &&
          *maybe_index < static_cast<int64_t>(memref.getRank())) {
        auto shape = memref.getShape();
...
void dump_spirv_mlir_if_requested(mlir::spirv::ModuleOp module,
                                  const std::string &entry_point) {
...
void dump_mlir_if_requested(mlir::ModuleOp module, const char *env_name,
                            const std::string &entry_point,
                            const char *suffix) {
  if (!module || !gfx_log_debug_enabled()) {
    return;
...
struct CompactMemrefAbiInfo {
  size_t total_buffer_args = 0;
...
std::optional<CompactMemrefAbiInfo>
read_compact_memref_abi_info(mlir::ModuleOp module) {
  if (!module) {
    return std::nullopt;
  }
  constexpr llvm::StringLiteral kCompactMemrefAbiEnabledAttr =
      "gfx.spirv.compact_memref_abi.enabled";
  const bool compact_memref_abi_enabled =
      module->hasAttr(kCompactMemrefAbiEnabledAttr);
  bool has_prebuilt_gpu_module = false;
  module.walk([&](mlir::gpu::GPUModuleOp) { has_prebuilt_gpu_module = true; });
  if (has_prebuilt_gpu_module && !compact_memref_abi_enabled) {
    return std::nullopt;
  }
  ...
  if (detail::gfx_mpsrt_read_stage_manifest_attrs(module, manifest) &&
    ...
    CompactMemrefAbiInfo info{};
    for (const auto role : roles) {
      if (is_gfx_kernel_buffer_role(role)) {
        ++info.total_buffer_args;
      }
      if (is_gfx_kernel_output_role(role)) {
        ++info.output_arg_count;
      }
    }
    if (info.total_buffer_args != 0 && info.output_arg_count != 0) {
      if (!compact_memref_abi_enabled) {
        mlir::Builder builder(module.getContext());
        module->setAttr(kCompactMemrefAbiEnabledAttr,
...
void annotate_compact_memref_runtime_arg_indices(mlir::ModuleOp module) {
  ...
  module.walk([&](mlir::FunctionOpInterface func) {
    ...
    const size_t limit = std::min<size_t>(arg_count, operand_kinds.size());
    for (size_t arg_idx = 0; arg_idx < limit; ++arg_idx) {
      if (operand_kinds[arg_idx] != 1 || operand_arg_indices[arg_idx] < 0 ||
...
void rebuild_compact_memref_operand_indices(mlir::ModuleOp module,
                                            const std::string &entry_point) {
  ...
  if (!entry_point.empty()) {
    if (auto f = module.lookupSymbol<mlir::func::FuncOp>(entry_point)) {
      func = f;
                   ...
                   module.lookupSymbol<mlir::gpu::GPUFuncOp>(entry_point)) {
      func = f;
    }
  }
  if (!func) {
  ...
  const size_t total_buffer_args = compact_abi->total_buffer_args;
  if (total_buffer_args == 0) {
    return;
  }
  const size_t output_arg_count = compact_abi->output_arg_count;
  ...
  for (unsigned arg_idx = 0; arg_idx < func.getNumArguments(); ++arg_idx) {
    ...
    if (auto attr = func.getArgAttr(arg_idx, kKernelRuntimeArgIndexAttr)) {
      if (auto idx_attr = mlir::dyn_cast<mlir::IntegerAttr>(attr)) {
        ...
        if (logical_index >= 0 &&
            static_cast<size_t>(logical_index) < operand_arg_indices.size()) {
          operand_arg_indices[static_cast<size_t>(logical_index)] =
  ...
  const size_t expected_untagged =
      total_buffer_args >= output_arg_count ? output_arg_count : 0;
  if (untagged_memref_args.size() != expected_untagged) {
    if (gfx_log_debug_enabled()) {
      gfx_log_debug("MLIR")
          << "Compact ABI arg-index rebuild skipped for " << entry_point
          << ": expected " << expected_untagged << " untagged memref args, saw "
  ...
  for (size_t logical_idx = 0; logical_idx < operand_arg_indices.size();
       ++logical_idx) {
  ...
  replace_spirv_kernel_operand_arg_indices(module, operand_arg_indices);
}
...
void annotate_kernel_scalar_args(mlir::ModuleOp module) {
  ...
  module.walk([&](mlir::gpu::LaunchFuncOp launch) {
    ...
    const bool preserve_compact_buffers =
        has_compact_memref_abi && module->hasAttr("gfx.kernel_operand_kinds");
      ...
      if (mlir::isa<mlir::MemRefType>(type)) {
        ...
        if (auto barg = mlir::dyn_cast<mlir::BlockArgument>(base)) {
          if (auto func = mlir::dyn_cast<mlir::FunctionOpInterface>(
            ...
            if (auto attr = func.getArgAttr(barg.getArgNumber(),
                                            kKernelRuntimeArgIndexAttr)) {
              if (auto idx_attr = mlir::dyn_cast<mlir::IntegerAttr>(attr)) {
          ...
          while (preserved_buffer_pos < existing_kinds.size() &&
                 existing_kinds[preserved_buffer_pos] == 0) {
    ...
    if (gfx_mlir_debug_enabled() && preserve_compact_buffers) {
      std::ostringstream oss;
      oss << "Compact-ABI kernel launch operands:";
      size_t scalar_idx = 0;
      for (auto operand : launch.getKernelOperands()) {
        oss << " [";
    ...
    if (!operand_kinds.empty()) {
      ...
      std::vector<int32_t> final_scalar_values;
      std::vector<int32_t> final_scalar_args;
      if (any_scalar) {
        final_scalar_values.assign(scalar_values.begin(), scalar_values.end());
      }
      ...
      std::vector<int32_t> final_operand_kinds(operand_kinds.begin(),
                                               operand_kinds.end());
      std::vector<int32_t> final_operand_arg_indices(
          operand_arg_indices.begin(), operand_arg_indices.end());
      ...
      replace_spirv_kernel_binding_attrs(
          module, final_operand_kinds, final_operand_arg_indices,
  ...
  if (!saw_launch && gfx_log_debug_enabled()) {
    gfx_log_debug("MLIR")
        << "No gpu.launch_func found for kernel scalar annotation";
  }
}
...
void reconcile_spirv_stage_manifest_with_launch_abi(mlir::ModuleOp module) {
  ...
  const auto operand_arg_indices = extract_kernel_operand_arg_indices(module);
  if (operand_kinds.empty() ||
      (!operand_arg_indices.empty() &&
       operand_arg_indices.size() != operand_kinds.size())) {
    return;
  }

  std::vector<GfxKernelBufferRole> roles;
  roles.reserve(operand_kinds.size());
  bool roles_changed_from_manifest = false;
  const auto scalar_values = extract_kernel_scalar_values(module);
  const auto old_roles = manifest.custom_kernel.external_buffer_abi.valid
                             ? materialize_kernel_external_buffer_roles(
                                   manifest.custom_kernel.external_buffer_abi)
                             : std::vector<GfxKernelBufferRole>{};

  size_t logical_input_count = manifest.semantic_input_roles.empty()
                                   ? 0
                                   : manifest.semantic_input_roles.size();
  size_t semantic_output_count = manifest.semantic_output_roles.empty()
                                     ? 1
                                     : manifest.semantic_output_roles.size();
  if (!old_roles.empty()) {
  ...
  if (semantic_output_count == 0) {
    return;
  }

  const auto &direct_abi = manifest.custom_kernel.external_buffer_abi;
  const bool has_direct_compact_abi =
      direct_abi.roles.empty() && (direct_abi.direct_input_count != 0 ||
                                   direct_abi.direct_output_count != 0);
  if (has_direct_compact_abi) {
    const size_t direct_input_count = direct_abi.direct_input_count;
    const size_t direct_output_count = direct_abi.direct_output_count;
    const size_t direct_buffer_count = direct_input_count + direct_output_count;
    if (direct_input_count == 0 || direct_output_count == 0) {
      return;
    }

    size_t buffer_operand_count = 0;
    for (const auto kind : operand_kinds) {
      if (kind == 1) {
        ++buffer_operand_count;
      } else if (kind != 0) {
        return;
      }
    }
    if (buffer_operand_count != direct_buffer_count) {
      return;
    }
    ...
    for (const auto kind : operand_kinds) {
      if (kind == 0) {
        roles.push_back(GfxKernelBufferRole::ScalarParam);
        continue;
      }
      if (next_buffer < direct_output_count) {
        roles.push_back(GfxKernelBufferRole::TensorOutput);
      } else {
        roles.push_back(GfxKernelBufferRole::TensorInput);
      }
      ++next_buffer;
    }
  } else {

    for (size_t i = 0; i < operand_kinds.size(); ++i) {
      const int32_t kind = operand_kinds[i];
      if (kind == 0) {
        roles.push_back(GfxKernelBufferRole::ScalarParam);
        continue;
      }
      ...
      const int32_t arg_idx = operand_arg_indices.empty()
                                  ? static_cast<int32_t>(i)
                                  : operand_arg_indices[i];
      if (arg_idx < 0) {
        return;
      }
      const size_t logical_idx = static_cast<size_t>(arg_idx);
      if (!old_roles.empty() && logical_idx < old_roles.size()) {
        const auto role = old_roles[logical_idx];
        if (!is_gfx_kernel_buffer_role(role)) {
          return;
        }
      ...
      if (logical_idx < logical_input_count) {
        roles.push_back(GfxKernelBufferRole::TensorInput);
      } else if (logical_idx < logical_input_count + semantic_output_count) {
        roles.push_back(GfxKernelBufferRole::TensorOutput);
      } else {
        roles.push_back(GfxKernelBufferRole::RuntimeParams);
      }
    }
  }

  if (old_roles.size() != roles.size()) {
    roles_changed_from_manifest = true;
  } else {
    for (size_t i = 0; i < roles.size(); ++i) {
      if (old_roles[i] != roles[i]) {
        roles_changed_from_manifest = true;
        break;
      }
    }
  }
  const bool scalar_args_changed =
      manifest.custom_kernel.scalar_args != scalar_values;
  if (!roles_changed_from_manifest && !scalar_args_changed) {
    return;
  }

  if (roles_changed_from_manifest) {
    manifest.custom_kernel.external_buffer_abi =
  ...
  auto plan = make_kernel_runtime_binding_plan_from_stage_manifest(manifest);
  if (plan.valid) {
    plan.runtime_binding.scalar_args = manifest.custom_kernel.scalar_args;
    annotate_kernel_operand_abi_attrs_for_spirv_adapter(module,
...
bool eliminate_memref_cast_chains(mlir::ModuleOp module) {
  ...
  module.walk([&](mlir::UnrealizedConversionCastOp cast_in) {
    ...
    auto ptr_ty = mlir::dyn_cast<mlir::spirv::PointerType>(
        cast_in.getOperand(0).getType());
    auto memref_ty =
        mlir::dyn_cast<mlir::MemRefType>(cast_in.getResult(0).getType());
    if (!ptr_ty || !memref_ty) {
      return;
    }
    ...
    if (auto collapse = mlir::dyn_cast<mlir::memref::CollapseShapeOp>(user)) {
      ...
      to_erase.push_back(user);
                   ...
                   mlir::dyn_cast<mlir::memref::ExpandShapeOp>(user)) {
    ...
    auto cast_out =
        mlir::dyn_cast_or_null<mlir::UnrealizedConversionCastOp>(cast_out_op);
    if (!cast_out || cast_out->getNumResults() != 1) {
      return;
    }
    auto out_ptr_ty = mlir::dyn_cast<mlir::spirv::PointerType>(
        cast_out.getResult(0).getType());
...
std::vector<uint32_t> serialize_spirv(mlir::spirv::ModuleOp spirv_module,
                                      std::string *log) {
...
std::vector<uint32_t> build_stub_spirv(const std::string &entry_point,
                                       std::string *log) {
  ...
  return spirv_module ? serialize_spirv(spirv_module, log)
                      : std::vector<uint32_t>{};
}
...
static std::vector<uint32_t> lower_to_spirv_impl(mlir::ModuleOp module,
  ...
  auto preserved_operand_arg_indices =
  ...
  try {
    run_mlir_pipeline(module, /*use_alloca=*/use_alloca,
                      /*use_parallel_loops=*/use_parallel_loops);
  ...
  restore_spirv_kernel_binding_attrs_if_missing(
      module, preserved_operand_kinds, preserved_operand_arg_indices,
      preserved_scalar_values, preserved_scalar_args);
  dump_mlir_if_requested(module, "OV_GFX_DUMP_MLIR_PRE_SPIRV", entry_point,
                         "_pre_spirv");
  ...
  auto vce = mlir::spirv::VerCapExtAttr::get(mlir::spirv::Version::V_1_0, caps,
                                             exts, ctx);
  auto target_env = mlir::spirv::TargetEnvAttr::get(
      vce, mlir::spirv::getDefaultResourceLimits(ctx),
...

  {
    ...
    const bool supports_kernel =
        target_env_obj.allows(mlir::spirv::Capability::Kernel);
  ...
  const bool has_explicit_parallel_dispatch = static_cast<bool>(
      module->getAttrOfType<mlir::BoolAttr>("gfx.parallel_dispatch"));
  ...
  if (!has_explicit_parallel_dispatch) {
    module->setAttr("gfx.parallel_dispatch",
                    mlir::BoolAttr::get(ctx, use_gpu_dispatch));
  }
  if (!use_gpu_dispatch && !has_explicit_gpu_kernel_ops(module) &&
      (!module->getAttrOfType<mlir::BoolAttr>("gfx.force_single_dispatch") ||
       module->getAttrOfType<mlir::BoolAttr>("gfx.force_single_dispatch")
           .getValue())) {
    module->setAttr("gfx.force_single_dispatch",
  ...
  if (use_gpu_dispatch) {
    ...
    module.walk([&](mlir::gpu::GPUFuncOp func) {
      ...
      } else if (auto known = func->getAttrOfType<mlir::DenseI32ArrayAttr>(
                     "known_block_size")) {
        auto vals = known.asArrayRef();
        if (vals.size() == 3) {
          for (int i = 0; i < 3; ++i) {
            local_size[static_cast<size_t>(i)] = vals[static_cast<size_t>(i)];
                     ...
                     func->getAttrOfType<mlir::ArrayAttr>("known_block_size")) {
        if (known.size() == 3) {
          for (int i = 0; i < 3; ++i) {
            if (auto iattr = mlir::dyn_cast<mlir::IntegerAttr>(known[i])) {
              local_size[static_cast<size_t>(i)] =
                  static_cast<int32_t>(iattr.getInt());
            }
          }
        }
      }
    ...
    if (spirv_debug) {
      module.walk([&](mlir::func::FuncOp func) {
        llvm::errs() << "[GFX][MLIR] SPIR-V type conversion check for "
                     << func.getName() << ":\n";
...

    {
      ...
      target->addIllegalDialect<
          ...
          mlir::memref::MemRefDialect, mlir::scf::SCFDialect,
          mlir::tensor::TensorDialect, mlir::vector::VectorDialect,
          mlir::math::MathDialect, mlir::cf::ControlFlowDialect>();
      target->addLegalOp<mlir::UnrealizedConversionCastOp>();
      if (mlir::failed(mlir::applyPartialConversion(
              module.getOperation(), *target, std::move(patterns)))) {
        if (spirv_debug) {
  ...
  if (mlir::spirv::needsInterfaceVarABIAttrs(target_env)) {
    module.walk([&](mlir::spirv::FuncOp func) {
      for (unsigned i = 0; i < func.getNumArguments(); ++i) {
        ...
        auto ptr_type = mlir::dyn_cast<mlir::spirv::PointerType>(
            func.getArgument(i).getType());
        if (!ptr_type ||
            ptr_type.getStorageClass() == mlir::spirv::StorageClass::Function) {
          continue;
        }
        func.setArgAttr(i, mlir::spirv::getInterfaceVarABIAttrName(),
                        mlir::spirv::getInterfaceVarABIAttr(
                            /*descriptorSet=*/0,
                            /*binding=*/i, std::nullopt, ctx));
      }
    });
  }
  ...
  if (spirv_module &&
      !spirv_module->hasAttr(mlir::spirv::getTargetEnvAttrName())) {
    spirv_module->setAttr(mlir::spirv::getTargetEnvAttrName(), target_env);
  }
  if (!spirv_module) {
    auto addr_model =
        mlir::spirv::getAddressingModel(target_env, /*use64bitAddress=*/false);
    ...
    spirv_module = builder.create<mlir::spirv::ModuleOp>(
        module.getLoc(), addr_model, *mem_model, vce_attr);
    spirv_module->setAttr(mlir::spirv::getTargetEnvAttrName(), target_env);
    ...
    for (auto it = src_ops.begin(); it != src_ops.end();) {
      auto &op = *it++;
      if (llvm::isa<mlir::spirv::FuncOp, mlir::spirv::GlobalVariableOp,
  ...
  if (spirv_module) {
    ...
    if (spv_func &&
        !spv_func->hasAttr(mlir::spirv::getEntryPointABIAttrName())) {
      auto abi = mlir::spirv::getEntryPointABIAttr(
          ctx, resolve_spirv_local_size(module));
      spv_func->setAttr(mlir::spirv::getEntryPointABIAttrName(), abi);
    }

    {
      std::string pre_abi_diag;
      mlir::ScopedDiagnosticHandler pre_abi_handler(
...
std::vector<uint32_t> lower_to_spirv(mlir::ModuleOp module,
  ...
  const SpirvModuleCacheKey cache_key{spirv_hash_string(module_text),
                                      spirv_hash_string(entry_point)};
  if (auto cached = SpirvModuleCache::instance().lookup(cache_key);
      cached.has_value()) {
  ...
  auto spirv = lower_to_spirv_impl(module, entry_point, &primary_log,
                                   /*use_alloca=*/false,
                                   /*use_parallel_loops=*/prefer_parallel);
  if (!spirv.empty()) {
    SpirvModuleCache::instance().store(
        cache_key, {spirv, capture_spirv_cached_metadata(module)});
    if (log) {
      *log = primary_log;
    }
    return spirv;
  ...
  const bool retry_with_alloca =
      primary_log.find("Unresolved unrealized_conversion_cast") !=
          std::string::npos ||
      primary_log.find("failed to legalize operation 'memref.alloc'") !=
          std::string::npos ||
      primary_log.find("MLIR pre-SPIR-V memref normalization failed") !=
          std::string::npos;
  ...
  auto parsed =
      mlir::parseSourceString<mlir::ModuleOp>(module_text, module.getContext());
  if (!parsed) {
    if (log) {
  ...
  auto fallback = lower_to_spirv_impl(*parsed, entry_point, &fallback_log,
                                      /*use_alloca=*/true,
                                      /*use_parallel_loops=*/false);
  if (!fallback.empty()) {
    restore_spirv_cached_metadata(module,
                                  capture_spirv_cached_metadata(*parsed));
    SpirvModuleCache::instance().store(
        cache_key, {fallback, capture_spirv_cached_metadata(*parsed)});
    if (log) {
      *log = fallback_log;
    }
    return fallback;
