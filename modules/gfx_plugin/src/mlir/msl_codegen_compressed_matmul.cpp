...
void write_quantized_weight_value(std::vector<uint8_t>& packed,
    ...
    if (type == ov::element::i4 || type == ov::element::u4) {
        ...
        if ((logical_index & 1u) == 0u) {
            ...
            packed[byte_index] = static_cast<uint8_t>((packed[byte_index] & 0x0fu) |
...
KernelSource make_compressed_matmul_msl_kernel_source(const CompressedMatMulInfo& info,
    ...
    source.msl_source = generate_msl_for_compressed_matmul(info, reduction_threads, output_block);
...
GfxCompressedMatMulMslKernelSourcePlan make_compressed_matmul_msl_kernel_source_plan(
    ...
    auto source = make_compressed_matmul_msl_kernel_source(info, reduction_threads, output_block);
    plan.custom_kernel_plan = make_gfx_custom_kernel_stage_plan("CompressedMatMul", source.entry_point);
