...
bool is_bn_op(mlir::Operation* op) {
    return op && op->getName().getStringRef() == "gfx.BatchNormInference";
}
...
struct ConvBatchNormSwishFusionPattern final : public mlir::RewritePattern {
    ConvBatchNormSwishFusionPattern(mlir::MLIRContext* ctx, FusionConfig config, llvm::StringRef conv_name)
        : mlir::RewritePattern(conv_name, /*benefit=*/5, ctx), m_config(config) {}
    ...
    mlir::LogicalResult matchAndRewrite(mlir::Operation* op,
        ...
        if (sigmoid->getNumOperands() != 1 || sigmoid->getOperand(0) != bn_out) {
            return mlir::failure();
        }
        ...
        const bool mul_ok = (mul->getOperand(0) == bn_out && mul->getOperand(1) == sig_out) ||
                            (mul->getOperand(1) == bn_out && mul->getOperand(0) == sig_out);
        if (!mul_ok) {
            return mlir::failure();
        }
        ...
        if (!conv_idx || !bn_idx || !sig_idx || !mul_idx) {
            return mlir::failure();
        }

        mlir::OperationState state(mul->getLoc(), "gfx.FusedConvBnAct");
...
void add_conv_batchnorm_swish_fusion_patterns(mlir::RewritePatternSet& patterns,
    ...
    patterns.add<ConvBatchNormSwishFusionPattern>(patterns.getContext(), config, "gfx.GroupConvolution");
}
