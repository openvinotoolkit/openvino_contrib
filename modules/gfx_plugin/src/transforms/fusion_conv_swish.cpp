    ...
    mlir::LogicalResult matchAndRewrite(mlir::Operation* op,
        ...
        if (direct_swish) {
            if (sigmoid || mul || direct_swish->getNumOperands() != 1 ||
                direct_swish->getOperand(0) != conv_out) {
                return mlir::failure();
            }
            ...
            fused_nodes = rewriter.getI64ArrayAttr({conv_idx.getInt(), swish_idx.getInt()});
        } else {
            if (!sigmoid || !mul) {
                return mlir::failure();
            }
            ...
            terminal = mul;
            fused_nodes = rewriter.getI64ArrayAttr({conv_idx.getInt(), sig_idx.getInt(), mul_idx.getInt()});
        }

        mlir::OperationState state(terminal->getLoc(), "gfx.FusedConvAct");
