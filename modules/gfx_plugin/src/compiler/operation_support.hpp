...
public:
  ...
  virtual OperationSupportResult
  query_operation(const OperationSupportQuery &query) const = 0;
};
...
public:
  ...
  const BackendTarget &target() const noexcept { return m_target; }

  GpuBackend backend() const noexcept { return m_target.backend(); }
  ...
  const PrecisionCapabilities &precision() const noexcept {
    return m_precision_capabilities;
  }

  const ArtifactFormatCapabilities &artifact_formats() const noexcept {
    return m_artifact_format_capabilities;
  }

  OperationSupportResult
  query_operation(const OperationSupportQuery &query) const;
