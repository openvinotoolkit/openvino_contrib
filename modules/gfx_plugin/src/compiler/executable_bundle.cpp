...
std::string make_manifest_ref(const StageRecord& stage, std::string_view abi_fingerprint) {
    std::ostringstream os;
    os << stage.stable_record_key << ":" << stage.kernel_unit_id << ":" << abi_fingerprint;
    return os.str();
}
...
KernelArtifactDescriptor make_artifact_descriptor(const StageRecord& stage) {
    ...
    descriptor.kernel.tensor_roles.reserve(stage.inputs.size() + stage.outputs.size());
    for (const auto& input : stage.inputs) {
        descriptor.kernel.tensor_roles.push_back(std::string(tensor_contract_role_to_string(input.role)));
    }
    for (const auto& output : stage.outputs) {
        descriptor.kernel.tensor_roles.push_back(std::string(tensor_contract_role_to_string(output.role)));
    }
    descriptor.kernel.scalar_roles.reserve(stage.runtime_params.params.size());
    for (const auto& param : stage.runtime_params.params) {
        descriptor.kernel.scalar_roles.push_back(std::string(runtime_param_kind_to_string(param.kind)) +
                                                 ":" + param.abi_type);
    }
    ...
    if (descriptor.kernel.origin == KernelArtifactOrigin::HandwrittenException) {
        descriptor.kernel.exception_ticket = "registered_exception:" + stage.kernel_unit_id;
    }
    descriptor.abi_fingerprint = make_kernel_abi_fingerprint(descriptor.kernel);
    descriptor.manifest_ref = make_manifest_ref(stage, descriptor.abi_fingerprint);
    descriptor.artifact_key = make_artifact_key(stage,
                                                descriptor.payload_kind,
                                                descriptor.abi_fingerprint);
    return descriptor;
}

std::shared_ptr<const KernelArtifactPayload> materialize_payload_for_stage(
    KernelArtifactDescriptor& descriptor,
...
std::string_view kernel_artifact_origin_to_string(KernelArtifactOrigin origin) noexcept {
    switch (origin) {
...
std::string_view kernel_artifact_payload_kind_to_string(KernelArtifactPayloadKind kind) noexcept {
    switch (kind) {
...
ExecutableBundleVerificationResult ExecutableBundle::verify() const {
    ...
    if (schema_version != kExecutableBundleSchemaVersion || target_fingerprint.empty()) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (executable_stage.stage_record_key != manifest_stage.stable_record_key ||
            ...
            result.diagnostics.push_back("executable bundle stage drift at " + std::to_string(i));
        }
        if (executable_stage.artifact_descriptor_index >= artifact_descriptors.size()) {
            result.diagnostics.push_back("executable bundle artifact index out of range at " + std::to_string(i));
            continue;
        }
        const auto& artifact = artifact_descriptors[executable_stage.artifact_descriptor_index];
        if (artifact.stage_record_key != manifest_stage.stable_record_key ||
            ...
            artifact.kernel.origin != origin_from_route(manifest_stage.execution_kind)) {
            result.diagnostics.push_back("executable bundle artifact drift at " + std::to_string(i));
        }
        if (artifact.manifest_ref.empty()) {
            result.diagnostics.push_back("executable bundle artifact manifest ref is empty at " + std::to_string(i));
        }
        if (artifact.abi_fingerprint.empty()) {
            result.diagnostics.push_back("executable bundle artifact ABI fingerprint is empty at " + std::to_string(i));
        }
        if (artifact.artifact_key.empty()) {
            result.diagnostics.push_back("executable bundle artifact key is empty at " + std::to_string(i));
        }
        if (artifact.kernel.origin == KernelArtifactOrigin::HandwrittenException &&
            artifact.kernel.exception_ticket.empty()) {
            result.diagnostics.push_back("handwritten artifact is missing exception ticket at " + std::to_string(i));
        }
        ...
        if (artifact.payload_kind == KernelArtifactPayloadKind::VendorDescriptor &&
            ...
            result.diagnostics.push_back("vendor payload has non-vendor origin at " + std::to_string(i));
        }
    }
    for (size_t i = 0; i < artifact_payloads.size(); ++i) {
        const auto& payload_record = artifact_payloads[i];
        if (payload_record.artifact_descriptor_index >= artifact_descriptors.size()) {
            result.diagnostics.push_back("executable bundle payload index out of range at " +
                                         std::to_string(i));
            continue;
        }
        ...
        if (payload_record.artifact_key != descriptor.artifact_key) {
            result.diagnostics.push_back("executable bundle payload artifact key drift at " +
                                         std::to_string(i));
        }
        if (!payload_record.payload || !payload_record.payload->valid()) {
            result.diagnostics.push_back("executable bundle payload is missing or invalid at " +
                                         std::to_string(i));
            continue;
        }
        if (payload_record.payload->payload_kind() != descriptor.payload_kind ||
            ...
            result.diagnostics.push_back("executable bundle payload identity drift at " +
                                         std::to_string(i));
        }
    }
    return result;
...
bool ExecutableBundle::valid() const {
    return verify().valid();
}

std::shared_ptr<const KernelArtifactPayload> ExecutableBundle::find_artifact_payload(
...
ExecutableBundleBuilder::ExecutableBundleBuilder(KernelArtifactPayloadResolver resolver)
    : m_payload_resolver(std::move(resolver)) {}

ExecutableBundle ExecutableBundleBuilder::build(const ManifestBundle& manifest) const {
...
ExecutableBundle ExecutableBundleBuilder::build(const ManifestBundle& manifest,
                                                const LoweringPlan& lowering_plan) const {
    auto bundle = build(manifest);
    const size_t count = std::min(bundle.stages.size(), lowering_plan.operations.size());
    for (size_t i = 0; i < count; ++i) {
        ...
        auto payload = materialize_payload_for_stage(descriptor,
                                                     lowering_plan.operations[i],
                                                     m_payload_resolver);
        if (!payload) {
            continue;
        }
