...
std::string stage_key_material(const LoweringPlan& plan, const PlannedOperation& op, size_t stage_id) {
    std::ostringstream os;
    os << plan.target.fingerprint()
       << "#" << stage_id
       << "#" << op.node_name
       << "#" << op.type_name
       << "#" << op.kernel_unit.manifest_key();
    return os.str();
}
...
TensorContract make_tensor_contract(const PlannedOperation& op,
    ...
    contract.logical_name = op.node_name +
...
std::vector<TensorContract> make_input_contracts(const PlannedOperation& op) {
    ...
    for (size_t i = 0; i < op.input_element_types.size(); ++i) {
        const std::string shape = i < op.input_shapes.size() ? op.input_shapes[i] : std::string{};
        contracts.push_back(make_tensor_contract(op,
                                                 TensorContractRole::TensorInput,
                                                 i,
                                                 op.input_element_types[i],
                                                 shape));
    }
    return contracts;
}
...
std::vector<TensorContract> make_output_contracts(const PlannedOperation& op) {
    ...
    for (size_t i = 0; i < op.output_element_types.size(); ++i) {
        const std::string shape = i < op.output_shapes.size() ? op.output_shapes[i] : std::string{};
        contracts.push_back(make_tensor_contract(op,
                                                 TensorContractRole::TensorOutput,
                                                 i,
                                                 op.output_element_types[i],
                                                 shape));
    }
    return contracts;
}
...
void verify_tensor_contract(const TensorContract& contract,
                            const char* label,
                            size_t stage_id,
                            ManifestVerificationResult& result) {
    if (contract.logical_name.empty()) {
        result.diagnostics.push_back("stage " + std::to_string(stage_id) +
                                     " has " + label + " with empty logical name");
    }
    if (contract.element_type.empty()) {
        result.diagnostics.push_back("stage " + std::to_string(stage_id) +
                                     " has " + label + " with empty element type");
    }
    if (contract.partial_shape.empty()) {
        result.diagnostics.push_back("stage " + std::to_string(stage_id) +
                                     " has " + label + " with empty partial shape");
    }
    if (contract.layout.empty() || contract.storage_kind.empty() || contract.lifetime_class.empty()) {
        result.diagnostics.push_back("stage " + std::to_string(stage_id) +
                                     " has " + label + " with incomplete memory contract");
    }
}
...
void verify_runtime_param_contract(const RuntimeParamContract& contract,
                                   size_t stage_id,
                                   ManifestVerificationResult& result) {
    if (contract.scalar_param_count + contract.shape_param_count != contract.params.size() ||
    ...
    for (size_t i = 0; i < contract.params.size(); ++i) {
        ...
        if (param.logical_name.empty() || param.abi_type.empty() || param.source_tensor.empty()) {
            result.diagnostics.push_back("stage " + std::to_string(stage_id) +
...
ManifestVerificationResult ManifestBundle::verify() const {
    ...
    for (size_t i = 0; i < stages.size(); ++i) {
        ...
        if (stage.stage_id != i) {
            result.diagnostics.push_back("stage " + std::to_string(i) + " has non-contiguous stage id");
        }
        ...
        if (stage.memory.input_lifetime.empty() || stage.memory.output_lifetime.empty() ||
...
bool ManifestBundle::valid() const {
    return verify().valid();
}
...
ManifestBundle ManifestBuilder::build(const LoweringPlan& plan) const {
    ...
    for (size_t i = 0; i < plan.operations.size(); ++i) {
        ...
        stage.kernel_unit_kind = std::string(kernel_unit_kind_to_string(op.kernel_unit.kind()));
