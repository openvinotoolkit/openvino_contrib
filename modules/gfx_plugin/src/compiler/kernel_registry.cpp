...
KernelUnit make_common_unit(const BackendTarget& target) {
    return KernelUnit::describe(LoweringRouteKind::Common,
                                KernelUnitKind::Common,
                                "common",
                                target.backend_id(),
                                "common_io");
}
...
bool same_unit_key(const KernelUnit& lhs, const KernelUnit& rhs) {
    return lhs.id() == rhs.id() &&
           lhs.backend_domain() == rhs.backend_domain();
}

}  // namespace

KernelRegistry::KernelRegistry(BackendTarget target, std::vector<KernelUnit> units)
...
KernelUnit KernelRegistry::resolve(LoweringRouteKind route_kind, std::string_view unit_id) const {
...
KernelRegistryAudit KernelRegistry::audit() const {
    ...
    for (size_t i = 0; i < m_units.size(); ++i) {
        ...
        if (!unit.valid() || unit.backend_domain().empty() || unit.op_family().empty()) {
        ...
        if (unit.kind() == KernelUnitKind::HandwrittenException) {
            ...
            if (unit.route_kind() != LoweringRouteKind::HandwrittenKernelException) {
                audit.diagnostics.push_back("handwritten kernel unit has non-exception route: " + unit.id());
            }
        }
