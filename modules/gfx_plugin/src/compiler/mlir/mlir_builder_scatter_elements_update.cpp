...
mlir::ModuleOp build_mlir_scatter_elements_update_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    for (const auto& node : model->get_ordered_ops()) {
        if (ov::as_type_ptr<const ov::op::v3::ScatterElementsUpdate>(node) ||
            ov::as_type_ptr<const ov::op::v12::ScatterElementsUpdate>(node)) {
            OPENVINO_ASSERT(!scatter, "ScatterElementsUpdate MLIR builder: expected single op");
            scatter = node;
        }
    }
    OPENVINO_ASSERT(scatter, "ScatterElementsUpdate MLIR builder: op not found");
    ...
    mlir::SmallVector<int64_t> idx_dims(indices_shape.begin(), indices_shape.end());
    ...
    auto func = mb.create<mlir::func::FuncOp>(mlir::UnknownLoc::get(&ctx), "scatter_elements_update_main", func_type);
    func.addEntryBlock();
