...
mlir::ModuleOp build_mlir_avgpool_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto acc = bow.create<mlir::arith::ConstantOp>(loc, mlir::FloatAttr::get(elem_ty, 0.0f));
    ...
    bkw_end.create<mlir::scf::YieldOp>(loc, sum.getResult());
    ...
    auto avg = bow.create<mlir::arith::DivFOp>(loc, for_kh.getResult(0), kernel_area.getResult());
