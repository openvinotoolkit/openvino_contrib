...
bool allows_stage_kind(std::string_view stage_type, bool convolution_enabled,
                       bool group_convolution_enabled) {
...
query_common_operation(const std::shared_ptr<const ov::Node> &node) {
...
void log_unsupported_operation(const BackendTarget &target,
  ...
  if (!result.semantic_legal && node && gfx_log_debug_enabled()) {
    gfx_log_debug("Compiler")
        << "Unsupported node for " << target.debug_string() << ": "
...
std::string_view
lowering_route_kind_to_string(LoweringRouteKind kind) noexcept {
  switch (kind) {
...
bool PostOpFusionCapabilities::allow_stage_bias_fusion(
    std::string_view stage_type) const {
  return allows_stage_kind(stage_type, enable_bias_fusion_for_convolution,
                           enable_bias_fusion_for_group_convolution);
}

bool PostOpFusionCapabilities::allow_stage_batchnorm_fusion(
    std::string_view stage_type) const {
  return allows_stage_kind(stage_type, enable_batchnorm_fusion_for_convolution,
                           enable_batchnorm_fusion_for_group_convolution);
}

bool PostOpFusionCapabilities::allow_stage_activation_fusion(
    std::string_view stage_type, ActivationKind kind) const {
  if (!allows_stage_kind(stage_type, enable_activation_fusion_for_convolution,
...
BackendCapabilities::BackendCapabilities(
    ...
    : m_target(std::move(target)),
      ...
      m_precision_capabilities(precision_capabilities),
      m_artifact_format_capabilities(artifact_format_capabilities) {}

OperationSupportResult
BackendCapabilities::query_operation(const OperationSupportQuery &query) const {
  if (!query.node) {
