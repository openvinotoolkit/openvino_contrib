...
set(GFX_DEFAULT_BACKEND "auto" CACHE STRING "Default GFX backend (auto, metal, opencl; vulkan is legacy diagnostic)")
set_property(CACHE GFX_DEFAULT_BACKEND PROPERTY STRINGS auto metal opencl vulkan)
...
if(GFX_DEFAULT_BACKEND STREQUAL "auto")
    ...
    elseif(GFX_BACKEND_VULKAN_AVAILABLE)
        message(WARNING "GFX: using legacy Vulkan default because OpenCL is unavailable.")
...
elseif(GFX_DEFAULT_BACKEND STREQUAL "metal")
    if(GFX_BACKEND_METAL_AVAILABLE)
        set(GFX_RESOLVED_DEFAULT_BACKEND "metal")
    elseif(GFX_BACKEND_OPENCL_AVAILABLE)
        message(WARNING "GFX: Metal backend requested but unavailable; falling back to OpenCL.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "opencl")
    elseif(GFX_BACKEND_VULKAN_AVAILABLE)
        message(WARNING "GFX: Metal backend requested but unavailable; falling back to legacy Vulkan because OpenCL is unavailable.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "vulkan")
    else()
...
elseif(GFX_DEFAULT_BACKEND STREQUAL "opencl")
    ...
    elseif(GFX_BACKEND_METAL_AVAILABLE)
        message(WARNING "GFX: OpenCL backend requested but unavailable; falling back to Metal.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "metal")
    elseif(GFX_BACKEND_VULKAN_AVAILABLE)
        message(WARNING "GFX: OpenCL backend requested but unavailable; falling back to legacy Vulkan.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "vulkan")
    else()
        message(WARNING "GFX: OpenCL backend requested but unavailable; defaulting to opencl anyway.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "opencl")
    endif()
elseif(GFX_DEFAULT_BACKEND STREQUAL "vulkan")
    if(GFX_BACKEND_OPENCL_AVAILABLE)
        message(WARNING "GFX: Vulkan backend is legacy; resolving requested Vulkan default to OpenCL.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "opencl")
    elseif(GFX_BACKEND_VULKAN_AVAILABLE)
        message(WARNING "GFX: Vulkan backend is legacy; using it only because OpenCL is unavailable.")
        set(GFX_RESOLVED_DEFAULT_BACKEND "vulkan")
    elseif(GFX_BACKEND_METAL_AVAILABLE)
    ...
    message(FATAL_ERROR "GFX_DEFAULT_BACKEND must be one of: auto, metal, opencl, vulkan")
endif()

message(STATUS "GFX backends: metal=${GFX_BACKEND_METAL_AVAILABLE} (sources=${GFX_HAS_METAL_SOURCES}), opencl=${GFX_BACKEND_OPENCL_AVAILABLE} (sources=${GFX_HAS_OPENCL_SOURCES}), vulkan=${GFX_BACKEND_VULKAN_AVAILABLE} (sources=${GFX_HAS_VULKAN_SOURCES}), default=${GFX_RESOLVED_DEFAULT_BACKEND}, requested=${GFX_DEFAULT_BACKEND}")
