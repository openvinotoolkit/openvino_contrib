...
This exports models, creates runtime venv with pip-installed OpenVINO, builds bev_pool, and runs the benchmark in ~5–10 minutes.
...
## Third-Party Source Sync (projects/)

The `projects/` tree is generated by `setup.sh` and is not maintained as a committed source tree.
`setup.sh` now materializes it as:

1. Upstream base from `https://github.com/Yzichen/FlashOCC.git`
2. Local overlay from patch files in `patches/flashocc/*.patch`

Useful flags:

- `--flashocc-ref <commit|tag|branch>`: choose upstream ref (pin a commit for reproducible runs)
- `--flashocc-patch-dir <path>`: override patch directory
- `--no-sync-thirdparty-projects`: skip materialization step (advanced/debug only)

After sync, `projects/.upstream-source` records the exact upstream commit used.
...
`setup.sh` handles model preparation (if needed), creates Python venv with pip-installed OpenVINO,
...
| `--prepare-models` | Force model download+conversion (creates `venv_convert`) |
| `--model-variant m0\|m1` | Variant for auto-prepare: M0 (14M params) or M1 (larger) (default: `m0`) |
| `--jobs N` | Parallel build/conversion jobs (default: `nproc`) |
| `--run-test` | Build venv + extension + run 80-frame benchmark |
| `--num-samples N` | Frames for `--run-test` (default: 80) |
| `--flashocc-ref REF` | FlashOCC upstream commit/tag/branch (default: `master`) |
| `--no-sync-thirdparty-projects` | Skip FlashOCC projects sync (advanced) |
...
├── setup.sh                              # Full setup: model export → venv + OV install → bev_pool build
...
├── venv_convert/                         # Python 3.10/3.9 venv for model conversion (created by --prepare-models)
...
├── work_dirs/                            # Conversion output (intermediate ONNX, configs)
├── projects/                             # Synced FlashOCC upstream + local patches
├── ov_build/                             # Build logs (git-ignored)
│   └── logs/
└── mmdetection3d/                        # MMDet3D config repo (cloned by --prepare-models, git-ignored)
```

**Key paths in `setup.env`** (sourced by `run_flashocc_ov_ws.sh`):
```bash
FLASHOCC_VENV="/path/to/venv_flashocc_ws"
