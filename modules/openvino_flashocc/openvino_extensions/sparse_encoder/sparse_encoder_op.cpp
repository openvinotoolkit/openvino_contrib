...
static size_t g_bev_sz = 0, g_shb_sz = 0, g_sfb_sz = 0;
...
bool SparseEncoderOp::evaluate(
    ...
    {
        // Download sparse features from GPU: [cur_N, cur_C] as FP16
        ...
        // Precompute valid voxel info to remove branches from hot loop
        struct VoxelInfo { int bev_base; int z; int feat_off; };
        static std::vector<VoxelInfo> valid_voxels;
        valid_voxels.clear();
        valid_voxels.reserve(cur_N);
        for (int idx = 0; idx < cur_N; idx++) {
            int x = coords[idx * 4 + 1];
            int y = coords[idx * 4 + 2];
            int z = coords[idx * 4 + 3];
            if (x >= 0 && x < fin_X && y >= 0 && y < fin_Y && z >= 0 && z < fin_Z) {
                valid_voxels.push_back({x * fin_Y + y, z, idx * cur_C});
            }
        }
        int valid_N = (int)valid_voxels.size();

        // ── Single memset + scatter all voxels (channel-inner loop) ──
        std::memset(bev_out, 0, bev_bytes);

        for (int vi = 0; vi < valid_N; vi++) {
            const auto& v = valid_voxels[vi];
            const uint16_t* src = &s2d_feat_buf[v.feat_off];
            int base_offset = v.z * XY + v.bev_base;
