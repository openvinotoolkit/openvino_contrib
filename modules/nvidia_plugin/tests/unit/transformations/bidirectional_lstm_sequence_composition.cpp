...
std::shared_ptr<ov::op::v5::LSTMSequence> create_lstm_sequence(
    const InputParameters& params,
    const Inputs& inputs,
    const Constants& constants) {
    return std::make_shared<ov::op::v5::LSTMSequence>(inputs.X, inputs.H, inputs.C,
        constants.S, constants.W, constants.R, constants.B,
        params.hidden_size, params.direction,
        params.activations_alpha, params.activations_beta,
        params.activations, params.clip);
}
std::shared_ptr<ov::op::v5::LSTMSequence> create_lstm_sequence(
    const InputParameters& params,
    ...
    const Constants& constants) {
    return std::make_shared<ov::op::v5::LSTMSequence>(X, H, C,
        constants.S, constants.W, constants.R, constants.B,
        params.hidden_size, params.direction,
        params.activations_alpha, params.activations_beta,
        params.activations, params.clip);
...
std::shared_ptr<LSTMSequenceOptimized> create_lstm_sequence_optimized(
    ...
    return std::make_shared<LSTMSequenceOptimized>(inputs.X, inputs.H, inputs.C,
        constants.S, constants.W, constants.R, constants.B,
        params.hidden_size, params.direction, major_format,
        params.activations_alpha, params.activations_beta,
        params.activations, params.clip);
}
Inputs get_inputs(const InputParameters& params, bool transpose_x = false) {
    ...
    if (transpose_x) {
        inputs.X = std::make_shared<op::v0::Parameter>(element::f32, Shape{params.seq_length, params.batch_size, params.input_size});
    } else {
        inputs.X = std::make_shared<op::v0::Parameter>(element::f32, Shape{params.batch_size, params.seq_length, params.input_size});
    }
    inputs.H = std::make_shared<op::v0::Parameter>(element::f32, Shape{params.batch_size, num_directions, params.hidden_size});
    inputs.C = std::make_shared<op::v0::Parameter>(element::f32, Shape{params.batch_size, num_directions, params.hidden_size});
...
Constants get_constants(const InputParameters& params, const uint64_t num_gates = 4) {
    ...
    constants.W = std::make_shared<op::v0::Constant>(element::f32,
                                                Shape{num_directions, num_gates * params.hidden_size, params.input_size},
                                                w_val);
    constants.R = std::make_shared<op::v0::Constant>(element::f32,
                                                Shape{num_directions, num_gates * params.hidden_size, params.hidden_size},
                                                r_val);
    constants.B =
...
TEST(bidirectional_lstm_sequence_composition, bidirectional_lstm_no_ho_co_batch) {
    ...
    InputParameters params;
    {
        ...
        auto reshape_const = op::v0::Constant::create<int64_t>(element::i64, Shape{3},
            {static_cast<int64_t>(params.seq_length), 1, static_cast<int64_t>(params.num_directions()*params.hidden_size)});
        auto reshape = std::make_shared<ov::op::v1::Reshape>(transpose0, reshape_const, false);
        auto transpose1 = get_3d_transpose(reshape);
        model = std::make_shared<Model>(
            OutputVector{transpose1->output(0)},
            inputs.get_parameter_vector());
    }
    {
        auto inputs = get_inputs(params);
        ...
        auto y_reshape_const = op::v0::Constant::create<int32_t>(element::i32, Shape{3},
            {static_cast<int32_t>(params.batch_size), static_cast<int32_t>(params.seq_length),
        ...
        model_ref = std::make_shared<Model>(
            OutputVector{y_reshape->output(0)},
            inputs.get_parameter_vector());
    }
    pass::Manager pass_manager;
    pass_manager.register_pass<pass::InitNodeInfo>();
...
TEST(bidirectional_lstm_sequence_composition, bidirectional_lstm_batch_major_4d_out_h0_c0_fail) {
    ...
    InputParameters params;
    {
        ...
        model = std::make_shared<Model>(
            OutputVector{y_transpose, h_transpose, c_transpose},
            inputs.get_parameter_vector());
        model_ref = model->clone();
    }
    pass::Manager pass_manager;
...
TEST(bidirectional_lstm_sequence_composition, bidirectional_lstm_batch_major_3d_out) {
    ...
    InputParameters params;
    {
        ...
        auto y_reshape_const = op::v0::Constant::create<int64_t>(element::i64, Shape{3}, {
            static_cast<int64_t>(params.seq_length), static_cast<int64_t>(params.batch_size),
        ...
        auto c_transpose = get_3d_transpose(lstm_sequence->output(2));
        model = std::make_shared<Model>(
            OutputVector{y_transpose1, h_transpose, c_transpose},
            inputs.get_parameter_vector());
    }
    {
        auto inputs = get_inputs(params);
        ...
        auto y_reshape = std::make_shared<ov::op::v1::Reshape>(lstm_sequence->output(0), y_reshape_const, false);
        model_ref = std::make_shared<Model>(
            OutputVector{y_reshape, lstm_sequence->output(1), lstm_sequence->output(2)},
            inputs.get_parameter_vector());
    }
    pass::Manager pass_manager;
    pass_manager.register_pass<pass::InitNodeInfo>();
...
TEST(bidirectional_lstm_sequence_composition, bidirectional_lstm_batch_major_matmul) {
    ...
    InputParameters params;
    {
        ...
        auto h_transpose = get_3d_transpose(lstm_sequence->output(1));
        auto c_transpose = get_3d_transpose(lstm_sequence->output(2));
        auto matmul_const = op::v0::Constant::create(element::f32, {80, params.num_directions()*params.input_size}, {1});
        auto matmul = std::make_shared<ov::op::v0::MatMul>(y_reshape, matmul_const, false, true);

        model = std::make_shared<Model>(
            OutputVector{matmul, h_transpose, c_transpose},
            inputs.get_parameter_vector());
    }
    {
        auto inputs = get_inputs(params);
        ...
        auto matmul = std::make_shared<ov::op::v0::MatMul>(y_reshape, matmul_const, false, true);
        model_ref = std::make_shared<Model>(
            OutputVector{matmul->output(0), lstm_sequence->output(1), lstm_sequence->output(2)},
            inputs.get_parameter_vector());
    }
    pass::Manager pass_manager;
    pass_manager.register_pass<pass::InitNodeInfo>();
...
TEST(bidirectional_lstm_sequence_composition, LSTMSequence_compose_batch_major) {
    ...
    InputParameters params;
    {
        ...
        auto concat_0 =
            std::make_shared<op::v0::Concat>(OutputVector{y_forward_transpose, y_reverse_transpose}, 2);
        auto concat_1 =
            std::make_shared<op::v0::Concat>(OutputVector{h_forward_transpose, h_reverse_transpose}, 0);
        auto concat_2 =
            std::make_shared<op::v0::Concat>(OutputVector{c_forward_transpose, c_reverse_transpose}, 0);

        model = std::make_shared<Model>(
            OutputVector{concat_0, concat_1, concat_2},
            inputs.get_parameter_vector());
    }

    {
        ...
        model_ref = std::make_shared<Model>(
            OutputVector{lstm_seq->output(0), lstm_seq->output(1), lstm_seq->output(2)},
            inputs.get_parameter_vector());
    }
    pass::Manager pass_manager;
