...
class TestTraits (CommonTestTraits):
    ...
    def cpp_test_file_begin_tag(self):
        return '{AUTOGENERATED_TESTS_BEGIN_TAG_SWISH}'

    @property
    def cpp_test_file_end_tag(self):
        return '{AUTOGENERATED_TESTS_END_TAG_SWISH}'

    @property
    def template_filename(self):
        return 'swish.cpp.jinja2'
...
class TestParamsProvider (TestParamsProviderBase):
    ...
    @property
    def cpp_shape(self):
        return 'std::pair<std::vector<size_t>, std::vector<size_t>>{{{}, {{}}}}'.format(
            utils.cpp_list_from_tuple_of_ints(self.op.input_ports[0].shape))

    @property
    def has_beta(self):
        return len(self.op.input_ports) > 1
