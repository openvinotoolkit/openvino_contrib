...
static void runConvEltwiseFusion(mlir::ModuleOp module) {
    ...
    if (std::getenv("METAL_MLIR_FUSION_LOG")) {
        llvm::errs() << "[METAL][MLIR] Fusion stats: conv " << beforeConv << " -> " << afterConv
...
void run_mlir_pipeline(mlir::ModuleOp module) {
    ...
    const auto env_debug = std::getenv("METAL_MLIR_DEBUG");
    const bool debug = env_debug && std::string(env_debug) != "0";
    const auto env_pre = std::getenv("METAL_MLIR_DEBUG_PRE_BUFFERIZE");
    ...
    if (debug_pre) {
        llvm::errs() << "[METAL][MLIR] Module before bufferization:\n";
        module.dump();
    }
