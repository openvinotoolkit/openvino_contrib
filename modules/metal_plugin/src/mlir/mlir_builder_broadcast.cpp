...
mlir::ModuleOp build_broadcast_stub(const std::shared_ptr<const ov::Model>& model, mlir::MLIRContext& ctx) {
    ...
    auto func = mb.create<mlir::func::FuncOp>(mlir::UnknownLoc::get(&ctx), "broadcast_main",
...
mlir::ModuleOp build_mlir_broadcast_from_model(const std::shared_ptr<const ov::Model>& model, mlir::MLIRContext& ctx) {
    return build_broadcast_stub(model, ctx);
}
