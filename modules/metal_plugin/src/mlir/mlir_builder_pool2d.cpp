...
mlir::ModuleOp build_mlir_maxpool_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto inp = bkw.create<mlir::memref::LoadOp>(loc, func.getArgument(0),
                                                mlir::ValueRange{for_n.getInductionVar(),
                                                                 ...
                                                                 ih,
                                                                 iw});
    bkw.create<mlir::scf::YieldOp>(loc, inp.getResult());
...
mlir::ModuleOp build_mlir_avgpool_from_model(const std::shared_ptr<const ov::Model>& model,
    ...
    auto after_kw = for_kw.getResult(0);
    auto k_total = b.create<mlir::arith::ConstantOp>(loc, b.getF32FloatAttr(static_cast<float>(kernel[0] * kernel[1])));
    auto avg = bow.create<mlir::arith::DivFOp>(loc, after_kw, k_total);
    bow.create<mlir::memref::StoreOp>(loc, avg, out_alloc,
                                      mlir::ValueRange{for_n.getInductionVar(),
                                                       for_c.getInductionVar(),
                                                       for_oh.getInductionVar(),
                                                       for_ow.getInductionVar()});
