...
#import "runtime/metal_op_gather_elements.hpp"
...
void MetalGatherElementsOp::compile(MetalBufferManager* buffer_manager) {
    ...
    auto module = build_mlir_gather_elements_from_model(make_single_op_model(m_node), ctx);
    auto source = generate_msl_from_mlir(module, m_desc);
    m_pipeline = compiler.compile_msl_from_source(source, "gather_elements_kernel", log);
    OPENVINO_ASSERT(m_pipeline, "MetalGatherElementsOp: failed to compile kernel: ", log);
...
void MetalGatherElementsOp::execute(MetalCommandBufferHandle cmd_buf_handle) {
    ...
    OPENVINO_ASSERT(cb, "MetalGatherElementsOp: command buffer is null");
