...
class METAL_OP_API MetalPadOp : public MetalOp {
public:
    MetalPadOp(const std::shared_ptr<const ov::Node>& node, void* device, void* queue);
    ~MetalPadOp() override = default;
...
private:
    ...
    std::vector<int> m_pads_begin;
    uint32_t m_num_elems = 0;
    float m_pad_value = 0.0f;
};
