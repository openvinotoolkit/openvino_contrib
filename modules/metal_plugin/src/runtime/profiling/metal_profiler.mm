...
MetalProfiler::MetalProfiler(MetalProfilerConfig cfg, MetalDeviceCaps caps, MetalDeviceHandle device)
...
void MetalProfiler::set_config(MetalProfilerConfig cfg) {
    m_cfg = cfg;
...
std::vector<ov::ProfilingInfo> MetalProfiler::export_ov() const {
    ...
    for (const auto& rec : m_nodes) {
        ...
        info.exec_type = rec.exec_type.empty() ? std::string{"METAL"} : rec.exec_type;
...
MetalProfilingReport MetalProfiler::export_extended() const {
    ...
    for (const auto& rec : m_nodes) {
        ...
        entry.exec_type = rec.exec_type.empty() ? std::string{"METAL"} : rec.exec_type;
