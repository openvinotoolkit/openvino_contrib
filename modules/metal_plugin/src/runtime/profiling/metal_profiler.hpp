...
#    define METAL_PROFILER_API __attribute__((visibility("default")))
#else
#    define METAL_PROFILER_API
#endif

class METAL_PROFILER_API MetalProfiler {
public:
    MetalProfiler(MetalProfilerConfig cfg, MetalDeviceCaps caps, MetalDeviceHandle device);

    void set_config(MetalProfilerConfig cfg);
    const MetalProfilerConfig& config() const { return m_cfg; }
