...
MetalLogLevel env_level() {
    if (const char* trace = std::getenv("OV_METAL_TRACE")) {
        if (std::string(trace) != "0")
            return MetalLogLevel::Trace;
    }
    if (const char* dbg = std::getenv("OV_METAL_DEBUG")) {
        if (std::string(dbg) != "0")
...
const MetalLogConfig& metal_log_config() {
    static const MetalLogConfig cfg = []() {
        MetalLogConfig c;
        c.level = env_level();
        const char* test_dbg = std::getenv("OV_METAL_TEST_DEBUG");
...
void metal_log(MetalLogLevel level, const char* category, const std::string& msg) {
    ...
    std::cerr << "[METAL][" << category << "][" << metal_log_level_str(level) << "] "
              << msg << std::endl;
}
