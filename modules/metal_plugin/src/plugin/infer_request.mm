...
void InferRequest::set_input_tensor(size_t idx, const ov::Tensor& tensor) {
    ...
    if (std::dynamic_pointer_cast<ov::IRemoteTensor>(impl._ptr)) {
        auto remote = std::dynamic_pointer_cast<MetalRemoteTensor>(impl._ptr);
        OPENVINO_ASSERT(remote, "METAL: remote tensor type mismatch");
...
void InferRequest::set_tensor(const ov::Output<const ov::Node>& port, const ov::SoPtr<ov::ITensor>& tensor) {
    ...
    if (remote) {
        auto metal_remote = std::dynamic_pointer_cast<MetalRemoteTensor>(remote);
        OPENVINO_ASSERT(metal_remote, "METAL: remote tensor type mismatch");
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        if (inputs[i] == port) {
            ...
            if (remote) {
                m_bound_inputs[i] = {};
                m_bound_remote_inputs[i] = std::dynamic_pointer_cast<MetalRemoteTensor>(remote);
            } else {
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
        if (outputs[i] == port) {
            ...
            if (remote) {
                m_bound_remote_outputs[i] = std::dynamic_pointer_cast<MetalRemoteTensor>(remote);
            } else {
    ...
    if (found.found() && found.is_output()) {
        ...
        if (m_tensor_map.has_output_device(idx)) {
            ...
            if (dev.buf.storage_mode != static_cast<uint32_t>(MTLStorageModeShared)) {
                OPENVINO_THROW("METAL: output buffer is not host-visible");
            }
...
void InferRequest::infer() {
    ...
    auto run_pipeline = [&]() {
        ...
        if (const char* env = std::getenv("OV_METAL_DEBUG_TENSORS")) {
            std::string spec = env;
            if (!spec.empty()) {
                dbg.enabled = true;
                if (spec == "all") {
        ...
        auto safe_check = [&](const char* label,
            ...
            if (!t || !t->buf.valid()) {
                if (require_valid) {
                    OPENVINO_ASSERT(false, "METAL: ", label, " buffer is null");
                }
                return;
            }
            if (shape.empty())
        ...
        for (size_t stage_id = 0; stage_id < descs.size(); ++stage_id) {
            ...
            OPENVINO_ASSERT(stage.op, "METAL: failed to clone op for stage ", desc.node->get_friendly_name());
        ...
        if (!m_bound_remote_outputs.empty()) {
            for (size_t out_idx = 0; out_idx < get_outputs().size(); ++out_idx) {
                ...
                if (auto it = node_map.find(src_node.get()); it != node_map.end()) {
                    ...
                    OPENVINO_ASSERT(src_port < outs.size(), "METAL: remote output port out of range");
                ...
                if (auto pit = param_map.find(src_node.get()); pit != param_map.end()) {
                    ...
                    if (input_idx < m_bound_remote_inputs.size() && m_bound_remote_inputs[input_idx]) {
                        ...
                        OPENVINO_ASSERT(in_buf == out_buf,
                                        "METAL: remote output must alias remote input for passthrough outputs");
                        continue;
                    }
                    OPENVINO_THROW("METAL: remote output cannot be bound to non-remote input passthrough");
                }
                OPENVINO_THROW("METAL: failed to bind remote output ", out_idx, " (pipeline incomplete)");
            }
        }
        ...
        OPENVINO_ASSERT(cq, "METAL: command queue is null");
        ...
        for (const auto& stage : pipeline) {
            ...
            if (metal_log_debug_enabled() || metal_safe_debug_enabled()) {
                ...
                if (metal_log_debug_enabled()) {
                    ...
                    METAL_LOG_DEBUG("PIPELINE", oss.str());
                }
                for (size_t i = 0; i < resolved.size(); ++i) {
                    if (!resolved[i])
                        continue;
        ...
        for (size_t out_idx = 0; out_idx < public_outputs.size(); ++out_idx) {
            ...
            OPENVINO_THROW("METAL: failed to bind output ", out_idx, " (pipeline incomplete)");
        }
    ...
    auto ensure_host_visible = [&](const MetalTensor& src,
        ...
        if (host_override && *host_override) {
            OPENVINO_ASSERT(host_override->get_element_type() == logical_type,
                            "METAL: output tensor type mismatch");
            OPENVINO_ASSERT(host_override->get_shape() == shape,
                            "METAL: output tensor shape mismatch");
            OPENVINO_ASSERT(host_override->data(), "METAL: output tensor has null data");
    ...
    for (size_t idx = 0; idx < get_outputs().size(); ++idx) {
        ...
        if (!m_tensor_map.has_output_device(idx)) {
            OPENVINO_THROW("METAL: output device tensor missing (pipeline incomplete)");
        }
        const auto& dev = m_tensor_map.get_output_device(idx);
        ov::element::Type logical = dev.expected_type == ov::element::dynamic ? dev.buf.type : dev.expected_type;
        ov::Shape shape = dev.shape;
