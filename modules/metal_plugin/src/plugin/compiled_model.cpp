...
CompiledModel::CompiledModel(const std::shared_ptr<const ov::Model>& model,
    ...
    if (auto it = properties.find(kMetalProfilingLevelProperty); it != properties.end()) {
    ...
    OPENVINO_ASSERT(m_command_queue, "METAL: failed to create command queue");
...
ov::Any CompiledModel::get_property(const std::string& name) const {
    ...
    if (ov::model_name == name) {
        ...
        props.push_back(ov::PropertyName{kMetalProfilingLevelProperty, ov::PropertyMutability::RW});
        props.push_back(ov::PropertyName{kMetalProfilingReportProperty, ov::PropertyMutability::RO});
        props.push_back(ov::PropertyName{kMetalMemStatsProperty, ov::PropertyMutability::RO});
    ...
    if (name == kMetalMemStatsProperty) {
        return ov::Any{m_last_stats};
    }
    if (name == kMetalProfilingReportProperty) {
        return last_profiling_report_json();
    }
    if (name == kMetalProfilingLevelProperty) {
...
void CompiledModel::build_op_pipeline() {
    ...
    if (!m_const_manager) {
        METAL_LOG_WARN("OpFactory", "Cannot build pipeline: const manager is null");
        return;
    }
    ...
    for (const auto& node : ordered_ops) {
        ...
        OPENVINO_ASSERT(op,
                        "METAL: unsupported op in MLIR pipeline: ",
