...
uint32_t parse_storage_mode(const ov::AnyMap& params) {
    auto it = params.find("METAL_STORAGE_MODE");
    if (it == params.end())
...
MetalRemoteTensor::MetalRemoteTensor(const ov::element::Type& type,
...
MetalRemoteTensor::~MetalRemoteTensor() {
#ifdef __OBJC__
...
ov::SoPtr<ov::IRemoteTensor> MetalRemoteContext::create_tensor(const ov::element::Type& type,
                                                               const ov::Shape& shape,
                                                               const ov::AnyMap& params) {
    ...
    if (!external) {
        if (auto it = merged.find("METAL_BUFFER"); it != merged.end())
            external = any_to_ptr(it->second);
    }
    ...
    if (external) {
        ...
        OPENVINO_ASSERT(m_handle, "METAL: remote context device handle is null");
#ifdef __OBJC__
        ...
        OPENVINO_ASSERT(buf, "METAL: failed to allocate remote buffer");
        tensor.buf.buffer = buf;
        tensor.buf.size = bytes;
        tensor.buf.storage_mode = static_cast<uint32_t>(buf.storageMode);
        owns_buffer = true;
#else
        OPENVINO_THROW("METAL remote tensor requires Objective-C++ (Metal)");
#endif
    }

    auto t = std::make_shared<MetalRemoteTensor>(type, shape, merged, m_device, tensor, owns_buffer);
