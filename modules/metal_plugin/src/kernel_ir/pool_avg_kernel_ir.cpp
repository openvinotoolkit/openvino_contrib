...
MetalKernelIR build_kernel_ir_for_avgpool(const std::shared_ptr<const ov::Model>& model) {
    MetalKernelIR ir;
    std::shared_ptr<const ov::op::v1::AvgPool> pool_node;
    ...
    OPENVINO_ASSERT(pool_node, "AvgPool builder: AvgPool op not found");
    auto in_shape = pool_node->get_input_shape(0);
    auto out_shape = pool_node->get_output_shape(0);
    OPENVINO_ASSERT(in_shape.size() == 4, "AvgPool2D expects rank-4 NCHW input");
    OPENVINO_ASSERT(pool_node->get_input_element_type(0) == ov::element::f32, "AvgPool only supports f32");
