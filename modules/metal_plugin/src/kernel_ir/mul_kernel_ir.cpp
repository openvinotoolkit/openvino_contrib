...
MetalKernelIR build_kernel_ir_for_mul(const std::shared_ptr<const ov::Model>& model) {
    MetalKernelIR ir;

    std::shared_ptr<const ov::op::v1::Multiply> mul_node;
...
MetalKernelIR build_kernel_ir_for_broadcast_mul(const std::shared_ptr<const ov::Model>& model) {
    MetalKernelIR ir;
    ...
    OPENVINO_ASSERT(mul_node, "Broadcast Multiply builder: no Multiply node found in model");
    OPENVINO_ASSERT(mul_node->get_input_size() == 2, "Multiply must have two inputs");
    OPENVINO_ASSERT(mul_node->get_output_size() == 1, "Multiply must have one output");
    OPENVINO_ASSERT(mul_node->get_input_element_type(0) == ov::element::f32,
                    "Multiply builder: only f32 Multiply is supported");
    ...
    auto shape0 = mul_node->get_input_shape(0);
    auto shape1 = mul_node->get_input_shape(1);
    auto out_shape = mul_node->get_output_shape(0);
