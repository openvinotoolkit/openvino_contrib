...
std::string generate_msl_for_conv3d(const Conv3DCodegenDesc& d, mlir::ModuleOp module) {
    ...
    if (loops.size() < 9) {
        mlir_codegen_log("[METAL MLIR] Conv3D: loop depth " + std::to_string(loops.size()) + " (expected 9) — proceeding with best-effort parse");
    }
    ...
    OPENVINO_ASSERT(input_idx.size() == 5 && weight_idx.size() == 5 && output_idx.size() == 5,
                    "Conv3D MLIR: unexpected index rank");

    auto do_log = []() { return std::getenv("METAL_MLIR_DEBUG") != nullptr || METAL_MLIR_DEBUG; };
    if (do_log()) {
        ...
        mlir_codegen_log("[METAL MLIR] Conv3D func=" + func.getName().str());
