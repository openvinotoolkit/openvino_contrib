...
std::string generate_msl_for_maxpool2d(const Pool2DCodegenDesc& d, mlir::ModuleOp module) {
    OPENVINO_ASSERT(d.N && d.C && d.H && d.W && d.kH && d.kW && d.outH && d.outW, "Pool2D desc incomplete");
    OPENVINO_ASSERT(!d.is_avg, "MaxPool2D codegen expects is_avg=false");
    if (!module)
    ...
    mlir_codegen_log("[METAL MLIR] MaxPool2D func=" + func.getName().str());
